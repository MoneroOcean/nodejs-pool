...
function createNamedPendingJobDatabase(initialStores) {
    const namedStores = new Map();
    for (const [name, entries] of Object.entries(initialStores || {})) {
        namedStores.set(name, new Map(Object.entries(entries)));
    }

    function getStore(name, create) {
        if (!namedStores.has(name)) {
            if (!create) throw new Error("MDB_NOTFOUND");
            namedStores.set(name, new Map());
        }
        return namedStores.get(name);
    }

    function createDbi(name) {
        return {
            name,
            close() {},
            drop(options) {
                options.txn.drop(this, true);
            }
        };
    }
    ...
    class Cursor {
        constructor(txn, dbi) {
            this.entries = Array.from(txn.getStore(dbi).entries());
            this.index = -1;
        }
    ...
    const database = {
        env: {
            openDbi(options) {
                getStore(options.name, options.create !== false);
                return createDbi(options.name);
            },
            beginTxn() {
                return {
                    getStore(dbi) {
                        return getStore(dbi.name, false);
                    },
                    putString(dbi, key, value) {
                        getStore(dbi.name, true).set(key, value);
                    },
                    del(dbi, key) {
                        getStore(dbi.name, false).delete(key);
                    },
                    drop(dbi) {
                        namedStores.delete(dbi.name);
                    },
                    abort() {},
                    commit() {}
                };
...
test("pending block storage migrates legacy remoteSharePendingBlocks rows into pending_blocks", () => {
    const restore = installRemoteShareGlobals();
    const logs = [];
    const now = Date.now();
    const { database, namedStores } = createNamedPendingJobDatabase({
        remoteSharePendingBlocks: {
            "alt:11812:1000:aa": JSON.stringify({
                key: "alt:11812:1000:aa",
                type: "altblock",
                blockId: 11,
                payload: "payload-a",
                createdAt: now,
                nextAttemptAt: now,
                attempts: 1,
                lastError: "waiting_for_depth"
            }),
            "block:12:bb": JSON.stringify({
                key: "block:12:bb",
                type: "block",
                blockId: 12,
                payload: "payload-b",
                ...
                nextAttemptAt: now,
                attempts: 2,
                lastError: "waiting_block_header"
            })
        }
    });
    ...
    try {
        assert.equal(namedStores.has("remoteSharePendingBlocks"), false);
        assert.equal(namedStores.has("pending_blocks"), true);
        assert.equal(namedStores.get("pending_blocks").size, 2);
        assert.equal(pendingJobs.getPendingSummary(), "Pending blocks: total=2 XMR(11812)=1 XMR(18081)=1");
        assert.equal(logs.some((line) => line.includes("Migrated 2 legacy pending block jobs from remoteSharePendingBlocks to pending_blocks")), true);
    } finally {
        pendingJobs.close();
        restore();
    }
