...
test("dual-submit retries keep a Tari child failure labeled as Tari", () => {
    const original = {
        coinFuncs: global.coinFuncs,
        config: global.config,
        consoleError: console.error,
    ...
    const storedBlocks = [];
    const blockData = Buffer.from("01020304", "hex");
    const resultBuff = Buffer.from("22".repeat(32), "hex");
    let auxSubmitCalls = 0;
    let submitCallbackValue = null;
    ...
    global.database = {
        ...
        storeAltBlock() {
            assert.fail("a failed Tari submit must not store an alternate block");
        }
    };
    global.protos = {
        Block: { encode(value) { return value; } },
        AltBlock: { encode(value) { return value; } }
    };
    global.support = {
        sendAdminFyi(key, subject, body) {
        ...
        rpcPortDaemon(port, method, params, callback) {
            ...
            if (port === MAIN_SUBMIT_PORT) return callback({ result: { status: "OK" } }, 200);
            auxSubmitCalls += 1;
            return callback({ result: "unknown" }, 503);
        }
    };
    global.coinFuncs = {
        getPoolProfile() {
            return {
                pool: {
                    ...
                    dualSubmitReportPort: DUAL_SUBMIT_PORT,
                    mainSubmitPort: MAIN_SUBMIT_PORT,
                    dualSubmitPort: DUAL_SUBMIT_PORT,
                    acceptSubmittedBlock({ rpcResult }) {
                        return Boolean(rpcResult && rpcResult.result && rpcResult.result.status === "OK");
                    },
                    resolveSubmittedBlockHash(_context, callback) {
                        callback("aa".repeat(32));
                    },
                    submitBlockRpc(context) {
                        const submit = function submit(port, nextSubmitBlockCB, isDisplaySubmitPort) {
                            context.support.rpcPortDaemon(port, "submitblock", [blockData.toString("hex")], function onSubmit(rpcResult, rpcStatus) {
                                return context.replyDispatcher(rpcResult, rpcStatus, port, nextSubmitBlockCB, isDisplaySubmitPort);
                            }, true);
                        };
                        if (!context.portUsedToSubmit || context.portUsedToSubmit === MAIN_SUBMIT_PORT) {
                            submit(MAIN_SUBMIT_PORT, context.submitBlockCB, false);
                        }
                        if (!context.portUsedToSubmit || context.portUsedToSubmit === DUAL_SUBMIT_PORT) {
                            submit(DUAL_SUBMIT_PORT, null, true);
                        }
                    }
                }
            };
    ...
    global.setTimeout = function immediateTimeout(callback, _delay, ...args) {
        callback(...args);
        return { unref() {} };
    };
    console.error = function ignoreExpectedFailureLog() {};
    ...
    try {
        ...
        helpers.submitBlock(
            { logString: "miner-1", payout: MAIN_WALLET, poolTypeEnum: 0, trust: { trust: 0 } },
            { coin: "" },
            { coin: "", port: MAIN_PORT, height: 100, difficulty: 1, xmr_difficulty: 1, xtm_difficulty: 1 },
            blockData,
            resultBuff,
            100,
            true,
        ...
        assert.equal(submitCallbackValue, true);
        assert.equal(auxSubmitCalls, 2);
        assert.deepEqual(rpcCalls.map((entry) => entry.port), [MAIN_SUBMIT_PORT, DUAL_SUBMIT_PORT, DUAL_SUBMIT_PORT]);
        assert.equal(storedBlocks.length, 1);
        assert.equal(emails.length, 1);
        assert.equal(emails[0].key, `pool:block-submit-rpc:XTM:${  DUAL_SUBMIT_PORT}`);
        assert.match(emails[0].subject, /XTM/);
        assert.match(emails[0].body, /XTM/);
    } finally {
        global.coinFuncs = original.coinFuncs;
        global.config = original.config;
        global.database = original.database;
