...
EOF
    for bin in monerod monero-wallet-cli monero-wallet-rpc; do
        ln -sf /workspace/repo/tests/deploy_support/fake_monerod.js "$dest/build/release/bin/$bin"
    done
}
...
start_fake_chain() {
    ...
    nohup /usr/bin/node /workspace/repo/tests/deploy_support/fake_monerod.js "${args[@]}" >"/tmp/codex-${role}.log" 2>&1 &
}
