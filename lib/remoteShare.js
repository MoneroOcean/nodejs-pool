...
function createRemoteShareRuntime(options) {
    ...
    const pendingJobs = opts.pendingJobs || createPendingJobs({
        database: global.database,
        migrateLegacyPendingJobs: !clusterEnabled || isPrimary
    });
    ...
    function logRequestSummary() {
        ...
        if (failureParts.length > 0) messageParts.push("fail=" + failureParts.join(","));
        ...
        if (rejectParts.length > 0) messageParts.push("reject=" + rejectParts.join(","));
    ...
    return {
        start() {
            ...
            if (!clusterEnabled || isPrimary) {
                state.shareSummaryInterval = setInterval(logMasterSummary, shareSummaryIntervalMs);
