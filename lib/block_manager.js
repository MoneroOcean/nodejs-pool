...
function createBlockManagerRuntime(options) {
    ...
    function sendAdminTemplateEmail(subjectItem, bodyItem, values, subjectFallback, bodyFallback) {
        ...
        if (subject.indexOf("FYI:") === 0 && typeof support.sendFyi === "function") return support.sendFyi(config.general.adminEmail, `block_manager:${  subjectItem}`, subject, body);
    ...
    function isAltblockOrphanResponse(body) {
        ...
        return Boolean(body && (body.orphan_status === true || body.topoheight === -1 || body.confirmations === -1 ||
