...
function full_stop(err) {
    ...
    global.support.sendEmail(global.config.general.adminEmail, "block_manager unable to make balance increase",
      "Hello,\r\nThe block_manager module has hit an issue making a balance increase: " + JSON.stringify(err) +
...
function altblockUnlocker(altblockUnlockerCB) {
    ...
    global.coinFuncs.getLastBlockHeader(function(err, body){
        ...
        async.eachSeries(blockList, function(block, next) {
  	    if (topBlockHeight - block.anchor_height <= 720) return next(); // wait one full day to avoid precalc or too many altblocks that will be merged anyway
            const is_pplns_block = block.poolType == global.protos.POOLTYPE.PPLNS;
            if (is_pplns_block && !(block.hash in payReadyBlockHashCalc) && block.pay_ready !== true) {
                ...
                } else global.support.sendEmail(global.config.general.adminEmail, "FYI: block_manager saw zero value locked block",
                    "Hello,\r\nThe block_manager module saw zero value locked block " + block.hash.toString('hex')
                );
                return next();
                ...
                if (block.pay_value !== 0) {
                    ...
                    global.coinFuncs.getPortBlockHeaderByHash(block.port, block.hash, (err, body) => {
                        if ( ( body.topoheight && body.topoheight === -1) ||
                            ...
                            const profile = global.coinFuncs.getPoolProfile(block.port);
                            if (profile && profile.rpc && profile.rpc.skipHashFallbackByHeight) return next();
        ...
        case global.protos.POOLTYPE.PPLNS:
            global.coinFuncs.getBlockHeaderByHash(block.hash, function (err, header) {
                if (err === null && block.height === header.height && block.value === header.reward && block.difficulty === header.difficulty){
                    ...
                    global.support.sendEmail(global.config.general.adminEmail, "block_manager unable to make blockPayments",
                    "Hello,\r\nThe block_manager module has hit an issue making blockPayments with block " + block.hash.toString('hex'));
                    return cb();
                }
            });
            break;
