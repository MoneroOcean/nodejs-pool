...
function createBlockManagerRuntime(options) {
    ...
    function logInfo(scope, message) { console.log(scope + ": " + message); }
    function logWarn(scope, message) { console.warn(scope + ": " + message); }
    function logError(scope, message) { console.error(scope + ": " + message); }
    ...
    function fullStop(error) {
        ...
        support.sendEmail(
            ...
            "Hello,\r\nThe block_manager module has hit an issue making a balance increase: " + message +
            ".  Please investigate and restart block_manager as appropriate"
        );
    }
    ...
    async function preCalculatePPLNSPayments(blockHexes, blockHeight, blockDifficulty, isStoreDump) {
        ...
        try {
            ...
            for (let currentHeight = blockHeight; currentHeight > 0 && totalPaid < rewardTotal; --currentHeight) {
                ...
                for (let found = cursor.goToRange(currentHeight) === currentHeight; found; found = cursor.goToNextDup()) {
                    cursor.getCurrentBinary(function (_key, data) {
                        ...
                        addPayment(userIdentifier, amountToPayAfterFees);
                        addPayment(config.payout.feeAddress, feesToPay - devDonation - poolDevDonation);
                        addPayment(coinFuncs.poolDevAddress, poolDevDonation);
                        addPayment(coinFuncs.coinDevAddress, devDonation);
        ...
        if (totalPayments === 0) {
            ...
            support.sendEmail(
                ...
                "PPLNS payout cycle for " + formatHashes(blockHexes) + " block does not have any shares so will be redone using top height"
            );
        ...
        if (isNeedCorrection) {
            ...
            support.sendEmail(
                ...
                "PPLNS payout cycle complete on block: " + blockHeight +
                " Payout Percentage: " + (totalPayments / payWindow) * 100 +
                "% (precisely " + totalPayments + " / " + payWindow + ")\n" +
                "(This PPLNS payout cycle complete on block was corrected: " + blockHeight +
                " Payout Percentage: " + (totalPayments / defaultWindow) * 100 +
                "% (precisely " + totalPayments + " / " + defaultWindow + "))"
            );
        }
        return isOk;
    }
    ...
    function notifyBlockPaymentIssue(block) {
        support.sendEmail(
            ...
            "Hello,\r\nThe block_manager module has hit an issue making blockPayments with block " + hashString(block.hash)
        );
    }
        ...
        if (block.pay_ready !== true) {
            if (block.value) {
                ...
                support.sendEmail(
                    ...
                    "Hello,\r\nThe block_manager module saw zero value locked block " + hashString(block.hash)
                );
            }
            return;
        }
    ...
    async function runAltblockUnlocker() {
        ...
        logInfo("Altblock unlocker", "start (" + blockList.length + " locked)");
        ...
        logInfo("Altblock unlocker", "precalc " + Object.keys(preCalcAnchorBlockHashes).length + " anchor height(s)");
        let preCalcCount = 0;
        ...
        if (Object.keys(blockHeightWait).length) {
            logInfo("Altblock unlocker", "waiting pay_value on " + formatAltWaitSummary(blockHeightWait));
        }
    }
