...
function validateNamedProof(ctx) {
    return isRecord(ctx.params) && (typeof ctx.params.job_id === "string" || typeof ctx.params.job_id === "number")
...
/** @typedef {{pearlSolutionId?: (headerHex: string, proof: Buffer) => unknown}} PearlIdentityCoinFuncs */
...
function getPearlSolutionIdentity(ctx) {
    ...
    const proof = isRecord(ctx.params) && typeof ctx.params.plain_proof === "string" ? ctx.params.plain_proof : "";
...
function verifyPearlShare(ctx) {
    ...
    ctx.coinFuncs.verifyPearlAsync(identity.headerHex, proof, targetHex, ctx.miner.payout, function onVerified(result, errorKind) {
        ...
        if (verified.solution_id !== identity.solution_id || verified.config.adjustment_factor !== identity.config.adjustment_factor) {
        ...
        ctx.params["pearl_solution_id"] = verified.solution_id;
