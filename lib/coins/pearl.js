...
/** @typedef {{header: Buffer, target: bigint, targetDecimal: string, targetHex: string, prevHash: string, difficulty: number, compact: number}} ParsedPearlHeader */
/** @typedef {Record<string, unknown> & {height: number, hash: string}} NormalizedPearlHeader */

/** @param {number} workerId @param {(error: unknown, info?: {header: string, parsed: ParsedPearlHeader, reward?: number}) => void} callback */
function requestMiningJob(workerId, callback) {
    pearl.gatewayRequest("getMiningInfo", { worker_id: workerId }, function onMiningInfo(error, body) {
        ...
        return callback(null, { header, parsed, ...(reward === undefined ? {} : { reward }) });
    }, { timeoutMs: pearl.PEARL_GATEWAY_TIMEOUT_MS });
}

/** @param {ParsedPearlHeader} parsed @returns {string} */
function workerTemplateGroup(parsed) {
    // Coinbase variants change only the Merkle root in the 76-byte incomplete header.
    return parsed.header.subarray(0, 36).toString("hex") + parsed.header.subarray(68).toString("hex");
}
...
function getMiningInfo(ctx, callback) {
    requestMiningJob(ctx.runtime.coinFuncs.uniqueWorkerId, function onMiningJob(error, info) {
        if (error || !info) return callback(error || new Error("invalid Pearl mining info"));
        const { header, parsed, reward } = info;
...
function createPearlRpc(config) {
    config.getWorkerBlockTemplate = function getWorkerBlockTemplate(ctx) {
        const base = ctx.baseTemplate;
        if (!Number.isSafeInteger(ctx.workerId) || ctx.workerId < 0 || ctx.workerId > 255 ||
            typeof base["worker_template_group"] !== "string" || typeof base["hash"] !== "string") {
            return ctx.callback(null, new Error("invalid Pearl worker template request"));
        }
        if (ctx.workerId === 0) return ctx.callback(base, null);
        return requestMiningJob(ctx.workerId, function onWorkerJob(error, info) {
            if (error || !info) return ctx.callback(null, error || new Error("invalid Pearl worker job"));
            if (workerTemplateGroup(info.parsed) !== base["worker_template_group"] ||
                info.parsed.targetDecimal !== base["target"] || info.parsed.difficulty !== base["difficulty"] ||
                info.reward !== base["expected_reward"]) {
                return ctx.callback(null, new Error("Pearl worker template changed"));
            }
            return ctx.callback({
                ...base,
                hash: info.header,
    ...
    config.getBlockTemplate = function getBlockTemplate(ctx) {
        getMiningInfo(ctx, function onMiningInfo(error, info) {
            ...
            const templateRecord = {
                ...
                incomplete_header_bytes: info.header,
                worker_template_group: workerTemplateGroup(parsed),
...
function submitPearlBlock(ctx) {
    ...
    pearl.gatewayRequest("submitPlainProof", {
        ...
        mining_job: {
            ...
            target_decimal: ctx.job.targetDecimal,
            cert_version: ctx.job.cert_version,
            worker_id: global.coinFuncs.uniqueWorkerId
        }
