...
function pearlSubmissionKey(job, decoded) {
    ...
    // native verifier accepts both forms as the same proof, so omit one final
    // zero from the duplicate identity as well.  No other valid encoding may
    // differ only by that trailing byte because the proof parser rejects
    // trailing input.
    const identityProof = decoded[decoded.length - 1] === 0
        ? decoded.subarray(0, decoded.length - 1)
        : decoded;
