...
const pearlSubmissionKeys = new WeakMap();
...
/** @param {import("../../types/pool_profiles").PoolJob|undefined} job @param {Buffer} decoded @returns {string} */
function pearlSubmissionKey(job, decoded) {
    // Pearl's legacy dense encoding omits the final Option::None byte.  The
    // dense body commonly ends in zero-valued little-endian bytes itself, so
    // remove the complete terminal zero run.  This gives both encodings one
    // identity.  The native parser rejects trailing bytes for every other
    // valid proof form, so this cannot merge distinct accepted proofs.
    let identityLength = decoded.length;
    while (identityLength > 0 && decoded[identityLength - 1] === 0) --identityLength;
    const identityProof = decoded.subarray(0, identityLength);
    const workIdentity = job && typeof job.incomplete_header_bytes === "string"
        ? job.incomplete_header_bytes
        : job?.blockHash;
    return crypto.createHash("sha256")
        .update("pearl-v3\0")
        .update(String(workIdentity || ""))
        .update("\0")
        .update(identityProof)
        .digest("hex");
}

/** @param {{params: import("../../types/pool_profiles").PoolSubmitParams, wireParams: unknown, job?: import("../../types/pool_profiles").PoolJob}} ctx @returns {boolean} */
function normalizeNamedSubmitParams(ctx) {
    ...
    pearlSubmissionKeys.delete(ctx.params);
    if (ctx.job) pearlSubmissionKeys.set(ctx.params, pearlSubmissionKey(ctx.job, decoded));
...
function buildPearlSubmissionKey(ctx) {
    const normalizedKey = pearlSubmissionKeys.get(ctx.params);
    if (normalizedKey) return normalizedKey;
    const proof = typeof ctx.params.plain_proof === "string" ? ctx.params.plain_proof : "";
    const decoded = pearl.decodePearlProof(proof);
    return pearlSubmissionKey(ctx.job, decoded || Buffer.alloc(0));
}
...
function verifyPearlShare(ctx) {
    ...
    if (!parsed || typeof proof !== "string" || proof.length > pearl.PEARL_MAX_PROOF_BASE64_CHARS || !pearl.isCanonicalBase64(proof) ||
