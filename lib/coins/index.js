...
/** @typedef {{jsonInput: Record<string, unknown>, cb: (result: unknown, errorKind?: string) => void, time: number, miner_address: string, mode?: "pearl", verifyBytes?: number}} VerifyTask */
...
function Coin(data) {
    ...
    this.verifyPearlAsync = function verifyPearlAsync(header, proof, target, miner_address, cb) {
        ...
        return bestQueue.queue.unshift({
            jsonInput: { algo: "pearlhash", header, proof: proof.toString("base64"), target },
            mode: "pearl",
            verifyBytes,
