...
/** @typedef {{jsonInput: {algo: string, blob: string}, cb: (result: unknown, errorKind?: string) => void, time: number, miner_address: string}} VerifyTask */
    ...
    const queue = createTaskQueue(16, /** @param {VerifyTask} task */ function verifyShareRemote(task, queueCB) {
        ...
        socket.on("end", function onEnd() {
            try {
                ...
                if (!("result" in jsonOutput)) return return_cb(false, "verify-host-error");
                health.errorCount = 0;
...
function Coin(data) {
    ...
    if (!mainProfile || !mainProfile.network || !mainProfile.addresses) throw new Error("Main coin profile requires network and address settings");
    const networkConfig = mainProfile.network[global.config.general.testnet === true ? "testnet" : "mainnet"];
    ...
    /** @param {{coin?: string, blob_type_num?: number | string, port?: number}} job */
    ...
    /** @param {string | number} key @param {number} [version] */
    ...
    /** @param {number} port @param {number | undefined} version @param {Coin} context */
    ...
    function applyReservedOffset(target, template, templateConfig, is_mm, blob) {
        if (templateConfig.reserveOffsetSource === "template") {
            target.reserved_offset = template.reserved_offset !== undefined ? template.reserved_offset : template.reservedOffset;
            return;
        }
        ...
        if (found_reserved_offset_template === -1) {
            target.reserved_offset = template.reserved_offset ? template.reserved_offset : template.reservedOffset;
            return;
        }
        const found_reserved_offset = (found_reserved_offset_template >> 1) + 2;
        if (is_mm) target.reserved_offset = found_reserved_offset;
        else if (templateConfig.reserveOffsetSource === "blob" || templateConfig.reserveOffsetSource === "found") target.reserved_offset = found_reserved_offset;
        else if (template.reserved_offset && !template._aux) target.reserved_offset = template.reserved_offset;
        else if (template.reservedOffset) target.reserved_offset = template.reservedOffset;
        else target.reserved_offset = found_reserved_offset;
    }

    function setTemplateBlob(target, template, templateConfig) {
        if (template.blocktemplate_blob) target.blocktemplate_blob = template.blocktemplate_blob;
        else if (template.blob) target.blocktemplate_blob = template.blob;
        else if (templateConfig.hashOnly) {
            const hash = template.hash;
            target.hash = target.idHash = hash;
            target.hash2 = template.hash2;
            target.block_version = 0;
            target.nextBlobHex = function nextBlobHex() { return hash; };
            return false;
        } else {
            console.error(`INTERNAL ERROR: No blob in ${  target.port  } port block template: ${  JSON.stringify(template)}`);
            target.blocktemplate_blob = extra_nonce_mm_template_hex;
        }
        return true;
    }
    ...
    this.BlockTemplate = function BlockTemplate(template) {
        ...
        const auxBaseDifficulty = template._aux && template._aux.base_difficulty !== undefined ? parseInt(template._aux.base_difficulty, 10) : this.difficulty;
        ...
        applyReservedOffset(this, template, templateConfig, is_mm, blob);
    ...
    this.slowHashAsync = function slowHashAsync(convertedBlob, template, miner_address, cb, verifyContext) {
        ...
        let jsonInput = { algo: port2algo[template.port], blob: convertedBlob.toString("hex") };
        if (profile && profile.pow && typeof profile.pow.verifyInput === "function") {
            jsonInput = profile.pow.verifyInput({
                algo: port2algo[template.port],
                blockTemplate: template,
                convertedBlob,
                mixhash,
                nonce,
    ...
    this.slowHashBuffAsync = function slowHashBuffAsync(convertedBlob, template, miner_address, cb, verifyContext) {
        ...
        const callback = isShifted ? miner_address : cb;
        const context = isShifted ? cb : verifyContext;
    ...
    /** @param {number[]} ring @param {number} blob_type_num @param {{port?: number, coin?: string}} [hint] */
    ...
    function normalizeDaemonIssue(issueOrHeight, topHeight, port) {
        if (typeof issueOrHeight === "object" && issueOrHeight !== null) {
            const issue = Object.assign({}, issueOrHeight);
            issue.reason = issue.reason || "template-stuck";
            if (typeof issue.topHeight !== "undefined" && typeof issue.expectedXmrHeight === "undefined") issue.expectedXmrHeight = issue.topHeight;
            if (typeof issue.height !== "undefined" && typeof issue.xmrHeight === "undefined") issue.xmrHeight = issue.height;
            return issue;
        }

        return {
            reason: "template-stuck",
            xmrHeight: issueOrHeight,
            expectedXmrHeight: topHeight,
            port
        };
    }
    ...
    function daemonIssueHeightSummary(issue) {
        const parts = [];
        if (typeof issue.xmrHeight !== "undefined" || typeof issue.expectedXmrHeight !== "undefined") {
            parts.push(`XMR height ${  typeof issue.xmrHeight !== "undefined" ? issue.xmrHeight : "unknown" 
                } expected ${  typeof issue.expectedXmrHeight !== "undefined" ? issue.expectedXmrHeight : "unknown"}`);
        }
        if (typeof issue.xtmHeight !== "undefined" || typeof issue.expectedXtmHeight !== "undefined") {
            parts.push(`XTM height ${  typeof issue.xtmHeight !== "undefined" ? issue.xtmHeight : "unknown" 
                } expected ${  typeof issue.expectedXtmHeight !== "undefined" ? issue.expectedXtmHeight : "unknown"}`);
        }
    ...
    this.fixDaemonIssue = function fixDaemonIssue(issueOrHeight, topHeight, port) {
        ...
        const issuePort = typeof issue.port !== "undefined" ? issue.port : "unknown";
    ...
    this.ethBlockCheck = function ethBlockCheck(port, miner_hex, nonce_hex, block_height_hex, callback) {
        ...
        global.support.rpcPortDaemon2(port, "", { jsonrpc: "2.0", id: 1, method: "eth_getBlockByNumber", params: [block_height_hex, true] }, function onBlock(body) {
            ...
            const resolved_block_height_hex = body.result.number;
            findSeries(Array.from({ length: body.result.uncles.length }, function mapUncle(_value, index) {
                return index;
            }), function eachUncle(index, next) {
                global.support.rpcPortDaemon2(port, "", { jsonrpc: "2.0", id: 1, method: "eth_getUncleByBlockNumberAndIndex", params: [resolved_block_height_hex, `0x${  index.toString(16)}`] }, function onUncle(body_uncle) {
                ...
                // body.result.number is a 0x-prefixed hex quantity; base 10 collapses it to 0.
                const block_height = parseInt(resolved_block_height_hex, 16);
