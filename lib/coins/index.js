...
const { fromBuffer, toBigInt, toBuffer } = require("./helpers.js");
...
if (global.config.verify_shares_host) global.config.verify_shares_host.forEach(function registerVerifyQueue(verify_shares_host, index) {
    ...
    shareVerifyQueue[index] = async.queue(function verifyShareRemote(task, queueCB) {
...
function Coin(data) {
    ...
    this.exchangeAddresses = (mainProfile.addresses.exchange || []).slice();
    this.prefix = networkConfig.prefix;
    this.subPrefix = networkConfig.subPrefix;
    this.intPrefix = networkConfig.intPrefix;
    this.supportsAutoExchange = !!mainProfile.supportsAutoExchange;
    ...
    this.getCoinProfile = function getCoinProfile(key) {
        if (typeof key === "number") return resolveProfile(key, undefined, this);
        if (typeof key === "string" && /^\d+$/.test(key)) return resolveProfile(parseInt(key, 10), undefined, this);
        if (key === "") return mainProfile;
        if (typeof key === "string") return getProfileByCoin(key);
        return null;
    };
    ...
    this.getResolvedProfile = function getResolvedProfile(key, version) {
        if (typeof key === "number") return resolveProfile(key, version, this);
        if (typeof key === "string" && /^\d+$/.test(key)) return resolveProfile(parseInt(key, 10), version, this);
        if (key === "") return mainProfile;
        if (typeof key === "string") return getProfileByCoin(key);
        return null;
    };
    ...
    this.validatePlainAddress = function validatePlainAddress(address) {
        const code = cnUtil.address_decode(Buffer.from(address));
    ...
    this.validateAddress = function validateAddress(address) {
        ...
        return cnUtil.address_decode_integrated(Buffer.from(address)) === this.intPrefix;
    };
    ...
    this.hasTemplateBlob = function hasTemplateBlob(template, port) {
        ...
        if (profile.template && profile.template.hashOnly) return true;
        return !!(template && (template.blocktemplate_blob || template.blob || template.blockhashing_blob));
    };
    ...
    this.getBlockID = function getBlockID(blockBuffer, port) {
        ...
        if (!profile || !profile.blob || typeof profile.blob.getBlockId !== "function") return cnUtil.get_block_id(blockBuffer, 0);
    ...
    this.c29 = function c29(header, ring, port) {
        const profile = resolveProfile(port, undefined, this);
        if (!profile || !profile.pow || typeof profile.pow.c29 !== "function") return multiHashing.c29s(header, ring);
    ...
    this.c29_packed_edges = function c29_packed_edges(ring, blob_type_num, hint) {
        ...
        if (!profile || !profile.pow || typeof profile.pow.packEdges !== "function") return multiHashing.c29s_packed_edges(ring);
    ...
    this.ethBlockCheck = function ethBlockCheck(port, miner_hex, nonce_hex, block_height_hex, callback) {
        ...
        global.support.rpcPortDaemon2(port, "", { jsonrpc: "2.0", id: 1, method: "eth_getBlockByNumber", params: [block_height_hex, true] }, function onBlock(body) {
            ...
            async.eachSeries(Array(body.result.uncles.length).fill().map(function mapUncle(_value, index) {
                return index;
    ...
    this.ethBlockFind = function ethBlockFind(port, nonce_hex, callback) {
        ...
        global.coinFuncs.ethBlockCheck(port, miner_hex, nonce_hex, "latest", function onLatest(block_hash, block_height) {
            ...
            async.eachSeries(Array(32).fill().map(function mapHeight(_value, index) {
    ...
    this.getPortBlockHeaderByID = function getPortBlockHeaderByID(port, blockId, callback) {
        const profile = resolveProfile(port, undefined, this);
        if (!profile || !profile.rpc || typeof profile.rpc.getBlockHeaderById !== "function") return callback(true, null);
        return profile.rpc.getBlockHeaderById({
            blockId: blockId,
            callback: callback,
            port: port,
            profile: profile,
    ...
    this.getBlockHeaderByID = function getBlockHeaderByID(blockId, callback) {
        return this.getPortBlockHeaderByID(global.config.daemon.port, blockId, callback);
    };

    this.getPortAnyBlockHeaderByHash = function getPortAnyBlockHeaderByHash(port, blockHash, is_our_block, callback) {
        const profile = resolveProfile(port, undefined, this);
        if (!profile || !profile.rpc || typeof profile.rpc.getAnyBlockHeaderByHash !== "function") return callback(true, null);
        return profile.rpc.getAnyBlockHeaderByHash({
            blockHash: blockHash,
            callback: callback,
    ...
    this.getPortBlockHeaderByHash = function getPortBlockHeaderByHash(port, blockHash, callback) {
        return this.getPortAnyBlockHeaderByHash(port, blockHash, true, callback);
    };

    this.getBlockHeaderByHash = function getBlockHeaderByHash(blockHash, callback) {
    ...
    this.getPortLastBlockHeader = function getPortLastBlockHeader(port, callback, no_error_report) {
        const profile = resolveProfile(port, undefined, this);
        if (!profile || !profile.rpc || typeof profile.rpc.getLastBlockHeader !== "function") return callback(true, null);
        return profile.rpc.getLastBlockHeader({
            callback: callback,
    ...
    this.getLastBlockHeader = function getLastBlockHeader(callback) {
    ...
    this.getPortBlockTemplate = function getPortBlockTemplate(port, callback) {
        const profile = resolveProfile(port, undefined, this);
        if (!profile || !profile.rpc || typeof profile.rpc.getBlockTemplate !== "function") return callback(null);
        return profile.rpc.getBlockTemplate({
            callback: callback,
            port: port,
            profile: profile,
            runtime: createRuntime(this)
    ...
    this.getBlockTemplate = function getBlockTemplate(callback) {
