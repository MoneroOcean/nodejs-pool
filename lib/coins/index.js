...
function Coin(data) {
    ...
    this.pearlSolutionId = function pearlSolutionId(header, proof) {
        ...
        try {
            const result = powHash.pearl_v3_solution_id(Buffer.from(header, "hex"), proof);
            if (!isRecord(result) || result["valid"] !== true || !Buffer.isBuffer(result["solution_id"]) ||
                result["solution_id"].length !== 32 || !isRecord(result["config"])) return { valid: false };
            return {
                valid: true,
                solution_id: result["solution_id"].toString("hex"),
