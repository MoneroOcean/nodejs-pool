...
function Coin(data) {
    ...
    this.slowHashAsync = function slowHashAsync(convertedBlob, blockTemplate, miner_address, cb, verifyContext) {
        ...
        const profile = resolveProfile(blockTemplate.port, blockTemplate.block_version, this);
        const isKawpow = profile && profile.algo === "kawpow";
        const hashLocal = () => this.slowHash(convertedBlob, blockTemplate, nonce, mixhash);

        if (!global.config.verify_shares_host) return cb(hashLocal());
        const kawpowVerifyIndex = isKawpow ? global.config.verify_shares_host.indexOf("127.0.0.1") : -1;
        if (isKawpow && kawpowVerifyIndex === -1) return cb(hashLocal());
        ...
        if (!isKawpow && best_index === null) shareVerifyQueueErrorTime.forEach(function findLeastBad(last_error_time, index) {
        ...
        return shareVerifyQueue[best_index].unshift({
            jsonInput: jsonInput,
            cb(result, errorKind) {
                if (isKawpow && errorKind) return cb(hashLocal());
                return cb(result);
            },
            time: time_now,
    ...
    this.fixDaemonIssue = function fixDaemonIssue(issueOrHeight, topHeight, port) {
        ...
        const height = typeof issue.xmrHeight !== "undefined" ? issue.xmrHeight : "unknown";
        const expectedHeight = typeof issue.expectedXmrHeight !== "undefined" ? issue.expectedXmrHeight : "unknown";
        ...
        global.support.sendEmail(
            ...
            height + " is behind expected height (" + expectedHeight + ") amongst other leaf nodes for " +
