...
// This module owns all XMR-family coin behavior. Shared pool code should only
// talk to the interface exposed here and keep coin-specific branching out of
// lib/pool/.
...
const coinConfig = require("./metadata.json");
const createConstants = require("./constants");
const { arr2hex, calcErgReward, calcEthReward, fromBuffer, toBigInt, toBuffer } = require("./helpers");
...
function Coin(data){
    ...
    this.coinDevAddress = coinConfig.addresses.coinDev;
    this.poolDevAddress = coinConfig.addresses.poolDev;
    this.blockedAddresses = [this.coinDevAddress, this.poolDevAddress].concat(coinConfig.addresses.blocked);
    this.exchangeAddresses = coinConfig.addresses.exchange.slice();

    const networkConfig = global.config.general.testnet === true ? coinConfig.network.testnet : coinConfig.network.mainnet;
    this.prefix = networkConfig.prefix;
    this.subPrefix = networkConfig.subPrefix;
    this.intPrefix = networkConfig.intPrefix;

    this.supportsAutoExchange = coinConfig.supportsAutoExchange;
    this.niceHashDiff = coinConfig.niceHashDiff;
    ...
    this.hasTemplateBlob = function(template, port) {
        const blob_type_num = this.portBlobType(port, 0);
        if (this.blobTypeEth(blob_type_num) || this.blobTypeErg(blob_type_num)) return true;
    ...
    this.constructNewBlob = function(blockTemplateBuffer, params, port) {
       ...
       if (global.coinFuncs.blobTypeXTM_C(blob_type_num)) {
            ...
            return cnUtil.construct_block_blob(blockTemplateBuffer,
                toBuffer(params.nonce, {endian: 'little', size: 4}, 10),
            ...
            return cnUtil.constructNewRavenBlob(blockTemplateBuffer,
                toBuffer(params.nonce, {endian: 'little', size: 8}, 16),
                toBuffer(params.mixhash, {endian: 'little', size: 32}, 16)
            );
