...
/** @typedef {{jsonInput: Record<string, unknown>, proof?: Buffer, cb: (result: unknown, errorKind?: string) => void, time: number, miner_address: string, mode?: "pearl", verifyBytes?: number}} VerifyTask */
...
const PEARL_PENDING_VERIFY_MAX_BYTES = 32 * 1024 * 1024;
const PEARL_PENDING_VERIFY_MAX_TASKS = 256;
let pearlPendingVerifyBytes = 0;
let pearlPendingVerifyTasks = 0;

/** @param {VerifyTask} task */
function releasePearlVerifyBudget(task) {
    if (task.mode !== "pearl" || !task.verifyBytes) return;
    pearlPendingVerifyBytes = Math.max(0, pearlPendingVerifyBytes - task.verifyBytes);
    pearlPendingVerifyTasks = Math.max(0, pearlPendingVerifyTasks - 1);
    task.verifyBytes = 0;
}
...
const shareVerifyQueues = (global.config.verify_shares_host || []).map(function registerVerifyQueue(verify_shares_host, index) {
    ...
    const queue = createTaskQueue(16, /** @param {VerifyTask} task */ function verifyShareRemote(task, queueCB) {
        ...
        if (Date.now() - task.time > 60 * 1000) {
            releasePearlVerifyBudget(task);
            cb(null, task.mode === "pearl" ? "verify-host-error" : undefined);
            return queueCB();
        }

        const socket = new net.Socket();
        let is_cb = false;
        /** @param {unknown} result @param {string} [errorKind] */
        ...
        socket.connect(2222, verify_shares_host, function onConnect() {
            if (task.mode === "pearl" && Buffer.isBuffer(task.proof)) {
                socket.cork();
                socket.write(`${JSON.stringify({ ...task.jsonInput, proof_bytes: task.proof.length })  }\n`);
                socket.write(task.proof);
                socket.uncork();
                return;
            }
        ...
        socket.on("data", function onData(buff1) {
            buff += buff1;
            if (buff.length > 16 * 1024 * 1024) {
                socket.destroy();
                return_cb(null, "verify-host-error");
            }
        });
    ...
    setInterval(function checkQueue(queue_obj, queueIndex) {
        ...
        queue_obj.remove(function removeExpired(task) {
            ...
            releasePearlVerifyBudget(d);
            d.cb(null, d.mode === "pearl" ? "verify-host-error" : undefined);
            return true;
        });
...
/** @returns {typeof shareVerifyQueues[number] | null} */
function selectShareVerifyQueue() {
    const timeNow = Date.now();
    /** @type {typeof shareVerifyQueues[number] | null} */
    let bestQueue = null;
    let minQueueSize = Infinity;
    for (const candidate of shareVerifyQueues) {
        if (timeNow - candidate.health.lastErrorAt < 60 * 1000 && candidate.health.errorCount > 0 && candidate.host !== "127.0.0.1") continue;
...
function Coin(data) {
    ...
    this.BlockTemplate = function BlockTemplate(template) {
        ...
        if (!templateBlob && templateConfig.hashOnly) {
            ...
            if (profile && profile.algo === "pearlhash") {
                const baseHeader = pearl.decodePearlHeader(template.incomplete_header_bytes);
                const recipe = pearl.decodeWorkerRecipe(template);
                if (!baseHeader || !recipe || template.hash !== template.incomplete_header_bytes) {
                    throw new Error("Invalid Pearl worker template");
                }
                const header = blockTemplate.derivePearlWorkerHeader(
                    baseHeader, recipe.coinbase, recipe.offset, recipe.branch, global.coinFuncs.uniqueWorkerId
                );
                hash = header.toString("base64");
                this.header = hash;
                this.cert_version = template.cert_version;
                this.target = template.target;
                this.pearl_target = template.pearl_target;
                this.target_hex = template.target_hex;
                this.target_compact = template.target_compact;
                this.incomplete_header_bytes = hash;
                this.gateway_target = template.gateway_target;
            }
    ...
    /** @param {Buffer} solutionData */
    this.pearlSolutionIdFromData = function pearlSolutionIdFromData(solutionData) {
        if (!Buffer.isBuffer(solutionData) || typeof blockTemplate.pearlSolutionId !== "function") return "";
        try {
            const solutionId = blockTemplate.pearlSolutionId(solutionData);
            return Buffer.isBuffer(solutionId) && solutionId.length === 32 ? solutionId.toString("hex") : "";
        } catch (_error) {
            return "";
        }
    };

    /** @param {string} header @param {Buffer} proof */
    this.pearlSolutionId = function pearlSolutionId(header, proof) {
        if (typeof header !== "string" || !/^[0-9a-fA-F]{152}$/.test(header) || !Buffer.isBuffer(proof)) {
            return { valid: false };
        }
        try {
            const result = powHash.pearl_v3(Buffer.from(header, "hex"), proof, false);
            if (!isRecord(result) || result["valid"] !== true || result["full"] !== false ||
                !Buffer.isBuffer(result["solution_data"]) || !isRecord(result["config"])) return { valid: false };
            const solutionId = self.pearlSolutionIdFromData(result["solution_data"]);
            if (!solutionId) return { valid: false };
            return {
                valid: true,
    ...
    /** @param {string} header @param {Buffer} proof @param {string} target @param {string} miner_address @param {(result: unknown, errorKind?: string) => void} cb */
    this.verifyPearlAsync = function verifyPearlAsync(header, proof, target, miner_address, cb) {
        if (!Buffer.isBuffer(proof) || proof.length === 0 || proof.length > pearl.PEARL_MAX_PROOF_BYTES) {
            return cb(null, "verify-host-error");
        }
        if (!hasVerifyShareHosts()) return cb(null, "verify-host-error");
        const pending = miner_address_verify[miner_address] ?? 0;
        if (pending > 100) return cb(null, "verify-host-error");
        const bestQueue = selectShareVerifyQueue();
        if (!bestQueue) return cb(null, "verify-host-error");
        const verifyBytes = proof.length + Buffer.byteLength(header, "utf8") +
            Buffer.byteLength(target, "utf8") + 256;
        if (pearlPendingVerifyTasks >= PEARL_PENDING_VERIFY_MAX_TASKS ||
            verifyBytes > PEARL_PENDING_VERIFY_MAX_BYTES - pearlPendingVerifyBytes) {
            return cb(null, "verify-host-overload");
        }
        miner_address_verify[miner_address] = pending + 1;
        pearlPendingVerifyBytes += verifyBytes;
        ++pearlPendingVerifyTasks;
        const time_now = Date.now();
        return bestQueue.queue.unshift({
            jsonInput: { algo: "pearlhash", header, target },
            proof,
            mode: "pearl",
            verifyBytes,
            cb(result, errorKind) { return cb(result, errorKind); },
            time: time_now,
            miner_address
        });
    };
    ...
    this.slowHashAsync = function slowHashAsync(convertedBlob, template, miner_address, cb, verifyContext) {
        ...
        return bestQueue.queue.unshift({
            jsonInput,
            cb(result, errorKind) {
                return cb(verifierResultToHex(result), errorKind);
            },
            time: time_now,
            miner_address
        });
