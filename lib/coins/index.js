...
const miner_address_verify = {};
const shareVerifyQueue = [];
const shareVerifyQueueErrorTime = [];
const shareVerifyQueueErrorCount = [];
...
function Coin(data) {
    ...
    this.hasTemplateBlob = function hasTemplateBlob(template, port) {
        ...
        return Boolean(profile) && ((profile.template && profile.template.hashOnly) || (template && (template.blocktemplate_blob || template.blob || template.blockhashing_blob)));
    };
    ...
    this.slowHashAsync = function slowHashAsync(convertedBlob, template, miner_address, cb, verifyContext) {
        ...
        if (!hasVerifyShareHosts()) return cb(hashLocal());
        ...
        if (best_index === null) return cb(null);
