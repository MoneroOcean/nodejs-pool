...
function Coin(data) {
    ...
    /** @param {string} header @param {string} proof @param {string} target @param {string} miner_address @param {(result: unknown, errorKind?: string) => void} cb */
    this.verifyPearlAsync = function verifyPearlAsync(header, proof, target, miner_address, cb) {
        ...
        const verifyBytes = Buffer.byteLength(proof, "utf8") + Buffer.byteLength(header, "utf8") +
        ...
        return bestQueue.queue.unshift({
            jsonInput: { algo: "pearlhash", header, proof, target },
            mode: "pearl",
            verifyBytes,
