...
const PEARL_MAX_COINBASE_BYTES = 16 * 1024;
const PEARL_MAX_MERKLE_BRANCH_BYTES = 32 * 32;
...
// Certificate generation can exceed a minute; wait for acceptance before resolving a block.
const PEARL_GATEWAY_SUBMIT_TIMEOUT_MS = 5 * 60 * 1000;
...
/** @param {unknown} value @param {number} maxBytes @param {boolean} [allowEmpty] @returns {Buffer|null} */
function decodeBoundedBase64(value, maxBytes, allowEmpty = false) {
    if (allowEmpty && value === "") return Buffer.alloc(0);
    if (typeof value !== "string" || value.length > Math.ceil(maxBytes / 3) * 4 || !isCanonicalBase64(value)) return null;
    const decoded = Buffer.from(value, "base64");
    return decoded.length <= maxBytes ? decoded : null;
}

/** @param {unknown} value @returns {{coinbase: Buffer, offset: number, branch: Buffer}|null} */
function decodeWorkerRecipe(value) {
    if (!isRecord(value)) return null;
    const coinbase = decodeBoundedBase64(value["worker_coinbase_bytes"], PEARL_MAX_COINBASE_BYTES);
    const branch = decodeBoundedBase64(value["worker_merkle_branch"], PEARL_MAX_MERKLE_BRANCH_BYTES, true);
    const offset = value["worker_coinbase_offset"];
    if (!coinbase || !branch || typeof offset !== "number" || !Number.isSafeInteger(offset) || offset < 0 || offset >= coinbase.length ||
        coinbase[offset] !== 0 || branch.length % 32 !== 0) return null;
    return { coinbase, offset, branch };
}
