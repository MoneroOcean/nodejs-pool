...
function normalizeBtcRewardBlock(value) {
    ...
    // Keep header metadata used by template polling and block verification.
    const normalized = { ...block, tx: [{ vout }, ...rawTransactions.slice(1)], difficulty };
    return normalized;
}
...
function createXtmMainRpc(overrides) {
    ...
    config.getLastBlockHeader = function getLastBlockHeader(ctx) {
        return ctx.runtime.support.rpcPortDaemon(ctx.port, "GetTipInfo", null, function onTip(body) {
            ...
            // The gRPC JSON proxy represents protobuf bytes as a byte array.
            const rawHash = metadata?.["best_block_hash"];
            const hash = Array.isArray(rawHash) && rawHash.length === 32 &&
                rawHash.every(byte => Number.isInteger(byte) && byte >= 0 && byte <= 255)
                ? Buffer.from(rawHash).toString("hex") : rawHash;
...
function submitXtmRxBlock(ctx) {
    ...
    // The mining blob is always padded to a 32-byte pow_data field, but Tari v6
    // requires the submitted RandomXT pow_data to use its minimal representation.
    // Removing only trailing zero padding preserves the exact mining blob Tari
    // reconstructs while satisfying the post-fork canonical encoding rule.
    let powDataEnd = blockData.length;
    while (powDataEnd > XTM_T_POW_DATA_OFFSET && blockData[powDataEnd - 1] === 0) --powDataEnd;
    const powData = blockData.subarray(XTM_T_POW_DATA_OFFSET, powDataEnd);

    xtmBlock.header.nonce = readUInt64BufferBE(blockData, XTM_T_NONCE_OFFSET);
    xtmBlock.header.pow.pow_data = [...powData];
