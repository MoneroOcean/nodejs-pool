...
const ERG_STRATUM_DIFFICULTY = Symbol("ergStratumDifficulty");
...
function isCoinProfile(value) { return Boolean(value && value[COIN_PROFILE_SYMBOL] === true); }
...
function parseBtcReward(block, config, poolAddress, isOurBlock) {
    ...
    block.reward = parseInt(reward * (config.rewardMultiplier || 1), 10);
...
function loadEthBlockReward(config, port, block, runtime, callback) {
    ...
    if (!Number.isFinite(baseReward) || baseReward <= 0) return callback(true, { error: { message: "invalid ETH-family base reward" } });
    const receipts = [];
...
function getWalletReward(config, body, rewardCheck, port, runtime, callback, noErrorReport) {
    ...
    runtime.support.rpcPortWalletShort(port + 1, "get_transfer_by_txid", { txid: minerTxHash }, function onTransfer(body2) {
        if (!body2 || body2.error || !body2.result || !body2.result.transfer) {
            const headerWithError = Object.assign({}, body.result.block_header, {
                error: body2 && body2.error ? body2.error : body2 || { message: "wallet reward lookup failed" },
...
function createCryptonoteRpc(blockByHashMode, overrides) {
    ...
    if (blockByHashMode === "header") {
        ...
        config.getAnyBlockHeaderByHash = function getAnyBlockHeaderByHash(ctx) {
            ...
            ctx.runtime.support.rpcPortDaemon(ctx.port, method, { hash: ctx.blockHash }, function onBlock(body) {
                ...
                let minerTx;
                try {
                    minerTx = JSON.parse(body.result.json).miner_tx;
                } catch (_parseError) {
                    return ctx.callback(true, body);
                }
                if (!minerTx || !Array.isArray(minerTx.vout) || minerTx.vout.length < 1) return ctx.callback(true, body);
                let rewardCheck = 0;
                ...
                return getWalletReward(config, body, rewardCheck, ctx.port, ctx.runtime, ctx.callback, ctx.noErrorReport);
            }, ctx.noErrorReport);
        };
    }
    ...
    config.getBlockTemplate = function getBlockTemplate(ctx) {
        ctx.runtime.support.rpcPortDaemon(ctx.port, "getblocktemplate", {
            ...
            return ctx.callback(body && body.result ? body.result : null, body && body.error ? body.error : null);
        }, ctx.noErrorReport);
    };
...
function createBtcRpc(overrides) {
    ...
    config.getAnyBlockHeaderByHash = function getAnyBlockHeaderByHash(ctx) {
        ctx.runtime.support.rpcPortDaemon2(ctx.port, "", { method: "getblock", params: [ctx.blockHash, 2] }, function onBlock(body) {
            if (!body || !body.result || !(body.result.tx instanceof Array) || body.result.tx.length < 1 || !body.result.tx[0] || !Array.isArray(body.result.tx[0].vout)) return ctx.callback(true, body);
    ...
    config.getBlockTemplate = function getBlockTemplate(ctx) {
        ctx.runtime.support.rpcPortDaemon2(ctx.port, "", { method: "getblocktemplate", params: [{ capabilities: ["coinbasetxn", "workid", "coinbase/append"], rules: ["segwit"] }] }, function onTemplate(body) {
            if (!(body && body.result)) return ctx.callback(null, body && body.error ? body.error : null);
            let built;
            try {
                built = config.createBlockTemplate(ctx.runtime.blockTemplate, body.result, ctx.runtime.getPoolAddress(ctx.profile));
            } catch (err) {
                ...
                console.error(`getblocktemplate build failed (port ${ctx.port}): ${err && err.message}`);
                return ctx.callback(null, err && err.message ? err.message : "template build failed");
            }
...
function createEthRpc(overrides) {
    ...
    config.getBlockHeaderById = function getBlockHeaderById(ctx) {
        ...
        ctx.runtime.support.rpcPortDaemon2(ctx.port, "", { jsonrpc: "2.0", id: 1, method: "eth_getBlockByNumber", params: [blockId, true] }, function onBlock(body) {
            ...
            return loadEthBlockReward(config, ctx.port, body.result, ctx.runtime, finish);
        }, ctx.noErrorReport);
    };
    ...
    config.getAnyBlockHeaderByHash = function getAnyBlockHeaderByHash(ctx) {
        ...
        ctx.runtime.support.rpcPortDaemon2(ctx.port, "", { jsonrpc: "2.0", id: 1, method: "eth_getBlockByHash", params: [`0x${  ctx.blockHash}`, true] }, function onBlock(body) {
            ...
            ctx.runtime.coinFuncs.getPortBlockHeaderByID(ctx.port, body.result.height, function onCanonical(err, canonical) {
                ...
                if (body.result.hash === canonical.hash) return loadEthBlockReward(config, ctx.port, body.result, ctx.runtime, finish);
    ...
    config.getBlockTemplate = function getBlockTemplate(ctx) {
        ctx.runtime.support.rpcPortDaemon2(ctx.port, "", { jsonrpc: "2.0", id: 1, method: "eth_getWork", params: [] }, function onWork(body) {
            return ctx.callback(body && body.result ? ctx.runtime.blockTemplate.EthBlockTemplate(body.result) : null, body && body.error ? body.error : null);
        }, ctx.noErrorReport);
    };
...
function createErgRpc(overrides) {
    ...
    config.getAnyBlockHeaderByHash = function getAnyBlockHeaderByHash(ctx) {
        ctx.runtime.support.rpcPortDaemon2(ctx.port, `blocks/${  ctx.blockHash}`, null, function onBlock(body) {
            if (!body || !body.header || !body.blockTransactions || !Array.isArray(body.blockTransactions.transactions)) return ctx.callback(true, body);
            ...
            ctx.runtime.support.rpcPortDaemon2(ctx.port, `blocks/at/${  body.header.height}`, null, function onCanonical(ids) {
                if (Array.isArray(ids) && ids.length === 1 && ids[0] !== body.header.id) {
                    body.header.reward = null;
                    body.header.confirmations = -1;
                ...
                if (!Number.isSafeInteger(body.header.reward) || body.header.reward <= 0) {
                    body.header.error = { message: "Ergo daemon reward data failed EIP-27 validation" };
                    body.header.errorSource = "erg_reward_validation";
                    return ctx.callback(true, body.header);
                }
                return ctx.callback(null, body.header);
            }, ctx.noErrorReport);
        }, ctx.noErrorReport);
    ...
    config.getBlockTemplate = function getBlockTemplate(ctx) {
        ctx.runtime.support.rpcPortDaemon2(ctx.port, "mining/candidate", null, function onCandidate(body) {
            return ctx.callback(body && body.pk ? ctx.runtime.blockTemplate.ErgBlockTemplate(body) : null, body && body.error ? body.error : null);
        }, ctx.noErrorReport);
    };
...
function createXtmMainRpc(overrides) {
    ...
    config.getAnyBlockHeaderByHash = function getAnyBlockHeaderByHash(ctx) {
        ctx.runtime.support.rpcPortDaemon(ctx.port, "GetHeaderByHash", { hash: Buffer.from(ctx.blockHash, "hex").toJSON().data }, function onHeader(body) {
            if (!body || !body.result || !body.result.header) return ctx.callback(true, body);
    ...
    config.getLastBlockHeader = function getLastBlockHeader(ctx) {
        ctx.runtime.support.rpcPortDaemon(ctx.port, "GetTipInfo", null, function onTip(body) {
            if (!body || !body.result || !body.result.metadata) return ctx.callback(true, body);
    ...
    config.getBlockTemplate = function getBlockTemplate(ctx) {
        ctx.runtime.support.rpcPortDaemon(ctx.port, "GetNewBlockTemplateWithCoinbases", {
            ...
            return ctx.callback(arr2hex(body.result.block), null);
        }, ctx.noErrorReport);
    };
...
function createXtmTRpc(overrides) {
    ...
    config.getBlockTemplate = function getBlockTemplate(ctx) {
        ctx.runtime.support.rpcPortDaemon(ctx.port, "GetNewBlockTemplateWithCoinbases", {
            ...
            if (!(body && body.result)) {
                return ctx.runtime.coinFuncs.getPortBlockTemplate(ctx.port, function retry(bt2, err2) { return ctx.callback(bt2, err2 || (body && body.error ? body.error : null)); }, ctx.noErrorReport);
            }
...
function buildVerifyInput(algo, convertedBlob, extras) {
    return Object.assign({ algo, blob: convertedBlob.toString("hex") }, extras || {});
}
...
function buildErgJobPayload(ctx) {
    // Ergo Stratum defines difficulty as a multiplier for the network target in
    // field b. Keep the final per-miner target and identity difficulty for
    // clients known not to apply mining.set_difficulty to b.
    const identityDifficulty = typeof ctx.miner.agent === "string" &&
        /(NBMiner|NiceHash)/i.test(ctx.miner.agent);
    const networkDifficulty = ctx.toBigInt(ctx.blockTemplate.difficulty);
    const shareDifficulty = ctx.toBigInt(ctx.coinDiff);
    const stratumDifficulty = identityDifficulty
        ? 1
        : Number(networkDifficulty) / Number(shareDifficulty);
    const targetDifficulty = identityDifficulty ? shareDifficulty : networkDifficulty;
    const payload = [
        ctx.newJob.id,
        ...
        "",
        2,
        (ctx.toBigInt(ctx.coinFuncs.baseDiff()) / ctx.toBigInt(targetDifficulty)).toString(),
        "",
        true
    ];
    Object.defineProperty(payload, ERG_STRATUM_DIFFICULTY, { value: stratumDifficulty });
    return payload;
}
...
function pushEthJob(ctx) {
    ...
    if (!ctx.miner.last_diff || ctx.miner.last_diff !== diff) {
        ctx.miner.pushMessage({ method: "mining.set_difficulty", params: [diff], id: null });
...
function pushErgJob(ctx) {
    ...
    const notifyMessage = { method: "mining.notify", params: ctx.job, algo: ctx.params.algo_name };
    if (isNiceHash) {
        notifyMessage.id = null;
    }
...
function validateStandardSubmit(ctx) {
    ...
    if (ctx.coinFuncs.nonceSize(ctx.job.blob_type_num) === 8) {
        if (this.sharedTemplateNonces === true) {
            ctx.params.nonce = ctx.normalizeExtraNonceSubmitNonce(ctx.params.nonce, ctx.job.extraNonce, {
...
function attachLoginExtraNonce(ctx) {
    let newId;
    if (Number.isInteger(ctx.socket.eth_extranonce_id)) {
        newId = ctx.socket.eth_extranonce_id;
    } else if (Number.isInteger(ctx.socket.eth_extranonce_preview_id)) {
...
function verifyRavenShare(ctx) {
    ...
    let quickResult;
    try {
        quickResult = ctx.coinFuncs.kawpowQuickHash(convertedBlob, ctx.params.nonce, ctx.params.mixhash);
    } catch (_error) {
        return rejectSpecialShare(ctx);
    }
    ...
    if (
        ...
        ctx.miner.trust &&
        ctx.isSafeToTrust(ctx.job.rewarded_difficulty2, ctx.miner.payout, ctx.miner.trust.trust) &&
...
function acceptStatusOkObject(ctx) {
    return Boolean(ctx.rpcResult && typeof ctx.rpcResult.result === "object" && ctx.rpcResult.result && ctx.rpcResult.result.status === "OK");
}
...
function resolveDeroSubmittedBlockHash(ctx, callback) { return callback(ctx.rpcResult.result.blid); }

function resolveResultHashSubmittedBlockHash(ctx, callback) { return callback(ctx.resultBuff.toString("hex")); }
...
function resolveErgSubmittedBlockHash(ctx, callback) {
    return setTimeout(function checkSubmittedErgBlock() {
        ctx.coinFuncs.getPortBlockHeaderByID(ctx.blockTemplate.port, ctx.blockTemplate.height, function onHeader(err, body) {
            ...
            const pk = err === null && body && body.powSolutions ? body.powSolutions.pk : undefined;
            callback(pk === ctx.blockTemplate.hash2 ? body.id : "0".repeat(64));
        });
    }, 10 * 1000);
}
...
function resolveEthSubmittedBlockHash(ctx, callback) {
    ...
    function attemptResolve() {
        ctx.coinFuncs.ethBlockFind(ctx.blockTemplate.port, ctx.blockData[0], function onBlockHash(blockHash) {
...
function submitCryptonoteBlock(ctx) {
    ctx.support.rpcPortDaemon(ctx.blockTemplate.port, "submitblock", [ctx.blockData.toString("hex")], ctx.replyFn, true);
}

function submitHttpBlockBody(ctx) {
    ctx.support.rpcPortDaemon2(ctx.blockTemplate.port, "block", ctx.blockData.toString("hex"), ctx.replyFn, true);
}

function submitBtcBlock(ctx) {
    ctx.support.rpcPortDaemon2(ctx.blockTemplate.port, "", { method: "submitblock", params: [ctx.blockData.toString("hex")] }, ctx.replyFn, true);
}
...
function submitDeroBlock(ctx) {
    ctx.support.rpcPortDaemon(ctx.blockTemplate.port, "submitblock", [ctx.blockTemplate.blocktemplate_blob, ctx.blockData.toString("hex")], ctx.replyFn, true);
}
