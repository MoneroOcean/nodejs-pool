...
function buildErgJobPayload(ctx) {
    ...
    // field b. Keep the historical pre-multiplied target for NBMiner, which is
    // documented by the reference server as not supporting set_difficulty.
    const identityDifficulty = typeof ctx.miner.agent === "string" && /NBMiner/i.test(ctx.miner.agent);
