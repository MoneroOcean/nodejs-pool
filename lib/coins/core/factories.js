...
function parseErgArrayMiningSubmit(ctx) {
    // SRBMiner autolykos submits both nonce suffix and full extranonce-prefixed
    // nonce: [wallet, job, suffix, "undefined", full]. Use the full nonce so
    // validation can keep enforcing the pool extranonce boundary.
    const fullNonce = ctx.params.raw_params[4];
    ctx.params.nonce = typeof fullNonce === "string" && fullNonce !== "undefined"
        ? fullNonce
        : ctx.params.raw_params[2];
    return true;
}
...
const pool = {
    ...
    erg(overrides) {
        return createPoolConfig({
            ...
            pushJob: pushErgJob,
            parseMiningSubmitParams: parseErgArrayMiningSubmit,
            submitSuccess: "boolean",
            sendLoginResult: sendExtraNonceLoginResult,
            ...
            sharedTemplateNonces: true,
            // Keep the same submit boundary as ETH/ETC. SRBMiner autolykos
            // provides the full extranonce-prefixed nonce as raw_params[4].
            requireFullNonceExtraNoncePrefix: true,
        }, overrides);
    },
    grin(overrides) {
