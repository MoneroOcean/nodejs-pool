...
function calcErgReward(height, blockTx) {
    ...
    if (blockTx.length && blockTx[0].outputs.length == 2 && blockTx[0].outputs[1].creationHeight == height) {
        ...
        // Subtract the Ergo treasury cut (nanoERG): 12 ERG once the reward is >= 15 ERG, else 3 ERG.
...
function toBuffer(value, options, base) {
    ...
    // options.size fixes the output to exactly that many bytes: left-pad with zeros or drop the low bytes.
