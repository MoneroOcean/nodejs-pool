...
function Coin(data){
    ...
    this.getPortLastBlockHeaderMM = function(port, callback, no_error_report) { // with merge mining header
		...
		// we use 18146 port instead 18144 port here since 18146 can be served locally
                global.coinFuncs.getPortLastBlockHeader(global.config.daemon.port, function(is_err, body2) {
                    if (is_err) return callback(is_err, body);
                    body.mm = body2;
                    return callback(null, body);
                }, no_error_report);
           ...
           global.support.rpcPortDaemon(port, 'GetNewBlockTemplateWithCoinbases', { algo: {pow_algo: 0 }, coinbases: [{address: global.config.pool["address_18146"], value: 1, stealth_payment: true, revealed_value_proof: true, coinbase_extra: [] }] }, function(body){
            ...
            global.support.rpcPortDaemon(port, 'GetNewBlockTemplateWithCoinbases', { algo: {pow_algo: 3 }, coinbases: [{address: global.config.pool["address_18146"], value: 1, stealth_payment: true, revealed_value_proof: true, coinbase_extra: [] }] }, function(body) {
                if (body && body.result) {
                    const result = body.result;
                    // The format of blobtemplate_blob is:
                    // 4 bytes nonce (generated on miner size) | 4 bytes (miner/instance id) |  32 bytes |
                    // | nonce (big endian) | mining_hash
                    return callback({ blocktemplate_blob: "00".repeat(4) + arr2hex(result.merge_mining_hash),
                                      reserved_offset: 0,
                                      bt_nonce_size: 4,
                                      difficulty: parseInt(result.miner_data.target_difficulty),
                                      reward: parseInt(result.miner_data.reward),
	...
	this.xtm_block          = template.xtm_block;
        ...
        if (template.bt_nonce_size === undefined || template.bt_nonce_size >= 16) {
          ...
          this.nextBlobHex = function () {
              // write 4 bytes of miner extraNonce merged with pool workerId
              this.extraNonce = ((global.coinFuncs.uniqueWorkerId << (32-global.coinFuncs.uniqueWorkerIdBits)) + ((++this.extraNonce2) % (1<<(32-global.coinFuncs.uniqueWorkerIdBits))) ) >>> 0;
              this.buffer.writeUInt32BE(this.extraNonce, this.reserved_offset);
              // Convert the buffer into something hashable.
              const blob = global.coinFuncs.convertBlob(this.buffer, this.port);
              return blob ? blob.toString('hex') : null;
