...
function bigintStringToLEBuffer(value, size, base) {
    let hex = (typeof value === 'bigint' ? value : BigInt(base === 16 ? `0x${value}` : value)).toString(16);
    if (hex.length % 2) hex = `0${hex}`;
    return Buffer.from(`${'00'.repeat(size)}${hex}`.slice(-size * 2), 'hex').reverse();
}
    ...
    this.constructNewBlob = function(blockTemplateBuffer, params, port) {
       ...
       if (global.coinFuncs.blobTypeXTM_C(blob_type_num)) {
            ...
            return cnUtil.construct_block_blob(blockTemplateBuffer,
                bigintStringToLEBuffer(params.nonce, 4, 10),
            ...
            return cnUtil.constructNewRavenBlob(blockTemplateBuffer,
                bigintStringToLEBuffer(params.nonce, 8, 16),
                bigintStringToLEBuffer(params.mixhash, 32, 16)
            );
