...
const BASE_DIFF_BIGINT = BigInt('0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF');
const BASE_RAVEN_DIFF_BIGINT = BigInt('0x00000000ff000000000000000000000000000000000000000000000000000000');
...
function bigIntToBuffer(value, options) {
    ...
    if (typeof options.size === 'number') {
        ...
        else if (hex.length > options.size * 2) hex = hex.slice(-options.size * 2);
    }
...
function divideBaseDiff(diff) {
    return BASE_DIFF_BIGINT / toBigInt(diff);
}
...
function getTargetHex(diff, size) {
    return pad_hex(bigIntToBuffer(divideBaseDiff(diff), {endian: 'little', size: size}).toString('hex'), size);
};

function getRavenTargetHex(diff) {
    return pad_hex((BASE_RAVEN_DIFF_BIGINT / toBigInt(diff)).toString(16), 32);
};
        ...
        this.getCoinJob = function (coin, params) {
            ...
            if (!this.proxy || isExtraNonceBT) {
	            ...
	            newJob.id,
                    ...
                    divideBaseDiff(coin_diff).toString(),
                    "",
                    true
                ]; else this.cachedJob = {
                    blob:       blob_hex,
...
function hashRavenBuffDiff(hash) { // float as result
    return Number(BASE_RAVEN_DIFF_BIGINT) / Number(bigIntFromBuffer(hash));
}
...
function ge(l, r) {
    if (typeof l === 'bigint' || typeof r === 'bigint') return toBigInt(l) >= toBigInt(r);
    return l >= r;
}
