...
function divideBaseDiff(diff) {
    return toBigInt(baseDiff) / toBigInt(diff);
}
...
function ravenTargetHex(diff) {
    return pad_hex((Number(baseRavenDiff) / Number(diff)).toString(16), 32);
}
...
function templateUpdate2(coin, port, isHashChange, isMMHashChange, coinHashFactor, isHashFactorChange, body_header, times_failed) {
    if (port == 8545 || port == 8645) {
        ...
        if (!global.coinFuncs.hasTemplateBlob(body_bt, port)) {
            times_failed += 1;
            console.error("Block template request attempt " + times_failed + " returned unusable template for " + port + " port (missing mining blob): " + JSON.stringify(body_bt));
            if (times_failed <= 2) {
                setTimeout(templateUpdate2, 500, coin, port, isHashChange, isMMHashChange, coinHashFactor, isHashFactorChange, body_header, times_failed);
            } else coinHashFactorUpdate(coin, 0);
            return;
...
function getTargetHex(diff, size) {
    return pad_hex(bigIntToBuffer(sizedTargetValue(diff, size), {endian: 'little', size: size}).toString('hex'), size);
};

function getRavenTargetHex(diff) {
    return ravenTargetHex(diff);
};
    ...
    for (let worker_name in wallet) {
        ...
        if (time_now - worker.time > global.config.pool.shareAccTime*1000) {
            ...
            if (acc != 0) {
                ...
                storeShareDiv(miner, acc, worker.acc2, worker.share_num, worker_name, bt_port, height, worker.difficulty, false, worker.trustedShare);
            }
...
function ge(l, r) {
    if (typeof l === 'bigint' || typeof r === 'bigint' || typeof l === 'object' || typeof r === 'object') return toBigInt(l) >= toBigInt(r);
    return l >= r;
}
...
if (global.__poolTestMode === true) {
    module.exports = {
        ...
        setTestCoinHashFactor: setTestCoinHashFactor,
        templateUpdate2: templateUpdate2,
