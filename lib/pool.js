...
function createPoolRuntime() {
    ...
    function startProduction() {
        if (fs.existsSync("pool_health_guard_unhealthy")) {
            console.error(`IMPORTANT: ${  stateTools.formatPoolEvent("Startup blocked", {
                reason: "pool-health-guard-marker",
                file: "pool_health_guard_unhealthy"
            })}`);
        ...
        if (global.config.general.allowStuckPoolKill && fs.existsSync("block_template_is_stuck")) {
            console.error(`IMPORTANT: ${  stateTools.formatPoolEvent("Startup blocked", {
                reason: "stuck-template-marker",
                file: "block_template_is_stuck"
            })}`);
            setTimeout(function exitAfterDelay() { process.exit(); }, 5 * 1000);
            return false;
        }
