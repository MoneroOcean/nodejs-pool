...
function Miner(id, login, pass, rigid, ipAddress, startingDiff, pushMessage, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    if (protoVersion === 1) {
        this.getCoinJob = function (coin, params) {
		    ...
		    noncebytes: 4,
                    nonceoffset: global.coinFuncs.blobTypeXTM_C(blob_type_num) ? 0 : null,
                    height:     bt.height,
                    job_id:     newJob.id,
                    difficulty: coin_diff,
                    id:         this.id
...
function processShare(miner, job, blockTemplate, params, processShareCB) {
    ...
    let verifyShare = function(verifyShareCB) {
        if (global.coinFuncs.blobTypeXTM_C(blob_type_num)) {
            ...
            const header = Buffer.concat([bignum(params.nonce, 10).toBuffer({endian: 'big', size: 4}), global.coinFuncs.convertBlob(blockData, port)]);
