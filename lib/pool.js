...
function toBigInt(value, base) {
    ...
    if (Buffer.isBuffer(value)) return bigintFromBuffer(value);
    if (typeof value === 'string') return BigInt(base === 16 ? `0x${value}` : value);
    return BigInt(value || 0);
}

function bigintToBuffer(value, endian, size) {
    let hex = toBigInt(value).toString(16);
    if (hex.length % 2) hex = `0${hex}`;
    if (hex.length < size * 2) hex = `${'00'.repeat(size)}${hex}`.slice(-size * 2);
    else if (hex.length > size * 2) hex = hex.slice(-size * 2);
    let buffer = Buffer.from(hex, 'hex');
    return endian === 'little' ? Buffer.from(buffer).reverse() : buffer;
}

function bigintFromBuffer(buffer, endian) {
    buffer = endian === 'little' ? Buffer.from(buffer).reverse() : Buffer.from(buffer);
...
let threadContextInitialized = false;
let threadResetInterval = null;
let threadStatsInterval = null;
...
function initThreadContext(isMaster, workerId, options) {
    if (threadContextInitialized) return;
    threadContextInitialized = true;
    options = options || {};

    if (isMaster) {
        threadName = options.threadName || "(Master) ";
        if (options.enableStats !== false) {
            threadStatsInterval = setInterval(function () {
        ...
        threadName = options.threadName || "(Worker " + resolvedWorkerId + " - " + process.pid + ") ";
        ...
        if (options.enableShareWindowReset !== false) {
            threadResetInterval = setInterval(function () {
...
function retargetMiners() {
    ...
    processSend({type: 'minerPortCount', data: { worker_id: process.env['WORKER_ID'], ports: minerCount } });
}
...
function getTargetHex(diff, size) {
    return pad_hex(bigintToBuffer(baseDiff / toBigInt(diff), 'little', size).toString('hex'), size);
};
    ...
    this.checkBan = function (validShare) {
        ...
        if (validShare) {
          ...
          if (this.validShares === 0) {
            ...
            removeMiner(this);
            processSend({type: 'banIP', data: this.ipAddress, wallet: this.payout});
            return;
          }
        }
        ...
        this.getCoinJob = function (coin, params) {
            ...
            if (!this.proxy || isExtraNonceBT) {
	            ...
	            newJob.id,
                    ...
                    (baseDiff / toBigInt(coin_diff)).toString(),
                    "",
                    true
                ]; else this.cachedJob = {
                    blob:       blob_hex,
...
function hashBuffDiff(hash) { // bigint as result
    return baseDiff / bigintFromBuffer(hash, 'little');
}

function hashRavenBuffDiff(hash) { // float as result
    return baseRavenDiff / Number(bigintFromBuffer(hash));
}

function hashEthBuffDiff(hash) { // bigint as result
    return baseDiff / bigintFromBuffer(hash);
}
...
function getHashVariants(resultHash, resultBuff, getDiff) {
    const reversedResultBuff = reverseBuffer(resultBuff);
    return [
        {
            hash: resultHash,
            buffer: resultBuff,
            diff: getDiff(resultBuff)
        },
        {
            hash: reversedResultBuff.toString('hex'),
            buffer: reversedResultBuff,
            diff: getDiff(reversedResultBuff)
        }
    ];
}

function selectHashVariantByBuffer(hashVariants, verifiedBuffer) {
    if (verifiedBuffer.equals(hashVariants[0].buffer)) return hashVariants[0];
    if (verifiedBuffer.equals(hashVariants[1].buffer)) return hashVariants[1];
    return null;
}

function selectHashVariantByString(hashVariants, verifiedHash) {
    if (verifiedHash === hashVariants[0].hash) return hashVariants[0];
    if (verifiedHash === hashVariants[1].hash) return hashVariants[1];
    return null;
}

// will work for numbers and bigint
function ge(l, r) {
    return l >= r;
}
...
function processShare(miner, job, blockTemplate, params, processShareCB) {
    ...
    let verifyShare = function(verifyShareCB) {
        if (global.coinFuncs.blobTypeXTM_C(blob_type_num)) {
            const header = Buffer.concat([bigintToBuffer(toBigInt(params.nonce, 16), 'big', 8), blockTemplate.buffer]);
	    ...
	    job.c29_packed_edges = Array.from(Buffer.from(c29_packed_edges_buff, "hex"));
            ...
            const header = Buffer.concat([global.coinFuncs.convertBlob(blockData, port), bigintToBuffer(toBigInt(params.nonce, 10), 'big', 4)]);
            if (global.coinFuncs.c29(header, params.pow, port)) {
                report_miner_share(miner, job);
                return processShareCB(invalid_share(miner));
            }
        ...
        const hashVariants = getHashVariants(resultHash, resultBuff, hashBuffDiff);
        let hashDiff = hashVariants[0].diff;
        ...
        if ( global.config.pool.trustedMiners &&
            ...
            if (isBlockDiffMatched) {
                if (miner.validShares || (miner.payout in minerWallets && minerWallets[miner.payout].hashes)) {
                    submit_block(miner, job, blockTemplate, blockData, resultBuff, hashDiff, true, true, null, function(block_submit_result) {
                        if (!block_submit_result) {
                            const buff = global.coinFuncs.slowHashBuff(convertedBlob, blockTemplate);
                            const hashVariant = selectHashVariantByBuffer(hashVariants, buff);
                            if (hashVariant === null) {
                                report_miner_share(miner, job);
                                return processShareCB(invalid_share(miner));
                            }
                            resultBuff = hashVariant.buffer;
                            hashDiff = hashVariant.diff;
                        }
                ...
                global.coinFuncs.slowHashAsync(convertedBlob, blockTemplate, miner.payout, function(hash) {
                    ...
                    const hashVariant = selectHashVariantByString(hashVariants, hash);
                    if (hashVariant === null) {
                        report_miner_share(miner, job);
                        return processShareCB(invalid_share(miner));
                    }
...
function resetRuntimeState() {
    if (threadResetInterval !== null) {
        clearInterval(threadResetInterval);
        threadResetInterval = null;
    }
    if (threadStatsInterval !== null) {
        clearInterval(threadStatsInterval);
    ...
    anchorBlockPrevHeight = undefined;
    freeEthExtranonces = [];
    ethJobId = 0;
    decId = 0;
}
...
function createPoolSocketHandler(portData) {
    ...
    return function socketConn(socket) {
        ...
        socket.on('data', function (d) {
            ...
            if (socket.miner_id) removeMiner(activeMiners.get(socket.miner_id));
...
function startPortServers(portList) {
    return Promise.all(portList.map(function (portData) {
        return new Promise(function (resolve, reject) {
            ...
            if ('ssl' in portData && portData.ssl === true) {
                ...
                server = net.createServer(socketConn);
            }
...
function startTestRuntime(options) {
    ...
    resetRuntimeState();
    initThreadContext(false, options.workerId || 1, {
    ...
    freeEthExtranonces = options.freeEthExtranonces || [...Array(1 << 16).keys()];

    newCoinHashFactor[""] = lastCoinHashFactor[""] = lastCoinHashFactorMM[""] = 1;
    if (global.config.daemon.enableAlgoSwitching) {
        COINS.forEach(function(coin) {
            newCoinHashFactor[coin] = lastCoinHashFactor[coin] = lastCoinHashFactorMM[coin] =
                options.coinHashFactors && coin in options.coinHashFactors ? options.coinHashFactors[coin] : 0;
        });
    }
    ...
    return startPortServers(options.ports || global.config.ports).then(function (servers) {
        return {
            servers: servers,
            ...
            getState: function () {
                return {
                    ...
                    activeBlockTemplates: activeBlockTemplates,
                    bannedTmpIPs: bannedTmpIPs,
                    bannedTmpWallets: bannedTmpWallets,
                    bannedBigTmpWallets: bannedBigTmpWallets,
                    bannedAddresses: bannedAddresses,
                    notifyAddresses: notifyAddresses,
                    minerWallets: minerWallets,
                    proxyMiners: proxyMiners,
                    ip_whitelist: ip_whitelist,
                    minerCount: minerCount,
                    walletTrust: walletTrust,
                    shareStats: {
                        totalShares: totalShares,
                        trustedShares: trustedShares,
                        normalShares: normalShares,
                        invalidShares: invalidShares,
                        outdatedShares: outdatedShares,
                        throttledShares: throttledShares
                    }
                };
            },
            stop: function () {
...
if (global.__poolTestMode === true) {
    module.exports = {
        startTestRuntime: startTestRuntime,
        resetRuntimeState: resetRuntimeState,
        setNewBlockTemplate: setNewBlockTemplate,
        setNewCoinHashFactor: setNewCoinHashFactor,
        registerPool: registerPool,
        retargetMiners: retargetMiners,
        checkAliveMiners: checkAliveMiners,
        handleMinerData: handleMinerData,
        messageHandler: messageHandler
    };
    return;
}

initThreadContext(cluster.isMaster, process.env['WORKER_ID']);
...
if (cluster.isMaster) {
    ...
    startPortServers(global.config.ports).catch(function (error) {
        console.error(threadName + "Unable to start worker pool servers: " + error);
        process.exit(1);
    });
});
