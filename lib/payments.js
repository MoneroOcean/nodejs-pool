...
/** @typedef {{mysql?: SqlPool, database?: DatabaseRuntime, support?: SupportRuntime, config?: PoolConfig, setTimeout?: (callback: () => void, delay: number) => PaymentTimer, clearTimeout?: (timer: PaymentTimer) => void, now?: () => number, advisoryLockName?: string}} PaymentsOptions */
...
function createPaymentsRuntime(options) {
    ...
    function canAdvancePaymentCycle(createdUnresolvedBatch) {
        return !createdUnresolvedBatch && !state.isFailStop && database && typeof database.setCache === "function";
    }
