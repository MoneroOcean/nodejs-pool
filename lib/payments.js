...
/** @typedef {{threadId?: number|string, once?: (event: string, listener: (...args: unknown[]) => void) => void, removeListener?: (event: string, listener: (...args: unknown[]) => void) => void, destroy?: () => void}} ConnectionExtras */
/** @typedef {SqlConnection & ConnectionExtras} PaymentConnection */
/** @typedef {ReturnType<typeof setTimeout>} PaymentTimer */
/** @typedef {{mysql?: SqlPool, database?: LocalDatabaseRuntime, support?: SupportRuntime, config?: PoolConfig, setTimeout?: (callback: () => void, delay: number) => PaymentTimer, clearTimeout?: (timer: PaymentTimer) => void, now?: () => number, advisoryLockName?: string}} PaymentsOptions */
/** @typedef {{activeBatchId: number|string|null, cyclePromise: Promise<{clean: boolean}>|null, failStopReason: string|null, isFailStop: boolean, lockConnection: PaymentConnection|null, lockConnectionEndHandler: ((...args: unknown[]) => void)|null, lockConnectionErrorHandler: ((...args: unknown[]) => void)|null, lockConnectionId: number|null, storeTimer: PaymentTimer|null, started: boolean, timer: PaymentTimer|null}} PaymentState */
/** @typedef {{activeBatchId: number|string|null, failStopReason: string|null, isFailStop: boolean, lockConnectionId: number|null, started: boolean}} PaymentStateSnapshot */
/** @typedef {{start: () => PaymentsRuntime, stop: () => Promise<void>, inspectState: () => PaymentStateSnapshot, planBatches: () => Promise<unknown[]>, recoverPendingBatches: (source: string) => Promise<boolean>, runCycle: () => Promise<{clean: boolean}>}} PaymentsRuntime */
/** @typedef {{createPaymentsRuntime: typeof createPaymentsRuntime, start?: () => PaymentsRuntime, stop?: () => Promise<void>, inspectState?: () => PaymentStateSnapshot, planBatches?: () => Promise<unknown[]>, recoverPendingBatches?: (source: string) => Promise<boolean>, runCycle?: () => Promise<{clean: boolean}>}} PaymentsModule */
...
*/
/** @param {PaymentsOptions} [options] @returns {PaymentsRuntime} */
function createPaymentsRuntime(options) {
    const opts = options || {};
    const mysqlPool = opts.mysql || global.mysql;
    const database = getLocalDatabase(opts.database || global.database);
    ...
    const state = {
        ...
        isFailStop: false,
        lockConnection: null,
        lockConnectionEndHandler: null,
        lockConnectionErrorHandler: null,
    ...
    /** @param {PaymentConnection} connection @returns {Promise<number|null>} */
    ...
    /** @param {boolean} destroyConnection @returns {void} */
    function clearAdvisoryLockConnection(destroyConnection) {
        const connection = state.lockConnection;
        if (!connection) return;
        if (typeof connection.removeListener === "function") {
            if (state.lockConnectionErrorHandler) connection.removeListener("error", state.lockConnectionErrorHandler);
            if (state.lockConnectionEndHandler) connection.removeListener("end", state.lockConnectionEndHandler);
        }
        state.lockConnection = null;
        state.lockConnectionErrorHandler = null;
        state.lockConnectionEndHandler = null;
        state.lockConnectionId = null;
        try {
            if (destroyConnection && typeof connection.destroy === "function") connection.destroy();
            else if (typeof connection.release === "function") connection.release();
        } catch (_error) { /* best-effort connection cleanup; ignore errors */ }
    }

    /** @param {string} message @returns {void} */
    function handleAdvisoryLockConnectionClose(message) {
        const connectionId = state.lockConnectionId;
        clearAdvisoryLockConnection(true);
        enterFailStop("lock", `advisory lock connection ${  connectionId  } closed: ${  message}`).catch(function onFailStopError(error) {
            logError("lock", `failed to enter fail-stop after advisory lock connection close: ${  formatError(error)}`);
        });
    }

    /** @param {PaymentConnection|null} connection @returns {void} */
    function attachAdvisoryLockConnectionHandlers(connection) {
        if (!connection || typeof connection.once !== "function") return;
        state.lockConnectionErrorHandler = function onLockConnectionError(error) {
            handleAdvisoryLockConnectionClose(formatError(error));
        };
        state.lockConnectionEndHandler = function onLockConnectionEnd() {
            handleAdvisoryLockConnectionClose("connection ended");
        };
        connection.once("error", state.lockConnectionErrorHandler);
        connection.once("end", state.lockConnectionEndHandler);
    }

    /** @param {string} scope @param {string} message @param {number|string|null} [batchId] @returns {Promise<boolean>} */
    async function enterFailStop(scope, message, batchId) {
        if (batchId !== null && typeof batchId !== "undefined") state.activeBatchId = batchId;
    ...
    async function ensureAdvisoryLock() {
        ...
        try {
            ...
            attachAdvisoryLockConnectionHandlers(state.lockConnection);
    ...
    async function releaseAdvisoryLock() {
        ...
        } catch (_error) { /* best-effort advisory lock release; ignore errors */ }
        clearAdvisoryLockConnection(false);
    }
    ...
    /** @param {boolean} createdUnresolvedBatch @returns {boolean} */
    function canAdvancePaymentCycle(createdUnresolvedBatch) {
        return !createdUnresolvedBatch && !state.isFailStop;
    }
    ...
    async function runCycle() {
        try {
            ...
            if (!await recoverPendingBatches("startup-recovery")) return { clean: false };
            ...
            if (canAdvancePaymentCycle(createdUnresolvedBatch)) {
    ...
    async function recoverPendingBatches(source) {
        if (state.isFailStop) return false;
        if (!await ensureAdvisoryLock()) return false;
        return await processor.recoverPendingBatches(source);
    }
    ...
    function scheduleNextCycle(delayMs) {
        ...
        state.timer = setTimeoutFn(async function onTimer() {
            ...
            try {
                ...
                if (state.started && !state.isFailStop) scheduleNextCycle(Number(config.payout["timer"]) * 60 * 1000);
            }
        }, delayMs);
...
/** @type {PaymentsModule} */
const exportedPayments = { createPaymentsRuntime };
if (runtime) Object.assign(exportedPayments, runtime);
module.exports = exportedPayments;
