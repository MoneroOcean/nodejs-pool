...
function createPaymentsRuntime(options) {
    ...
    function formatAdvisoryLockError(error) {
        const message = error && error.message ? error.message : formatError(error);
        if (/Cannot enqueue Query after fatal error/i.test(message)) {
            return "MySQL advisory-lock connection is no longer usable; payments stopped to preserve exclusive payout ownership";
        }
    ...
    function attachAdvisoryLockConnectionHandlers(connection) {
        ...
        state.lockConnectionErrorHandler = function onLockConnectionError(error) {
            handleAdvisoryLockConnectionClose(formatAdvisoryLockError(error));
        };
    ...
    async function ensureAdvisoryLock() {
        ...
        try {
            ...
            clearAdvisoryLockConnection(true);
            return await enterFailStop("lock", "failed to acquire advisory lock: " + formatAdvisoryLockError(error));
        }
    }
    ...
    async function verifyAdvisoryLock() {
        ...
        try {
            ...
            clearAdvisoryLockConnection(true);
            return await enterFailStop("lock", "advisory lock check failed: " + formatAdvisoryLockError(error));
        }
    }
    ...
    function canAdvancePaymentCycle(createdUnresolvedBatch, hadRetryableBatch) {
        return !createdUnresolvedBatch && !hadRetryableBatch && !state.isFailStop && database && typeof database.setCache === "function";
    }
    ...
    async function runCycle() {
        try {
            ...
            if (canAdvancePaymentCycle(createdUnresolvedBatch, hadRetryableBatch)) {
