...
/** @typedef {import("../../types/pool_profiles").PoolBlockTemplate} BlockTemplateRecord */
 ...
 * @property {Map<string, import("../../types/pool_profiles").PoolMiner>} activeMiners
 * @property {Map<string, Set<string>>} activeMinersByPayout
 * @property {Map<string, import("./servers").PoolSocket>} activeMinerSockets
 * @property {TemplateMap} activeBlockTemplates
 * @property {Record<string, {enq: (value: BlockTemplateRecord) => void, toarray: () => BlockTemplateRecord[]}>} pastBlockTemplates
 ...
 * @property {Record<string, import("./servers").ProtocolWarning>} protocolWarningState
 ...
 * @property {Record<number, number>} minerCount
 * @property {Record<number, Record<number, number>>} workerMinerCounts
...
module.exports = function createPoolState() {
    ...
    /** @param {string} methodName @returns {number|bigint} */
    ...
    function ravenTargetHex(diff) {
        return padHex((Number(callCoinNumber("baseRavenDiff")) / Number(diff)).toString(16), 32);
    }
        ...
        if (!state.freeEthExtranonces.length) {
            ...
            if (state.lastEthExtranonceOverflowNoticeAt === 0 || timeNow - state.lastEthExtranonceOverflowNoticeAt >= cooldownMs) {
                ...
                if (global.support && typeof global.support.sendAdminFyi === "function") {
                    global.support.sendAdminFyi("pool:eth-extranonce-overflow", "FYI: Pool node has extranonce overflow", errStr, { cooldownMs });
                }
            }
            return null;
        }
    ...
    /** @overload @param {number} id @returns {string} */
    /** @overload @param {null} id @returns {null} */
     ...
     * @param {{threadName?: string, enableStats?: boolean, enableShareWindowReset?: boolean}} [options]
     * @returns {void}
     */
    ...
    function resetRuntimeState() {
        ...
        clearObject(state.minerCount);
        clearObject(state.workerMinerCounts);
