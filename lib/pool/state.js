...
module.exports = function createPoolState() {
    ...
    function getNewEthExtranonceId() {
        if (!state.freeEthExtranonces.length) {
            const errStr = state.threadName + "Pool server " + global.config.hostname + " has overflow extranonce of " + (16 - global.coinFuncs.uniqueWorkerIdBits) + " bits";
            ...
            if (state.lastEthExtranonceOverflowNoticeAt === 0 || timeNow - state.lastEthExtranonceOverflowNoticeAt >= cooldownMs) {
                ...
                if (global.support && typeof global.support.sendAdminFyi === "function") {
                    global.support.sendAdminFyi("pool:eth-extranonce-overflow", "FYI: Pool node has extranonce overflow", errStr, { cooldownMs: cooldownMs });
                }
            }
            return null;
        }
    ...
    function callCoinDisplay(method, port) {
        if (typeof global.coinFuncs[method] !== "function") return null;
        const coin = global.coinFuncs[method](port);
    ...
    function initThreadContext(isMaster, workerId, options) {
        ...
        if (isMaster) {
            state.threadName = options.threadName || formatThreadName({ primary: true });
            ...
            state.threadName = options.threadName || formatThreadName({ workerId: resolvedWorkerId, pid: process.pid });
