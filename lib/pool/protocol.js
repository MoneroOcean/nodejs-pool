...
module.exports = function createProtocolHandler(deps) {
    ...
    return function handleMinerData(socket, id, method, params, ip, portData, sendReply, sendReplyFinal, pushMessage) {
        ...
        let rateMethod = null;
        if (method === "login" || method === "mining.authorize") rateMethod = "login";
        ...
        case "login": {
            ...
            if (socket.firstShareTimer) clearTimeout(socket.firstShareTimer);
            if ((typeof socket.destroy === "function" || typeof socket.end === "function") && securityConfig.minerFirstShareTimeoutMs > 0) {
                ...
                }, securityConfig.minerFirstShareTimeoutMs);
            }
