 ...
 *     sendNamedLoginResult?: PoolProfileSettings["sendNamedLoginResult"],
 *     normalizeNamedAuthorizeParams?: PoolProfileSettings["normalizeNamedAuthorizeParams"],
 ...
 *     normalizeNamedSubmitParams?: PoolProfileSettings["normalizeNamedSubmitParams"],
 *     sanitizeSubmitParams?: PoolProfileSettings["sanitizeSubmitParams"],
...
function isProtocolParams(value) {
    ...
    for (const [key, field] of Object.entries(value)) {
        if (["login", "pass", "rigid", "agent", "result", "header_hash", "mixhash", "block_submit_test_result", "wallet", "worker", "plain_proof", "proof_encoding", "type"].includes(key)) {
            ...
            if (typeof field !== "string" && (!Array.isArray(field) || !field.every(item => typeof item === "string"))) return false;
        } else if (key === "pow") {
...
module.exports = function createProtocolHandler(deps) {
    ...
    return function handleMinerData(socket, id, method, inputParams, ip, portData, sendReply, sendReplyFinal, pushMessage, request) {
        ...
        const rateMethod = Object.hasOwn(RATE_LIMIT_METHODS, method) ? RATE_LIMIT_METHODS[method] : null;
        if (rateMethod && !consumeRpcRateLimit(rateMethod, ip, timeNow)) {
            sendReplyFinal(`Rate limit exceeded for ${  rateMethod  } requests`);
            return;
        }
        params = normalizeProtocolParams(inputParams);

        /** @returns {boolean} */
        function normalizeAuthorizeParams() {
            if (!(wireParams instanceof Array)) {
                params = normalizeProtocolParams(inputParams);
                let profile = getPoolProfileForPort(portData.port);
                let poolSettings = profile && profile.pool;
                // Pearl's stratum dialect identifies itself with wallet/worker
                // plus either pass or the SRBMiner agent/type pair before a job
                // can bind the shared-port session to a coin profile.
                if ((!poolSettings || typeof poolSettings.normalizeNamedAuthorizeParams !== "function") &&
                    typeof params.wallet === "string" && typeof params.worker === "string" &&
                    (typeof params.pass === "string" || (typeof params.agent === "string" && typeof params.type === "string"))) {
                    profile = global.coinFuncs.getPoolProfile("PRL");
                    poolSettings = profile && profile.pool;
                }
                if (profile && poolSettings && typeof poolSettings.normalizeNamedAuthorizeParams === "function") {
                    if (poolSettings.normalizeNamedAuthorizeParams({ params, port: portData.port, profile })) return true;
                    sendReplyFinal("Invalid authorize params");
                    return false;
                }
        ...
        function buildNativeGetJobResult(miner) {
            ...
            let freshJob = miner.getCoinJob(coin, jobParams);
            if (freshJob === null && miner.cachedJob !== null) {
                // Native proxies treat every explicit getjob reply as new work.
                // Do not replay the cached job (and its job_id), which makes
                // xmrig-proxy disconnect on its duplicate-job guard.
                delete miner.jobLastBlockHash;
                freshJob = miner.getCoinJob(coin, jobParams);
            }
        ...
        function sendLoginJob(miner, minerId) {
            ...
            if (method === "mining.authorize") {
                const namedCoin = miner.selectBestCoin();
                const namedSettings = namedCoin !== false ? getPoolSettingsForCoin(namedCoin) : null;
                if (typeof namedCoin === "string" && namedSettings && typeof namedSettings.sendNamedLoginResult === "function") {
                    namedSettings.sendNamedLoginResult({
                        coin: namedCoin,
                        jobParams: deps.getCoinJobParams(namedCoin),
                        miner,
                        minerId,
                        sendReply: function sendNamedLoginReply(error, result) {
        ...
        function normalizeMiningSubmitParams() {
            if (!(wireParams instanceof Array)) {
                ...
                const jobId = typeof params.job_id === "number" ? String(params.job_id) : params.job_id;
                const job = miner && miner.validJobs.toarray().find(function findNamedSubmitJob(candidate) {
                    return candidate.id === jobId;
                });
                const jobPort = job && typeof job.coin === "string" ? global.coinFuncs.COIN2PORT(job.coin) : undefined;
                const profile = typeof jobPort === "number" ? getPoolProfileForPort(jobPort) : null;
                const poolSettings = profile && profile.pool;
                // Do not inspect or decode a named proof until the job id has
                // passed the normal lookup and invalid-job accounting path.
                if (method === "mining.submit" && miner && !job) return true;
                if (method === "mining.submit" && miner && poolSettings && typeof poolSettings.normalizeNamedSubmitParams === "function") {
                    const namedSubmitContext = {
                        coinFuncs: global.coinFuncs,
                        params,
                        wireParams,
                        ...(job === undefined ? {} : { job }),
                    ...
                    if (poolSettings.normalizeNamedSubmitParams(namedSubmitContext)) return true;
                    rejectBadShare(miner, "Invalid job params");
                    return false;
                }
        ...
        /** @param {PoolSubmitSettings} poolSettings @returns {Record<string, unknown>} */
        function getSubmitLogParams(poolSettings) {
            if (poolSettings.sensitiveSubmitData === true) {
                if (typeof poolSettings.sanitizeSubmitParams === "function") {
                    try { return poolSettings.sanitizeSubmitParams({ params }); } catch (_error) { return {}; }
                }
                return {};
            }
            return params;
        ...
        function validateSubmitNonce(miner, job, poolSettings) {
            ...
            if (!isNonceValid) {
                console.warn(state.threadName + formatPoolEvent("Malformed nonce", { miner: miner.logString, params: getSubmitLogParams(poolSettings) }));
        ...
        function handleSubmitRequest() {
            ...
            const nonceTest = poolSettings.submissionKey({ coinFuncs: global.coinFuncs, miner, job: trackedJob, params });
        ...
        const handler = Object.hasOwn(methodHandlers, method) ? methodHandlers[method] : undefined;
        if (handler) handler();
