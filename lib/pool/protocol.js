...
module.exports = function createProtocolHandler(deps) {
    ...
    function hasSlowStartClaimedExtranonceAlgo(miner) {
        if (!miner || !(miner.algos instanceof Object)) return false;
        return "autolykos2" in miner.algos || "etchash" in miner.algos || "ethash" in miner.algos;
    }
    ...
    return function handleMinerData(socket, id, method, params, ip, portData, sendReply, sendReplyFinal, pushMessage) {
        function closeSocketAfterReply() {
            ...
            if (typeof socket.end === "function" && socket.writable) socket.end();
        ...
        function scheduleFirstShareTimer(minerId) {
            ...
            const config = getPoolSecurityConfig();
            let timeoutMs = config.minerFirstShareTimeoutMs;
            if (
                Number.isInteger(socket.eth_extranonce_id) &&
                config.claimedExtranonceFirstShareTimeoutMs > 0 &&
                !hasSlowStartClaimedExtranonceAlgo(miner)
            ) {
                timeoutMs = Math.min(timeoutMs, config.claimedExtranonceFirstShareTimeoutMs);
            }
