...
const RATE_LIMIT_METHODS = Object.freeze({ login: "login", "eth_submitLogin": "login", "mining.authorize": "login", "mining.subscribe": "login", "mining.extranonce.subscribe": "login", submit: "submit", "mining.submit": "submit", "eth_submitWork": "submit", keepalive: "keepalive", keepalived: "keepalive", eth_mining: "keepalive", eth_submitHashrate: "keepalive" });
const ETH_PROXY_WORK_RETENTION = 64;
...
module.exports = function createProtocolHandler(deps) {
    ...
    function claimEthExtranonceId(socket) {
        if (Number.isInteger(socket.eth_extranonce_id)) return socket.eth_extranonce_id;
        if (Number.isInteger(socket.eth_extranonce_preview_id)) {
    ...
    function ethProxyTarget(coinDiff) {
        ...
        // eth-style target = floor((2^256 - 1) / difficulty), computed in BigInt with
        // scale (1e6) giving 6 decimal digits of fractional-difficulty precision.
    ...
    function buildEthProxyWorkResult(job) {
        return [hexPrefix(job[2]), hexPrefix(job[1]), ethProxyTarget(job[4])];
    }

    function rememberEthProxyWork(miner, job) {
        if (!(job instanceof Array) || job.length < 5) return;
        if (!(miner.ethProxyWorkByHeader instanceof Map)) miner.ethProxyWorkByHeader = new Map();
        miner.ethProxyWorkByHeader.set(normalizeHex(job[2]), String(job[0]));
        while (miner.ethProxyWorkByHeader.size > ETH_PROXY_WORK_RETENTION) miner.ethProxyWorkByHeader.delete(miner.ethProxyWorkByHeader.keys().next().value);
    }

    function prepareEthProxyWorkTracking(miner) {
        miner.validJobs = global.support.circularBuffer(ETH_PROXY_WORK_RETENTION);
        miner.ethProxyWorkByHeader = new Map();
        miner.rememberEthProxyWork = function rememberMinerEthProxyWork(job) {
            rememberEthProxyWork(miner, job);
        };
        miner.buildEthProxyWorkResult = buildEthProxyWorkResult;
    }
    ...
    return function handleMinerData(socket, id, method, params, ip, portData, sendReply, sendReplyFinal, pushMessage) {
        ...
        function normalizeAuthorizeParams() {
            ...
            // eslint-disable-next-line no-param-reassign -- normalizes the raw array request into the shared object form consumed by the rest of this handler
        ...
        function normalizeEthProxyLoginParams() {
            if (!params || !(params instanceof Array)) {
                sendReplyFinal("No array params specified");
                return false;
            }
            const authorizeAlgoState = getAuthorizeAlgoState(portData.port);
            // eslint-disable-next-line no-param-reassign -- normalizes the raw array request into the shared object form consumed by the rest of this handler
            params = { login: params[0], pass: params[1], agent: "[generic_eth_getwork]", algo: authorizeAlgoState.algos, "algo-perf": authorizeAlgoState.algosPerf, "algo-min-time": authorizeAlgoState.algoMinTime };
            return true;
        }
        ...
        function applyAuthorizeExtranonce(miner) {
            ...
            const newId = claimEthExtranonceId(socket);
            if (newId !== null) {
        ...
        function registerMinerWallet(miner) {
            ...
            if (state.minerWallets[miner.payout].count < global.config.pool.workerMax) {
                ++state.minerWallets[miner.payout].count;
                return true;
            }
        ...
        function sendLoginJob(miner, minerId) {
            ...
            if (method === "eth_submitLogin") {
                sendReply(null, true);
                miner.protocol = "ethproxy";
                prepareEthProxyWorkTracking(miner);
                return true;
            }
        ...
        function getSocketMiner() {
            const minerId = socket.miner_id || "";
            return minerId ? state.activeMiners.get(minerId) : null;
        }
        ...
        function handleEthProxyGetWorkRequest() {
            const miner = getAuthenticatedSocketMiner();
            if (!miner) return;
            if (!consumePreShareRateLimit("job-request", miner, ip, timeNow)) {
                sendReplyFinal("Rate limit exceeded for job requests before first valid share");
            ...
            miner.touchProtocolActivity();
            const job = miner.getBestCoinJob() || miner.cachedJob;
            if (!(job instanceof Array) || job.length < 5) {
                sendReplyFinal("No block template yet. Please wait.");
                return;
            }
            rememberEthProxyWork(miner, job);
            sendReply(null, buildEthProxyWorkResult(job));
        }
        ...
        function getSubmitBlockTemplate(miner, job) {
            ...
            if (state.activeBlockTemplates[job.coin].idHash === job.blockHash) {
                return state.activeBlockTemplates[job.coin];
            }
        ...
        function rejectExpiredShare(miner, job, errStr) {
            ...
            if (miner.protocol === "ethproxy") closeSocketAfterReply();
        }
        ...
        function handleSubmitRequest() {
            ...
            if (!Number.isFinite(currentCoinHashFactor) || currentCoinHashFactor <= 0) {
                const blockTemplate = state.activeBlockTemplates[trackedJob.coin];
                if (currentCoinHashFactor === 0 && blockTemplate && Date.now() - blockTemplate.timeCreated > 10 * 60 * 1000) {
            ...
            if (trackedJob.coinHashFactor > currentCoinHashFactor) {
                sendReply("Block expired");
                return;
            }
            miner.touchProtocolActivity();
        ...
        function handleEthProxySubmitWorkRequest() {
            const miner = getSubmitMiner();
            if (!miner) return;
            ...
            const header = normalizeHex(params[1]);
            const jobId = miner.ethProxyWorkByHeader instanceof Map ? miner.ethProxyWorkByHeader.get(header) : null;
            if (!jobId) {
                rejectInvalidSubmitJob(miner);
                return;
            }
            // eslint-disable-next-line no-param-reassign -- normalizes the raw array request into the shared object form consumed by the rest of this handler
            params = {
                job_id: jobId,
                nonce: params[0],
                raw_params: [miner.payout, jobId, params[0], params[1], params[2]]
            };
            handleSubmitRequest();
        }

        function handleEthProxyTelemetryRequest() {
            const miner = getAuthenticatedSocketMiner();
            if (!miner) return;
            miner.touchProtocolActivity();
            sendReply(null, true);
        ...
        const methodHandlers = {
            eth_submitLogin: function onEthProxyLogin() { if (normalizeEthProxyLoginParams()) handleLoginRequest(); },
            eth_getWork: handleEthProxyGetWorkRequest,
            eth_submitWork: handleEthProxySubmitWorkRequest,
            eth_submitHashrate: handleEthProxyTelemetryRequest,
            eth_mining: handleEthProxyTelemetryRequest,
