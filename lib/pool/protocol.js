 ...
 *     extensions?: string[],
 *     algo?: string[],
 *     "algo-perf"?: Record<string, number>,
 *     "algo-min-time"?: number|string,
...
function isProtocolParams(value) {
    ...
    for (const [key, field] of Object.entries(value)) {
        if (["login", "pass", "rigid", "agent", "result", "header_hash", "mixhash", "block_submit_test_result"].includes(key)) {
        ...
        } else if (["job_id", "nonce", "algo-min-time"].includes(key)) {
            ...
            if (!field || typeof field !== "object" || Array.isArray(field) ||
                !Object.values(field).every(item => typeof item === "number")) return false;
        }
    }
    return true;
}
