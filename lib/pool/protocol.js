...
module.exports = function createProtocolHandler(deps) {
    ...
    function normalizeExtraNonceSubmitNonce(nonce, extraNonce) {
        ...
        // the full 8-byte nonce. Full nonces must still stay inside the segment
        // assigned by the pool extranonce.
        ...
        if (normalizedNonce.length === fullNonceHexLength) {
            return normalizedNonceLower.startsWith(normalizedExtraNonce) ? normalizedNonceLower : null;
        }
    ...
    return function handleMinerData(socket, id, method, params, ip, portData, sendReply, sendReplyFinal, pushMessage) {
        ...
        case "login": {
            ...
            if (!miner.valid_miner) {
                ...
                if (!(invalidLogKey in state.lastMinerLogTime) || timeNow - state.lastMinerLogTime[invalidLogKey] > 10 * 60 * 1000) {
                    console.log(state.threadName + formatPoolEvent("Invalid miner", {
            ...
            if (minerNotification && (!(miner.payout in state.lastMinerNotifyTime) || timeNow - state.lastMinerNotifyTime[miner.payout] > 60 * 60 * 1000)) {
                ...
                console.error(state.threadName + formatPoolEvent("Miner notice", {
                    miner: miner.logString,
                    message: minerNotification
                }));
        ...
        case "mining.subscribe": {
            ...
            if (previewId !== null) {
                sendReply(null, [["mining.notify", utils.getNewId(), "EthereumStratum/1.0.0"], utils.ethExtranonce(previewId)]);
            } else {
        ...
        case "submit": {
            ...
            if (!isNonceValid) {
                console.warn(state.threadName + formatPoolEvent("Malformed nonce", {
                    miner: miner.logString,
                    params
                }));
            ...
            if (state.activeBlockTemplates[job.coin].idHash !== job.blockHash) {
                ...
                if (!blockTemplate || isOutdated) {
                    ...
                    if (!(miner.payout in state.lastMinerLogTime) || timeNow - state.lastMinerLogTime[miner.payout] > 30 * 1000) {
                        console.warn(state.threadName + formatPoolEvent("Share rejected", {
                            reason: errStr,
                            height: job.height,
                            diff: job.difficulty,
                            miner: miner.logString
            ...
            if (!shareWindowThrottled) {
                ...
                if (hasReachedSubmissionLimit(job, miner)) {
                    console.warn(state.threadName + formatPoolEvent("Share limit", {
                        tracked: submissions.size,
                        miner: miner.logString
                    }));
