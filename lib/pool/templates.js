...
/** @typedef {import("../../types/pool_profiles").PoolBlockTemplate} ActiveTemplate */
...
/** @typedef {{trust?: {check_height: number}, algos: Record<string, number>, curr_coin?: string|false, sendBestCoinJob: () => void, sendCoinJob: (coin: string, params: TemplateJobParams) => void}} TemplateMiner */
/** @typedef {{isMaster: boolean, workers?: Record<number, {send: (message: ProtoMessage) => void}|undefined>}} TemplateCluster */
...
function isActiveTemplate(value) {
    return typeof readField(value, "coin") === "string" && typeof readField(value, "port") === "number" &&
        ...
        typeof readField(value, "idHash") === "string" &&
        typeof readField(value, "nextBlobHex") === "function";
}
...
/** @param {TemplateManagerDeps} deps */
module.exports = function createTemplateManager(deps) {
    ...
    const workerTemplateIds = new Map();
    /** @type {Map<string, {template: UsableTemplate, headers: Map<string, number>}>} */
    const workerTemplateBases = new Map();

    /** @param {number} clusterWorkerId @param {number} uniqueWorkerId */
    function registerWorkerTemplate(clusterWorkerId, uniqueWorkerId) {
        if (!cluster.isMaster || !Number.isSafeInteger(clusterWorkerId) || clusterWorkerId < 1 ||
            !Number.isSafeInteger(uniqueWorkerId) || uniqueWorkerId < 0 || !cluster.workers?.[clusterWorkerId]) return;
        workerTemplateIds.set(clusterWorkerId, uniqueWorkerId);
        for (const [coin, base] of workerTemplateBases) {
            if (Number(newCoinHashFactor[coin]) > 0) publishWorkerTemplate(coin, base, clusterWorkerId, uniqueWorkerId);
        }
    }

    /** @param {number} clusterWorkerId */
    function unregisterWorkerTemplate(clusterWorkerId) {
        workerTemplateIds.delete(clusterWorkerId);
    }

    /** @param {string} coin @param {{template: UsableTemplate, headers: Map<string, number>}} base @param {number} clusterWorkerId @param {number} uniqueWorkerId @param {number} [attempt] */
    function publishWorkerTemplate(coin, base, clusterWorkerId, uniqueWorkerId, attempt = 0) {
        const worker = cluster.workers?.[clusterWorkerId];
        if (!worker) return;
        if (workerTemplateIds.get(clusterWorkerId) !== uniqueWorkerId ||
            workerTemplateBases.get(coin) !== base || !(Number(newCoinHashFactor[coin]) > 0)) return;

        /** @param {TemplateRecord|null} variant @param {unknown} [error] */
        function onVariant(variant, error) {
            if (!worker || workerTemplateBases.get(coin) !== base || workerTemplateIds.get(clusterWorkerId) !== uniqueWorkerId ||
                cluster.workers?.[clusterWorkerId] !== worker || !(Number(newCoinHashFactor[coin]) > 0)) return;
            const original = base.template;
            const valid = variant && isRpcTemplate(variant) && typeof variant.hash === "string" &&
                variant.height === original.height && variant.difficulty === original.difficulty &&
                (base.headers.get(variant.hash) === undefined || base.headers.get(variant.hash) === uniqueWorkerId);
            if (valid && variant && typeof variant.hash === "string") {
                base.headers.set(variant.hash, uniqueWorkerId);
                try {
                    worker.send({ type: "newBlockTemplate", data: processRpcTemplate(
                        variant, coin, original.port, original.coinHashFactor, original.isHashFactorChange
                    ) });
                } catch (sendError) {
                    console.error(getThreadName() + formatPoolEvent("Worker template send failed", {
                        chain: formatCoinPort(coin, original.port), worker: uniqueWorkerId,
                        error: formatRpcErrorDetail(sendError)
                    }));
                }
                return;
            }
            if (attempt < 2) {
                const retry = setTimeout(publishWorkerTemplate, 500, coin, base, clusterWorkerId, uniqueWorkerId, attempt + 1);
                if (typeof retry.unref === "function") retry.unref();
            } else {
                console.error(getThreadName() + formatPoolEvent("Worker template unavailable", {
                    chain: formatCoinPort(coin, original.port), worker: uniqueWorkerId,
                    reason: error ? formatRpcErrorDetail(error) : "variant-mismatch"
                }));
            }
        }

        const getVariant = global.coinFuncs.getPoolProfile(coin)?.rpc?.getWorkerBlockTemplate;
        if (typeof getVariant !== "function") return;
        getVariant({
            baseTemplate: base.template,
            workerId: uniqueWorkerId,
            callback(rawVariant, error) {
                onVariant(isRpcTemplate(rawVariant) ? rawVariant : null, error);
            }
        });
    }
    ...
    const DAEMON_FACTOR_GRACE_MS = 60 * 1000;
    /** @type {Map<string, Partial<Record<"header"|"template", {timer?: NodeJS.Timeout}>>>} */
    const factorFailureTimers = new Map();

    /** @param {string} coin @param {"header"|"template"} rpc @returns {void} */
    function clearFactorFailure(coin, rpc) {
        const timers = factorFailureTimers.get(coin);
        if (!timers) return;
        const failure = timers[rpc];
        if (!failure) return;
        if (failure.timer) clearTimeout(failure.timer);
        delete timers[rpc];
        if (!timers.header && !timers.template) factorFailureTimers.delete(coin);
    }

    /** @param {string} coin @param {"header"|"template"} rpc @returns {void} */
    function handleDaemonRpcFailure(coin, rpc) {
        if (coin === "") return;
        const currentFactor = Number(newCoinHashFactor[coin]);
        if (!(currentFactor > 0)) {
            newCoinHashFactor[coin] = 0;
            return coinHashFactorUpdate(coin, 0);
        }

        let timers = factorFailureTimers.get(coin);
        if (!timers) {
            timers = {};
            factorFailureTimers.set(coin, timers);
        }
        if (timers[rpc]) return;
        const failure = {};
        timers[rpc] = failure;
        failure.timer = setTimeout(function disableFailedCoin() {
            const currentTimers = factorFailureTimers.get(coin);
            if (!currentTimers || currentTimers[rpc] !== failure) return;
            delete currentTimers[rpc];
            if (!currentTimers.header && !currentTimers.template) factorFailureTimers.delete(coin);
            // Assign first because coinHashFactorUpdate intentionally skips a
            // duplicate zero when the last published factor is already zero.
            newCoinHashFactor[coin] = 0;
            coinHashFactorUpdate(coin, 0);
        }, DAEMON_FACTOR_GRACE_MS);
        if (failure.timer && typeof failure.timer.unref === "function") failure.timer.unref();
    }

    /** @param {string} coin @param {number} port @param {TemplateHeader} header @returns {void} */
    function markHeaderHealth(coin, port, header) {
        if (coin !== "") {
            clearFactorFailure(coin, "header");
            return markDaemonFailure(coin, port, false);
        }
    ...
    /** @param {string} coin @param {number} port @param {number} coinHashFactor @param {boolean} isHashFactorChange @param {TemplateRecord} bodyBt @returns {void} */
    function templateUpdate3(coin, port, coinHashFactor, isHashFactorChange, bodyBt) {
        ...
        if (cluster.isMaster) {
            ...
            if (typeof profile?.rpc?.getWorkerBlockTemplate === "function") {
                const base = {
                    template: /** @type {UsableTemplate} */ (template),
                    headers: new Map([[String(template.hash), 0]])
                };
                workerTemplateBases.set(coin, base);
                for (const [clusterWorkerId, uniqueWorkerId] of workerTemplateIds) {
                    publishWorkerTemplate(coin, base, clusterWorkerId, uniqueWorkerId);
                }
            } else {
    ...
    function templateUpdate2(coin, port, isHashChange, isMMHashChange, coinHashFactor, isHashFactorChange, bodyHeader, timesFailed) {
        ...
        /** @param {string} reason @param {Record<string, unknown>} fields @param {boolean} [daemonRpcFailure] */
        function retryTemplateFetch(reason, fields, daemonRpcFailure) {
            ...
            if (daemonRpcFailure) {
                ...
                handleDaemonRpcFailure(coin, "template");
            }
        ...
        /** @param {RpcTemplate|null} bodyBt @param {unknown} templateError @param {AuxTemplate|null} auxChainXtm @param {boolean} daemonRpcFailure @returns {boolean} */
        function rejectInvalidTemplate(bodyBt, templateError, auxChainXtm, daemonRpcFailure) {
            if (!bodyBt) return retryTemplateFetch("Template fetch failed", { error: formatRpcErrorDetail(templateError) }, daemonRpcFailure), true;
        ...
        global.coinFuncs.getPortBlockTemplate(port, function onTemplate(rawTemplate, templateError) {
            ...
            const auxChainXtm = isAuxTemplate(auxValue) ? auxValue : null;
            const daemonRpcFailure = rawTemplate === null || typeof rawTemplate === "undefined";
            if (rejectInvalidTemplate(template, templateError, auxChainXtm, daemonRpcFailure)) return;
            clearFactorFailure(coin, "template");
            ...
            if (isHashChange || isMMHashChange || (isTimeChange && (!isRewardCheckReady || expectedReward !== previousReward)) || isRewardChange) {
                ...
                return templateUpdate3(coin, port, coinHashFactor, isHashFactorChange, template);
            }
        }, true);
    }
    ...
    function coinHashFactorUpdate(coin, coinHashFactor) {
        ...
        if (coinHashFactor === 0) {
            clearFactorFailure(coin, "header");
            clearFactorFailure(coin, "template");
            workerTemplateBases.delete(coin);
        }
    ...
    function handleTemplateHeader(coin, port, coinHashFactor, repeating, err, body) {
        ...
        if (err !== null || !body || !body.hash) {
            ...
            markDaemonFailure(coin, port, true);
            handleDaemonRpcFailure(coin, "header");
    ...
    return {
        ...
        templateUpdate,
        templateUpdate2,
        registerWorkerTemplate,
        unregisterWorkerTemplate,
