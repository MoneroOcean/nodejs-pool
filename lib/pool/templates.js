...
module.exports = function createTemplateManager(deps) {
    ...
    function setNewBlockTemplate(template) {
        ...
        let candidate;
        try {
            candidate = new global.coinFuncs.BlockTemplate(template);
        } catch (error) {
            console.error(getThreadName() + formatPoolEvent("Template invalid", {
                chain: formatCoinPort(coin, template.port), reason: formatRpcErrorDetail(error)
            }));
            return;
        }
        // Keep the previous work live if the new template is malformed.
