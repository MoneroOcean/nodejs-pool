...
module.exports = function createTemplateManager(deps) {
    ...
    function markHeaderHealth(coin, port, header) {
        ...
        setDaemonFailure(`xmr:${  port}`, stale(header.timestamp));
        setDaemonFailure(`xtm:${  port}`, header.mm && stale(header.mm.timestamp));
    }
    ...
    function templateUpdate3(coin, port, coinHashFactor, isHashFactorChange, bodyBt, bodyHeader = bodyBt) {
    ...
    function templateUpdate2(coin, port, isHashChange, isMMHashChange, coinHashFactor, isHashFactorChange, bodyHeader, timesFailed) {
        ...
        global.coinFuncs.getPortBlockTemplate(port, function (bodyBt, templateError) {
            ...
            if (isHashChange || isMMHashChange || (isTimeChange && (!isRewardCheckReady || bodyBt.expected_reward !== lastBlockReward[coin])) || isRewardChange) {
                ...
                return templateUpdate3(coin, port, coinHashFactor, isHashFactorChange, bodyBt, bodyHeader);
            }
        }, true);
    }
    ...
    function handleTemplateHeader(coin, port, coinHashFactor, repeating, err, body) {
        ...
        if (err !== null || !body.hash) {
            ...
            markDaemonFailure(coin, port, true);
            coinHashFactorUpdate(coin, 0);
            scheduleTemplateRepeat(coin, repeating, global.config.daemon.pollInterval, true);
            return;
        }
    ...
    function getCoinJobParams(coin) {
        ...
        const hashesPerDifficulty = typeof global.coinFuncs.getPoolHashesPerDifficulty === "function"
            ? global.coinFuncs.getPoolHashesPerDifficulty(params.bt.port)
            : 1;
        if (hashesPerDifficulty !== 1) params.hashesPerDifficulty = hashesPerDifficulty;
