...
module.exports = function createTemplateManager(deps) {
    ...
    function getDaemonHealthKeys(coin, port) {
        ...
        const factorKey = `factor:${  coin}:${  port}`;
        if (typeof coin === "string" && coin.startsWith("XTM")) return [`xtm:${  mainPort}`, factorKey];
        return [factorKey];
    }
    ...
    /** @param {string} coin @param {number} port @returns {void} */
    function handleDaemonRpcFailure(coin, port) {
        if (coin === "") return;
        ...
        const healthKeys = getDaemonHealthKeys(coin, port);
        const healthKey = healthKeys[healthKeys.length - 1];
        const failureSince = typeof healthKey === "string" ? Number(daemonFailureSince[healthKey]) : NaN;
        if (Number.isFinite(failureSince) && Date.now() - failureSince >= DAEMON_FACTOR_GRACE_MS) {
    ...
    function templateUpdate2(coin, port, isHashChange, isMMHashChange, coinHashFactor, isHashFactorChange, bodyHeader, timesFailed) {
        ...
        function retryTemplateFetch(reason, fields, daemonRpcFailure) {
            ...
            if (daemonRpcFailure) {
                ...
                handleDaemonRpcFailure(coin, port);
            }
