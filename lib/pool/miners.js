...
module.exports = function createMinerFactory(deps) {
    ...
    function validateLoginAndPayout(miner, loginDiffSplit, loginDivSplit, loginBase, dottedSuffix) {
        ...
        if (Math.abs(loginDivSplit.length % 2) === 0 || loginDivSplit.length > 5) return rejectMiner(miner, LOGIN_FORMAT_ERROR);
    ...
    function createMiner(id, login, pass, rigid, ipAddress, startingDiff, pushMessage, protoVersion, portType, port, agent, algos, algosPerf, algoMinTime) {
        ...
        const parsedPass = parsePassword(pass);
        const passAlgoSplit = parsedPass.passAlgoSplit;
        const passSplit = parsedPass.passSplit;
