...
module.exports = function createMinerFactory(deps) {
    ...
    function createMiner(id, login, pass, rigid, ipAddress, startingDiff, pushMessage, protoVersion, portType, port, agent, algos, algosPerf, algoMinTime) {
        ...
        miner.agent = agent;
        const normalizedLogin = normalizeInvalidLogValue(login);
        if (normalizedLogin !== "") miner.invalidLogKey = "invalid-login:" + normalizedLogin;
        ...
        for (let index = 1; index < loginDivSplit.length - 1; index += 2) {
            ...
            if (address.length !== 95 || !global.coinFuncs.validateAddress(address)) {
                return rejectMiner(
                    ...
                    "invalid-address-login:" + normalizeInvalidLogValue(address)
                );
            }
            if (address in state.bannedAddresses) {
                return rejectMiner(
                    "Permanently banned payment address " + address + " provided: " + state.bannedAddresses[address],
                    "banned-login:" + address
                );
            }
            if (address in state.bannedTmpWallets) {
                return rejectMiner("Temporary (10 minutes max) banned payment address " + address, "banned-login:" + address);
            }
        ...
        if (miner.payout in state.bannedAddresses) {
            return rejectMiner(
                ...
                "banned-login:" + miner.payout
            );
        }
        if (miner.payout in state.bannedTmpWallets) {
            return rejectMiner("Temporary (10 minutes max) banned payment address " + miner.payout, "banned-login:" + miner.payout);
        }
        ...
        if (!global.coinFuncs.validateAddress(miner.address)) {
            return rejectMiner(
                ...
                "invalid-address-login:" + normalizeInvalidLogValue(miner.address)
            );
        }
        ...
        if (portType !== "pplns") console.error("Unsupported portType " + portType + ", defaulting to pplns");
