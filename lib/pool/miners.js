...
module.exports = function createMinerFactory(deps) {
    ...
    function createMiner(id, login, pass, rigid, ipAddress, startingDiff, pushMessage, protoVersion, portType, port, agent, algos, algosPerf, algoMinTime) {
        ...
        function validatePayoutAddress(address, requirePlainAddress) {
            ...
            if (address in state.bannedTmpWallets) {
                return rejectMiner("Temporary (10 minutes max) banned payment address " + address, logKey);
            }
        ...
        if (
            ...
            return rejectMiner(LEGACY_PAYMENT_ID_ERROR, normalizeInvalidLogValue(loginBase));
        }
        if (typeof dottedSuffix !== "undefined" && miner.identifier === "x") {
            miner.identifier = dottedSuffix.substring(0, 64);
        }
        ...
        if (loginDiffSplit.length > 2) {
            return rejectMiner(LOGIN_FORMAT_ERROR);
        }
        ...
        for (let index = 1; index < loginDivSplit.length - 1; index += 2) {
            ...
            const invalidSplitAddress = validatePayoutAddress(address, true);
        ...
        const invalidPayoutAddress = validatePayoutAddress(miner.payout, false);
        if (invalidPayoutAddress) return invalidPayoutAddress;
