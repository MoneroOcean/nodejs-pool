...
module.exports = function createShareBlockHelpers(deps) {
    ...
    function isSafeToTrust(rewardDiff, trustKey, minerTrust) {
        ...
        const walletOk = (trustKey in walletTrust) && rewardDiff2 * trustThreshold < walletTrust[trustKey];
        ...
        function storeResolvedBlock(newBlockHash, isDisplaySubmitPort, reportDiff, reportPort, reportHeight) {
            ...
            if (isMainPort && !isDisplaySubmitPort) {
                ...
                const workDifficulty = typeof global.coinFuncs.getPoolWorkDifficulty === "function"
                    ? global.coinFuncs.getPoolWorkDifficulty(reportPort, reportDiff)
                    : reportDiff;
                global.database.storeAltBlock(Math.floor(timeNow / 1000), global.protos.AltBlock.encode({ hash: newBlockHash, difficulty: workDifficulty, shares: 0, timestamp: timeNow, poolType: miner.poolTypeEnum, unlocked: false, valid: true, port: reportPort, height: reportHeight, anchor_height: anchorState.current }));
            }
        }
