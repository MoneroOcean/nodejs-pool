...
module.exports = function createShareBlockHelpers(deps) {
    ...
    function isSafeToTrust(rewardDiff, trustKey, minerTrust) {
        ...
        // Invalid trust controls disable the shortcut until configuration is repaired.
        const configuredTrustThreshold = finiteNumberOr(global.config.pool["trustThreshold"], Number.POSITIVE_INFINITY);
        const trustThreshold = configuredTrustThreshold > 0 ? configuredTrustThreshold : Number.POSITIVE_INFINITY;
        const configuredTrustMin = finiteNumberOr(global.config.pool["trustMin"], 256);
        const trustMin = configuredTrustMin >= 0 && configuredTrustMin <= 256 ? configuredTrustMin : 256;
    ...
    /** @param {PoolBlockTemplate} blockTemplate @param {Buffer|unknown[]|string} blockData @param {Buffer|null} resultBuff @param {PoolJob} job @param {PoolMiner} miner @param {(result: LocalBlockCheck|null) => unknown} callback @returns {unknown} */
    ...
    /** @param {PoolBlockTemplate} blockTemplate @param {Buffer|unknown[]|string} blockData @param {Buffer|null} resultBuff @param {number|bigint} hashDiff @param {number|bigint|string|undefined|null} requiredDiff @param {PoolJob} job @param {PoolMiner} miner @param {(message: string) => unknown} callback @returns {unknown} */
     ...
     * @param {Buffer|unknown[]|string} blockData
    ...
    function submitBlock(miner, job, blockTemplate, blockData, resultBuff, hashDiff, isTrustedShare, isParentBlock, portUsedToSubmit, submitBlockCB, submitParams, submitRetryCount) {
        ...
        function formatSubmitData() {
            if (poolSettings.sensitiveSubmitData === true) return "[redacted]";
            return Buffer.isBuffer(blockData) ? blockData.toString("hex") : JSON.stringify(blockData);
        }

        /** @returns {unknown} */
        function formatSubmitParams() {
            if (poolSettings.sensitiveSubmitData === true && typeof poolSettings.sanitizeSubmitParams === "function") {
                try { return poolSettings.sanitizeSubmitParams({ params: submitParams || {} }); } catch (_error) { return {}; }
            }
            return submitParams;
        }
        ...
        const replyFn = function (rpcResult, rpcStatus, port, nextSubmitBlockCB, isDisplaySubmitPortOverride) {
            ...
            const blockDataStr = formatSubmitData();
