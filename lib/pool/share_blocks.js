...
module.exports = function createShareBlockHelpers(deps) {
    ...
    function getLocalBlockDiff(blockTemplate, blockData, job) {
        ...
        try {
            ...
            if (job && (job.coin === "ETH" || job.coin === "ETC" || job.coin === "ERG")) return hashEthBuffDiff(buff);
            if (job && (job.coin === "RVN" || job.coin === "XNA")) return hashRavenBuffDiff(buff);
            return hashBuffDiff(buff);
        } catch (_error) {
            return null;
        }
    }

    function getBlockSubmitDiffMessage(blockTemplate, blockData, hashDiff, requiredDiff, job) {
        const localDiff = getLocalBlockDiff(blockTemplate, blockData, job);
        ...
        if (localDiff !== null) {
            ...
            if (requiredDiff !== undefined && requiredDiff !== null && requiredDiff !== "" && !ge(localDiff, requiredDiff)) {
    ...
    function submitBlock(miner, job, blockTemplate, blockData, resultBuff, hashDiff, isTrustedShare, isParentBlock, portUsedToSubmit, submitBlockCB) {
        ...
        const replyFn = function (rpcResult, rpcStatus, port, nextSubmitBlockCB) {
            ...
            const getDiffMessage = function () {
                return getBlockSubmitDiffMessage(blockTemplate, blockData, hashDiff, requiredBlockDiff, job);
            };
            const blockDataStr = Buffer.isBuffer(blockData) ? blockData.toString("hex") : JSON.stringify(blockData);

            if (rpcResult && (rpcResult.error || rpcResult.result === false || rpcResult.result === "high-hash" || rpcResult.result === "bad-txnmrklroot" || rpcResult.result === "bad-cbtx-mnmerkleroot")) {
