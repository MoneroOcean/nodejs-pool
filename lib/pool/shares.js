...
const TRUSTED_QUEUE_KEY_MAX_RETAINED_BYTES = 16 * 1024 * 1024;
const TRUSTED_QUEUE_MAX_RETAINED_BYTES = 32 * 1024 * 1024;
...
/** @typedef {{run: (forceVerify: boolean, done: () => void) => void, cancel: () => void, retainedBytes: number}} TrustQueueEntry */
/** @typedef {{payout: string, inFlight: number, queue: TrustQueueEntry[], draining: boolean, forceVerify: boolean, queuedBytes: number}} TrustGate */
...
/** @typedef {(hashDiff: number|bigint, resultBuff: Buffer|null, blockData: ShareBlockData|null, isTrustedShare: boolean, isNeedCheckBlockDiff: boolean, forceBlockCandidate?: boolean) => void} ShareVerificationCallback */
 ...
 * @property {Record<string, number>} ipWhitelist
...
module.exports = function createShareProcessor(deps) {
    ...
    let trustedQueuedBytes = 0;
    /** @type {Record<string, number>} */
    const shareThrottleLogTimes = Object.create(null);
    ...
    function getTrustVerificationGate(trustKey, payout) {
        ...
        if (!gate) {
            gate = { payout, inFlight: 0, queue: [], draining: false, forceVerify: false, queuedBytes: 0 };
    ...
    /** @param {Record<string, unknown>} poolSettings @param {PoolSubmitParams} params @returns {number} */
    function getTrustedQueueRetainedBytes(poolSettings, params) {
        const getRetainedBytes = poolSettings["getTrustedQueueRetainedBytes"];
        if (typeof getRetainedBytes !== "function") return 0;
        const retainedBytes = getRetainedBytes(params);
        return Number.isSafeInteger(retainedBytes) && retainedBytes > 0 ? retainedBytes : 0;
    }

    /** @param {TrustGate} gate @param {TrustQueueEntry} entry @returns {void} */
    function releaseTrustedQueueEntry(gate, entry) {
        gate.queuedBytes = Math.max(0, gate.queuedBytes - entry.retainedBytes);
        trustedQueuedBytes = Math.max(0, trustedQueuedBytes - entry.retainedBytes);
        entry.retainedBytes = 0;
    }

    /** @param {string} reason @param {Record<string, unknown>} fields */
    function logShareThrottle(reason, fields) {
        const timeNow = Date.now();
        if (timeNow - (shareThrottleLogTimes[reason] || 0) < 30 * 1000) return;
        shareThrottleLogTimes[reason] = timeNow;
        console.warn(`${getThreadName()  }IMPORTANT: ${  formatPoolEvent("Share throttle", { reason, ...fields })}`);
    }
    ...
    function dropTrustedQueue(trustKey, gate) {
        ...
        for (const entry of gate.queue.splice(0)) {
            releaseTrustedQueueEntry(gate, entry);
            entry.cancel();
        }
    }
    ...
    function processShare(miner, job, blockTemplate, params, processShareCB) {
        ...
        if (minerWallet) {
            ...
            miner.whiteList = miner.ipAddress in ipWhitelist;
            if (!miner.whiteList && ++minerWallet.last_ver_shares > threshold) {
                logShareThrottle("wallet-rate", {
                    count: minerWallet.last_ver_shares,
                    limit: threshold,
                    proxy: Boolean(proxyMiners[miner.payout]),
                    wallet: global.support.maskWalletAddress(miner.payout),
                    whitelisted: false
                });
        ...
        function enqueueTrustedShare(resumeShareProcessing) {
            ...
            const queueLimit = getTrustedQueueLimit(miner.payout);
            const retainedBytes = getTrustedQueueRetainedBytes(poolSettings, params);
            if (gate.queue.length >= queueLimit ||
                gate.queuedBytes + retainedBytes > TRUSTED_QUEUE_KEY_MAX_RETAINED_BYTES ||
                trustedQueuedBytes + retainedBytes > TRUSTED_QUEUE_MAX_RETAINED_BYTES) {
                logShareThrottle("trusted-queue", {
                    inFlight: gate.inFlight,
                    limit: queueLimit,
                    proxy: Boolean(proxyMiners[miner.payout]),
                    queued: gate.queue.length,
                    queuedBytes: gate.queuedBytes,
                    retainedBytes,
                    wallet: global.support.maskWalletAddress(miner.payout),
                    whitelisted: miner.ipAddress in ipWhitelist
                });
                processSend({ type: "throttledShare" });
                processShareCB(null);
                return;
            ...
            gate.queuedBytes += retainedBytes;
            trustedQueuedBytes += retainedBytes;
            gate.queue.push({
        ...
        function runSpecialShareVerifier(verifyShareCB) {
            return typeof poolSettings.verifySpecialShare === "function" && poolSettings.verifySpecialShare({
                ...
                verifyShareCB: function specialVerifyShareCB(hashDiff, resultBuff, blockData, isTrustedShare, isNeedCheckBlockDiff, forceBlockCandidate) {
                    verifyShareCB(hashDiff, resultBuff, blockData, isTrustedShare, isNeedCheckBlockDiff, forceBlockCandidate);
                }
            });
        }
        ...
        /** @param {number|bigint} hashDiff @param {Buffer|null} resultBuff @param {ShareBlockData|null} blockData @param {boolean} isTrustedShare @param {boolean} isNeedCheckBlockDiff @param {boolean} [forceBlockCandidate] @returns {boolean} */
        function submitMainBlockIfNeeded(hashDiff, resultBuff, blockData, isTrustedShare, isNeedCheckBlockDiff, forceBlockCandidate) {
            const allowUntrustedBlockSubmitTest = isBlockSubmitTestBypassEnabled(miner) && !hasBlockSubmitTrust(miner);
            if (!isNeedCheckBlockDiff || (!forceBlockCandidate && !ge(hashDiff, blockTemplate.difficulty))) return false;
            const nextBlockData = getVerifiedBlockData(blockData);
            if (!nextBlockData) return true;
            if (forceBlockCandidate) {
        ...
        verifyShare(function (hashDiff, resultBuff, blockData, isTrustedShare, isNeedCheckBlockDiff, forceBlockCandidate) {
            if (submitMainBlockIfNeeded(hashDiff, resultBuff, blockData, isTrustedShare, isNeedCheckBlockDiff, forceBlockCandidate)) return;
    ...
    function resetShareState() {
        ...
        for (const key of Object.keys(shareThrottleLogTimes)) delete shareThrottleLogTimes[key];
