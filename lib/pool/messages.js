 ...
 * @property {Record<number, MinerPortCounts>} workerMinerCounts
...
/** @typedef {number[]|Record<string, number>} MinerPortCounts */
/** @typedef {{worker_id: number, ports: MinerPortCounts}} MinerPortData */
...
/** @typedef {{type: "workerIdentity", data: {clusterWorkerId: number, uniqueWorkerId: number}}} WorkerIdentityMessage */
/** @typedef {{type: "sendRemote", body: string}} RemoteMessage */
/** @typedef {ShareStatMessage|BanMessage|TemplateMessage|HashFactorMessage|MinerPortMessage|WorkerIdentityMessage|RemoteMessage} PoolMessage */
 ...
 * @property {(clusterWorkerId: number, uniqueWorkerId: number) => void} registerWorkerTemplate
...
function isMessageObject(value) {
    ...
    return typeof typeValue.value === "string";
}
...
function isTemplateData(value) {
    ...
    return typeof coin.value === "string";
}
...
function isHashFactorData(value) {
    ...
    if (!coin || !factor) return false;
    return typeof coin.value === "string" && typeof factor.value === "number";
}
...
function isMinerPortData(value) {
    ...
    if (!workerId || !ports) return false;
    return typeof workerId.value === "number" && isPortCounts(ports.value);
}

/** @param {unknown} value @returns {value is MinerPortCounts} */
function isPortCounts(value) {
    if (Array.isArray(value)) return value.every((count) => typeof count === "number" && Number.isFinite(count));
    if (!value || typeof value !== "object") return false;
    return Object.keys(value).every((key) => {
        const count = Object.getOwnPropertyDescriptor(value, key);
        if (!count) return false;
        return typeof count.value === "number" && Number.isFinite(count.value);
    });
}
...
function isPoolMessage(value) {
    ...
    case "workerIdentity":
        return value.data !== null && typeof value.data === "object" &&
            Number.isSafeInteger(Reflect.get(value.data, "clusterWorkerId")) && Number.isSafeInteger(Reflect.get(value.data, "uniqueWorkerId"));
    case "sendRemote":
...
module.exports = function createMessageHandler(deps) {
    ...
    return function messageHandler(message) {
        ...
        case "newCoinHashFactor":
            ...
            break;
        case "minerPortCount":
            if (cluster.isMaster) state.workerMinerCounts[message.data.worker_id] = message.data.ports;
            break;
        case "workerIdentity":
            if (cluster.isMaster) registerWorkerTemplate(message.data.clusterWorkerId, message.data.uniqueWorkerId);
            break;
        case "sendRemote":
            if (cluster.isMaster) getRemoteDatabase(global.database).sendQueue.push({ body: Buffer.from(message.body, "hex") });
            break;
        }
    };
};
