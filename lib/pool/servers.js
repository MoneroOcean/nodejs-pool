...
const LARGE_FRAME_MAX_STRUCTURE_TOKENS = 64;

/**
 * Bounding JSON punctuation before JSON.parse prevents wide or deeply nested
 * objects from expanding far beyond their wire size. String contents are
 * ignored so miner-specific extension values remain compatible.
 * @param {string} message
 * @returns {boolean}
 */
function hasBoundedLargeFrameStructure(message) {
    let escaped = false;
    let quoted = false;
    let tokens = 0;
    for (let offset = 0; offset < message.length; ++offset) {
        const byte = message.charCodeAt(offset);
        if (quoted) {
        ...
        if (byte === 44 || byte === 58 || byte === 91 || byte === 93 || byte === 123 || byte === 125) {
            if (++tokens > LARGE_FRAME_MAX_STRUCTURE_TOKENS) return false;
        }
    }
    return true;
}
...
/** Large-frame framing checks only the stable Pearl submit shape. The coin
 * profile owns extension and proof semantics after the complete JSON arrives.
 * @param {unknown} value
 */
function isPearlSubmitRequest(value) {
    ...
    return isValidRpcId(value["id"]) && validJobId && typeof proof === "string" && proof.length > 0;
}
        ...
        return /** @param {PoolSocket} socket */ function socketConn(socket) {
            ...
            socket.on("data", function onData(chunk) {
                ...
                for (const message of messages) {
                    ...
                    try {
                        if (largeFrame && !hasBoundedLargeFrameStructure(message)) throw new Error("large request is too complex");
...
module.exports.isPearlSubmitRequest = isPearlSubmitRequest;
module.exports.hasBoundedLargeFrameStructure = hasBoundedLargeFrameStructure;
module.exports.LARGE_FRAME_MAX_STRUCTURE_TOKENS = LARGE_FRAME_MAX_STRUCTURE_TOKENS;
