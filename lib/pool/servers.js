...
/** @typedef {import("./miner_registry").MinerSocket & {normalizedRemoteAddress?: string, subnet24?: string | null, lastSocketActivity?: number, protocolErrorCount?: number, debugMiner?: boolean, finalReplyTimer?: NodeJS.Timeout | null, eth_extranonce_id?: number, eth_extranonce_preview_id?: number}} PoolSocket */
...
const PEARL_PACKET_BYTES = 12 * 1024 * 1024;
const PEARL_BUFFERED_MAX_BYTES = 32 * 1024 * 1024;
...
function sanitizeRequestForLog(request) {
    if (!isRecord(request["params"]) || typeof request["params"]["plain_proof"] !== "string") return request;
    const proof = request["params"]["plain_proof"];
    return { ...request, params: { ...request["params"], plain_proof: `<redacted:${Buffer.byteLength(proof, "utf8")} bytes>` } };
}
...
module.exports = function createServerFactory(deps) {
    ...
    function createPoolSocketHandler(portData, options = {}) {
        ...
        return /** @param {PoolSocket} socket */ function socketConn(socket) {
            ...
            let pearlPacket = false;
            let accountedPearlBytes = 0;
            function releasePearlBuffer() {
                pearlBufferedBytes = Math.max(0, pearlBufferedBytes - accountedPearlBytes);
                accountedPearlBytes = 0;
            }
            /** @returns {boolean} */
            function accountPearlBuffer() {
                const additionalBytes = dataBufferBytes - accountedPearlBytes;
                if (additionalBytes <= 0) return true;
                if (additionalBytes > PEARL_BUFFERED_MAX_BYTES - pearlBufferedBytes) return false;
                pearlBufferedBytes += additionalBytes;
                accountedPearlBytes = dataBufferBytes;
                return true;
            }
            ...
            socket.on("data", function onData(chunk) {
                ...
                if (dataBufferBytes > STANDARD_PACKET_BYTES && !pearlPacket) {
                    const miner = socket.miner_id ? state.activeMiners.get(socket.miner_id) : undefined;
                    pearlPacket = hasPearlJob(miner) && isPearlSubmitPrefix(dataBuffer);
                }
                if (pearlPacket && !accountPearlBuffer()) {
                    dataBuffer = "";
                    dataBufferBytes = 0;
                    releasePearlBuffer();
                    socket.destroyReason = "pearl-buffer-limit";
                    socket.destroy();
                    return;
                }
                if (dataBufferBytes > (pearlPacket ? PEARL_PACKET_BYTES : STANDARD_PACKET_BYTES)) {
                    dataBuffer = "";
                    dataBufferBytes = 0;
                    releasePearlBuffer();
                ...
                if (dataBufferBytes > STANDARD_PACKET_BYTES) {
                    const miner = socket.miner_id ? state.activeMiners.get(socket.miner_id) : undefined;
                    pearlPacket = hasPearlJob(miner) && isPearlSubmitPrefix(dataBuffer);
                    if (!pearlPacket || dataBufferBytes > PEARL_PACKET_BYTES || !accountPearlBuffer()) {
                        dataBuffer = "";
                        dataBufferBytes = 0;
                        releasePearlBuffer();
                        socket.destroy();
