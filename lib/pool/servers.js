...
module.exports = function createServerFactory(deps) {
    ...
    const connectionReservations = new WeakMap();
    const pendingTlsReservationsByTuple = new Map();
    ...
    function getSocketTupleKey(socket) {
        ...
        const localAddress = normalizeRemoteAddress(socket.localAddress);
        if (!remoteAddress || !localAddress || !Number.isInteger(socket.remotePort) || !Number.isInteger(socket.localPort)) return null;
        return JSON.stringify([remoteAddress, socket.remotePort, localAddress, socket.localPort]);
    }

    function clearTlsHandshakeTimeout(reservation) {
        if (!reservation) return;
        if (reservation.handshakeTimer) {
            clearTimeout(reservation.handshakeTimer);
            reservation.handshakeTimer = null;
        }
    }

    function attachConnectionRelease(socket, reservation) {
        if (!socket || !reservation || reservation.boundSockets.has(socket)) return;
        reservation.boundSockets.add(socket);
        if (typeof socket.once !== "function") return;
        socket.once("close", reservation.release);
        socket.once("error", reservation.release);
    }

    function releaseConnectionReservation(reservation) {
        if (!reservation || reservation.released) return;
        reservation.released = true;
        clearTlsHandshakeTimeout(reservation);
        if (reservation.tlsTupleKey && pendingTlsReservationsByTuple.get(reservation.tlsTupleKey) === reservation) {
            pendingTlsReservationsByTuple.delete(reservation.tlsTupleKey);
        }
        decrementConnectionCount(state.activeConnectionsByIP, reservation.normalizedIp);
        decrementConnectionCount(state.activeConnectionsBySubnet, reservation.subnet);
    }

    function getConnectionReservation(socket) {
        if (!socket || typeof socket !== "object") return null;
        const directReservation = connectionReservations.get(socket);
        if (directReservation) return directReservation;

        const tupleKey = getSocketTupleKey(socket);
        if (!tupleKey) return null;
        const pendingReservation = pendingTlsReservationsByTuple.get(tupleKey);
        if (!pendingReservation) return null;
        pendingTlsReservationsByTuple.delete(tupleKey);
        connectionReservations.set(socket, pendingReservation);
        attachConnectionRelease(socket, pendingReservation);
        return pendingReservation;
    }

    function reserveConnection(socket, isTls) {
        const existingReservation = connectionReservations.get(socket);
        ...
        const reservation = {
            ...
            subnet,
            isTls,
            rawSocket: isTls ? socket : null,
            tlsTupleKey: isTls ? getSocketTupleKey(socket) : null,
            released: false,
            boundSockets: new WeakSet(),
            handshakeTimer: null,
            release: null
        };
        reservation.release = function release() {
            releaseConnectionReservation(reservation);
        };
        connectionReservations.set(socket, reservation);
        if (reservation.tlsTupleKey) pendingTlsReservationsByTuple.set(reservation.tlsTupleKey, reservation);
        attachConnectionRelease(socket, reservation);
        return reservation;
    }

    function armTlsHandshakeTimeout(socket, reservation) {
        if (!reservation || !reservation.isTls || reservation.released) return;
        const timeoutMs = getPoolSecurityConfig().tlsHandshakeTimeoutMs;
        const onHandshakeTimeout = function onHandshakeTimeout() {
            if (reservation.released) return;
            rejectConnection(socket, "tls-handshake-timeout");
            reservation.release();
        };
        reservation.handshakeTimer = setTimeout(onHandshakeTimeout, timeoutMs);
    }
    ...
    function createPoolSocketHandler(portData, options = {}) {
        ...
        function handleMessage(socket, jsonData, pushMessage) {
            if (typeof jsonData.id === "undefined" || jsonData.id === null || jsonData.id === "") {
        ...
        return function socketConn(socket) {
            const reservation = getConnectionReservation(socket) || (isTls ? null : reserveConnection(socket, false));
            if (!reservation) {
            ...
            socket.on("data", function onData(chunk) {
                ...
                else if ("eth_extranonce_preview_id" in socket) state.freeEthExtranonces.push(socket.eth_extranonce_preview_id);
            });
        };
    }
        ...
        return Promise.all(portList.map(function startPortServer(portData) {
            return new Promise(function createServer(resolve, reject) {
                ...
                if (portData.ssl === true) {
                    const onTlsConnection = function onTlsConnection(socket) {
                        const reservation = reserveConnection(socket, true);
                        if (!reservation) return;
                        armTlsHandshakeTimeout(socket, reservation);
                    };
                    if (typeof server.prependListener === "function") server.prependListener("connection", onTlsConnection);
                    else server.on("connection", onTlsConnection);
                    server.on("tlsClientError", function onTlsClientError(_error, socket) {
                        const reservation = getConnectionReservation(socket);
                        if (reservation) reservation.release();
                        if (socket && typeof socket.destroy === "function" && !socket.destroyed) socket.destroy();
                        const rawSocket = reservation && reservation.rawSocket;
                        if (rawSocket && rawSocket !== socket && typeof rawSocket.destroy === "function" && !rawSocket.destroyed) {
                            rawSocket.destroy();
                        }
                    });
                }
