...
module.exports = function createServerFactory(deps) {
    ...
    function createPoolSocketHandler(portData, options = {}) {
        ...
        function handleMessage(socket, jsonData, pushMessage) {
            const rpcId = jsonData && typeof jsonData === "object" ? jsonData.id : undefined;
            ...
            const rpcMethod = jsonData.method;
            ...
            const sendReply = function sendReply(error, result) {
                ...
                if (rpcId === "Stratum") reply.method = rpcMethod;
            ...
            if (debug.enabled) debug(`MINER: ${  formatPoolEvent("Request", { body: jsonData })}`);
            handleMinerData(socket, rpcId, rpcMethod, jsonData.params, socket.normalizedRemoteAddress, portData, sendReply, sendReplyFinal, pushMessage, jsonData);
