...
/** @typedef {import("./miner_registry").RegistryMiner & {touchSocketActivity?: (time: number) => void}} ConnectedMiner */
...
function isPearlSubmitPrefix(value) {
    ...
    return /^\s*\{/.test(prefix) && /"method"\s*:\s*"mining\.submit"/.test(prefix) && /"plain_proof"\s*:\s*"/.test(prefix);
}
    ...
    function createPoolSocketHandler(portData, options = {}) {
        ...
        return /** @param {PoolSocket} socket */ function socketConn(socket) {
            ...
            socket.on("data", function onData(chunk) {
                ...
                dataBuffer += chunk;
                const packetBytes = Buffer.byteLength(dataBuffer, "utf8");
                const packetLimit = portData.port === PEARL_PORT && isPearlSubmitPrefix(dataBuffer)
                    ? PEARL_PACKET_BYTES
                    : STANDARD_PACKET_BYTES;
