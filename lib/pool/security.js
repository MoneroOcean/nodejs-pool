...
function getPoolSecurityConfig() {
    ...
    // A TLS handshake is performed before the application-level socket auth
    // timer can be installed. Keep that pre-auth phase bounded even when a
    // deployment has configured an unusually long socketAuthTimeout.
    const tlsHandshakeTimeoutMs = Math.min(socketAuthTimeoutMs, 30 * 1000);
    return {
        socketAuthTimeoutMs,
