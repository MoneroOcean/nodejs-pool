...
module.exports = function createLifecycle(deps) {
    ...
    function scheduleWorkerReloads() {
        ...
        function dumpTrustAndAgentFiles() {
            ...
            // eslint-disable-next-line eqeqeq -- intentional loose compare: WORKER_ID env var is a string, matched against numeric 1
    ...
    function scheduleMasterDbSync(numWorkers) {
        ...
        function updatePoolRows() {
            if ("" in state.activeBlockTemplates) {
                ...
                const onPoolUpdateError = function onPoolUpdateError(error) {
    ...
    function daemonStatus(options) {
        ...
        if (key in state.lastBlockFixTime && now - state.lastBlockFixTime[key] < config.cooldownMs) return { status: "cooldown", since };
