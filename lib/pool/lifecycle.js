...
module.exports = function createLifecycle(deps) {
    ...
    function clearDaemonLagState(key) {
        delete state.lastBlockLagStartTime[key];
        delete state.lastBlockFixTime[key];
        state.lastBlockFixCount[key] = 0;
    }

    function stuckExit(port) {
    ...
    function recordDaemonFix(keys, now) {
        keys.forEach(function recordKey(key) {
            state.lastBlockFixTime[key] = now;
        });
    }
    ...
    function handleDaemonLag(options) {
        const { chain, port, height, topHeight, config, now } = options;
        const key = `${chain  }:${  port}`;
        if (!topHeight || height === null) {
            clearDaemonLagState(key);
            return "no-height";
        }

        const lag = topHeight - height;
        if (lag < config.lagBlocks) {
            clearDaemonLagState(key);
            return "healthy";
        }

        if (!(key in state.lastBlockLagStartTime)) {
        ...
        if (now - state.lastBlockLagStartTime[key] < config.graceMs) return "grace";
        if (key in state.lastBlockFixTime && now - state.lastBlockFixTime[key] < config.cooldownMs) return "cooldown";

        state.lastBlockFixCount[key] = key in state.lastBlockFixCount ? state.lastBlockFixCount[key] + 1 : 1;
        ...
        if (chain !== "xtm" && state.lastBlockFixCount[key] > 5 && global.config.general.allowStuckPoolKill && port == global.config.daemon.port) {
            stuckExit(port);
            return "exit";
        }
    ...
    function checkStuckTemplateHealth() {
        if (!("" in state.activeBlockTemplates)) return Promise.resolve(false);
        ...
        return global.mysql.query("SELECT blockID, xtmBlockID, port FROM pools WHERE last_checkin > date_sub(now(), interval 30 minute)").then(function onRows(rows) {
            ...
            let xmr = handleDaemonLag({ chain: "xmr", port, height: xmrHeight, topHeight: topXmrHeight, config, now });
            let xtm = handleDaemonLag({ chain: "xtm", port, height: xtmHeight, topHeight: topXtmHeight, config, now });
            ...
            return { xmr, xtm, topXmrHeight, topXtmHeight };
