    ...
    async function planBatches() {
        ...
        for (const row of rows) {
            ...
            if (isIntegratedAddress(row.payment_address)) {
                // Integrated addresses are paid separately and use the
                // integrated-address minimum payout threshold.
                if (grossAmount < threshold) continue;
                if (grossAmount < integratedAddressThreshold && (!customThreshold || grossAmount <= threshold)) continue;
