...
module.exports = function createPaymentsPlanner(ctx, common) {
    ...
    async function preflightWalletBalance(batch) {
        ...
        if (unlocked === null || unlocked < totalNet) {
            return {
                ok: false,
                reason: "wallet preflight insufficient balance" + balanceSnapshotText(balance, unlocked, totalNet)
            };
        }
        return { ok: true, walletBalance: balance, walletUnlocked: unlocked };
    }
    ...
    async function planBatches() {
        ...
        for (const row of rows) {
            // The refactor still mirrors the old address-only payout behavior.
            // Standalone payment-id destinations were previously unreachable in
            // practice, so they stay out of automated planning here as well.
            if (row.payment_id !== null && typeof row.payment_id !== "undefined") continue;
            if (normalizePaymentId(row.payment_id) !== null) continue;
            const recipientKey = row.payment_address;
            // One payout candidate per address per cycle keeps the planner close
            // to the old module and prevents multiple balance rows for the same
            // destination from generating duplicate sends in one pass.
            ...
            if (row.pool_type === "fees" && row.payment_address === config.payout.feeAddress) {
                ...
                const exchangeMin = payoutAtomic("exchangeMin");
                // The pool's own fee wallet keeps a reserve for paying future
                // on-chain transaction fees instead of draining itself to zero.
                grossAmount = grossAmount >= feesForTxn + exchangeMin ? grossAmount - feesForTxn : 0;
            }
            const remainder = denomAtomic() ? grossAmount % denomAtomic() : 0;
            // Payouts are rounded down to the configured denomination before
            // threshold checks so planning matches the amount that would really
            ...
            if (isIntegratedAddress(row.payment_address)) {
                // Integrated/exchange addresses are always sent one-per-tx. They
                // must satisfy the user's own threshold first, then also satisfy
                // the exchange minimum unless the user explicitly raised the bar
                // above that minimum.
                if (grossAmount < threshold) continue;
                if (grossAmount < payoutAtomic("exchangeMin") && (!customThreshold || grossAmount <= threshold)) continue;
                integratedBatches.push({
                    batchType: "integrated",
                    items: [item],
