...
module.exports = function createPaymentsProcessor(ctx, state, deps) {
    ...
    function shouldSendRetryFyi(options) {
        ...
        if (preflight && preflight.code === "wallet_preflight_insufficient_balance" && preflight.pendingBalanceEnough) return false;
        return true;
    }
    ...
    async function resolveSubmittedTxKey(batch, items, txHash, totalFee, transferReply) {
        const txKey = typeof transferReply.result.tx_key === "string" && transferReply.result.tx_key ? transferReply.result.tx_key : null;
