...
/** @typedef {PaymentBatch & {items: PaymentItem[]}} ReservableBatch */
...
/** @typedef {{isFailStop: boolean, lockConnection: SqlConnection|null, activeBatchId: number|string|null}} PaymentState */
/** @typedef {{common: ReturnType<typeof import("./common.js")>, planner: ReturnType<typeof import("./planner.js")>, enterFailStop: (scope: string, message: string, batchId?: number|string) => Promise<boolean>, verifyExclusiveAccess?: () => Promise<boolean>}} ProcessorDeps */
/** @typedef {{executor?: SqlPool|SqlConnection, expectedStatuses?: string[], requireNullFields?: string[]}} StatusUpdateOptions */
/** @typedef {{preflight?: PreflightResult}} RetryOptions */
/** @typedef {{txHash?: string|null, txKey?: string|null, totalFee?: number|null}} MatchDetails */
/** @typedef {{status: "retry", message: string}|{status: "missing", message: string}|{status: "ok", txKey: string}} TxKeyResult */
/** @typedef {{status: "wallet_unavailable", message: string}|{status: "tx_not_found"}|{status: "tx_lookup_unavailable", message: string}|{status: "ok", transfers: WalletTransfer[]}} WalletLoadResult */
/** @typedef {{status: "multiple_matches"|"no_match"|"incomplete_match"}|{status: "wallet_unavailable", message: string}|{status: "matched_waiting_tx_key", message?: string, totalFee: number, txHash: string}|{status: "matched_missing_tx_key", totalFee: number, txHash: string}|{status: "matched", totalFee: number, txHash: string, txKey: string}} WalletMatchResult */
/** @typedef {{status: "visible"}|{status: "wallet_unavailable", message: string}|{status: "tx_not_found"}} VisibilityResult */
/** @typedef {"cleared"|"unresolved"|"manual_review"} PaymentOutcome */
 ...
 * @param {ProcessorDeps} deps
 * @returns {{recoverPendingBatches: (source: string) => Promise<boolean>, submitReservedBatch: (batch: ReservableBatch) => Promise<PaymentOutcome>}}
 */
module.exports = function createPaymentsProcessor(ctx, state, deps) {
    ...
    /** @param {unknown} totalFee @returns {boolean} */
    function hasPlausibleWalletFee(totalFee) {
        const fee = normalizeInteger(totalFee);
        return fee !== null && fee > MIN_SUCCESS_FEE_ATOMIC;
    }
    ...
    /** @param {PaymentBatch} batch @param {StatusUpdateOptions} [options] @returns {StatusUpdateOptions} */
    ...
    /** @param {PaymentBatch} batch @param {Record<string, SqlParam>} fields @param {StatusUpdateOptions} [options] @returns {Promise<boolean>} */
    ...
    /** @param {SqlConnection} connection @param {PaymentBatch} batch @param {Record<string, SqlParam>} fields @param {StatusUpdateOptions} [options] @returns {Promise<boolean>} */
    ...
    /** @param {PaymentBatch|null|undefined} batch @returns {PaymentOutcome} */
    ...
    /** @param {PaymentBatch} batch @returns {number} */
    ...
    /** @param {PaymentBatch} batch @returns {string} */
    ...
    /** @param {RetryOptions} [options] @returns {boolean} */
    function shouldSendRetryFyi(options) {
        const preflight = options && typeof options === "object" && options.preflight && typeof options.preflight === "object" ? options.preflight : null;
        if (preflight && preflight.ok === false && preflight.code === "wallet_preflight_insufficient_balance" && preflight.pendingBalanceEnough) return false;
        return true;
    }
    ...
    /** @template T @param {() => Promise<T>} search @param {(result: T) => boolean} shouldRetry @returns {Promise<T>} */
    async function retryPostSubmitSearch(search, shouldRetry) {
        const [firstDelay, ...remainingDelays] = SUBMIT_SEARCH_DELAYS_MS;
        if (typeof firstDelay !== "number") throw new Error("submit search retry schedule is empty");
        await sleepMs(firstDelay);
        let result = await search();
        if (!shouldRetry(result)) return result;
        for (const delayMs of remainingDelays) {
    ...
    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @param {string} message @returns {Promise<void>} */
    ...
    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @param {string} message @param {string} [warningMessage] @param {Record<string, SqlParam>} [extraFields] @returns {Promise<PaymentOutcome>} */
    ...
    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @param {string} message @param {string} [warningMessage] @param {RetryOptions} [options] @returns {Promise<PaymentOutcome>} */
    async function holdBatchForRetry(batch, items, message, warningMessage, options) {
        ...
        logWarn(`batch#${  batch.id}`, warningMessage || message);
        if (shouldSendRetryFyi(options)) await sendPendingBatchFyi(batch, items, message);
        return "unresolved";
    }

    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @param {string} message @param {string} warningMessage @param {boolean} incrementHealthyPass @param {Record<string, SqlParam>} [extraFields] @returns {Promise<PaymentOutcome>} */
    ...
    /** @param {WalletMatchResult} matchResult @returns {MatchDetails} */
    function matchResultDetails(matchResult) {
        /** @type {MatchDetails} */
        const details = {};
        if ("txHash" in matchResult && matchResult.txHash) details.txHash = matchResult.txHash;
        if ("txKey" in matchResult && typeof matchResult.txKey === "string") details.txKey = matchResult.txKey;
        if ("totalFee" in matchResult && normalizeInteger(matchResult.totalFee) !== null) details.totalFee = matchResult.totalFee;
        return details;
    }

    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @param {string} message @param {MatchDetails} [details] @returns {Promise<PaymentOutcome>} */
    async function markBatchManualReview(batch, items, message, details) {
        /** @type {Record<string, SqlParam>} */
        const fields = {
    ...
    /** @param {string} txHash @returns {Promise<TxKeyResult>} */
    async function resolveTxKey(txHash) {
        ...
        if (hasRpcError(reply)) {
            return { status: "missing", message: describeWalletReply(reply) };
        }
        const result = resultObject(reply);
        const txKey = result && typeof result["tx_key"] === "string" ? result["tx_key"] : "";
        return txKey ? { status: "ok", txKey } : { status: "missing", message: `wallet returned no tx_key for ${  txHash}` };
    }

    /** @param {{filterByHeight?: boolean}} [options] @returns {Promise<WalletLoadResult>} */
    async function loadWalletTransfers(options) {
        ...
        if (!options || options.filterByHeight !== false) {
            ...
            const heightResult = resultObject(heightReply);
            const walletHeight = heightResult ? normalizeInteger(heightResult["height"]) : null;
            if (heightReply instanceof Error || typeof heightReply === "string" || !heightResult || walletHeight === null) {
        ...
        const result = resultObject(reply);
        if (reply instanceof Error || typeof reply === "string" || !result) {
        ...
        for (const key of ["out", "pending", "pool"]) {
            ...
            if (Array.isArray(values)) {
                for (const value of values) if (isRecord(value)) transfers.push(value);
            }
        }
        return { status: "ok", transfers };
    }

    /** @param {string} txHash @returns {Promise<WalletLoadResult>} */
    async function loadWalletTransferByTxHash(txHash) {
        ...
        if (hasRpcError(reply)) {
            ...
            if (isRecord(error) && normalizeInteger(error["code"]) === 0 && error["message"] === "") {
        ...
        /** @type {WalletTransfer[]} */
        const transfers = [];
        const transferValues = result["transfers"];
        if (Array.isArray(transferValues)) {
            for (const value of transferValues) if (isRecord(value)) transfers.push(value);
        } else if (isRecord(result["transfer"])) {
    ...
    /** @param {number|string} batchId @returns {Promise<PaymentBatch|null>} */
    async function loadBatchById(batchId) {
        ...
        return rows.length && rows[0] ? rows[0] : null;
    }

    /** @param {PaymentBatch} batch @param {string} expectedStatus @returns {Promise<boolean>} */
    ...
    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @param {string} expectedStatus @returns {Promise<PaymentOutcome>} */
    async function handleLostSubmitClaim(batch, items, expectedStatus) {
        ...
        // Any other status (notably submitting/submitted) means another runtime
        // already owns this batch. Standing down avoids racing its reconcile path
        // and potentially rewriting finalized state back to submitted.
        return "unresolved";
    }

    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @returns {Promise<WalletMatchResult>} */
    async function findWalletTransferMatch(batch, items) {
        const loadedTransfers = await loadWalletTransfers();
        if (loadedTransfers.status !== "ok") {
            const message = "message" in loadedTransfers && typeof loadedTransfers.message === "string"
                ? loadedTransfers.message
                : "wallet transfer lookup returned no data";
            return { status: "wallet_unavailable", message };
        }
        ...
        if (!txHash || totalFee === null || !hasPlausibleWalletFee(totalFee)) return { status: "incomplete_match" };
    ...
    /** @param {WalletTransfer} transfer @returns {boolean} */
    ...
    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @returns {Promise<VisibilityResult>} */
    async function findSubmittedTransferVisibility(batch, items) {
        const txHash = normalizeHash(batch.tx_hash);
        if (!txHash) return { status: "tx_not_found" };
        const loadedTransfers = await loadSubmittedTransfersByTxHash(txHash);
        if (loadedTransfers.status !== "ok") {
            return loadedTransfers.status === "tx_not_found"
                ? { status: "tx_not_found" }
                : { status: "wallet_unavailable", message: loadedTransfers.message || "wallet transfer lookup returned no data" };
        }
    ...
    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @param {string} reason @param {{txHash: string, txKey: string, totalFee: number}} matchResult @param {string} lastErrorText @returns {Promise<PaymentOutcome>} */
    ...
    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @param {string} errorMessage @returns {Promise<PaymentOutcome>} */
    ...
    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @returns {Promise<void>} */
    async function sendBatchNotifications(batch, items) {
        for (const item of items) {
            const addressValue = pickValue(item, "paymentAddress", "payment_address");
            const username = typeof addressValue === "string" ? addressValue : "";
            ...
            try {
                ...
                const proofUrl = txHash && txKey ? makeProofUrl(txHash, username, txKey) : "";
                const emailData = {
                    pool: poolEmailBrand(),
                    status: "confirmed",
                    coin: coinCode(),
    ...
    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @param {string} reason @returns {Promise<boolean>} */
    async function finalizeBatch(batch, items, reason) {
        let txKey = batch.tx_key || null;
        const submittedHash = typeof batch.tx_hash === "string" ? batch.tx_hash : null;
        if (!submittedHash) {
            await markBatchManualReview(batch, items, `wallet send for batch ${  batch.id  } has no transaction hash`, {});
            return false;
        }
        if (!txKey) {
            const txKeyResult = await resolveTxKey(submittedHash);
            if (txKeyResult.status === "retry") {
                await holdBatch(
                    batch,
                    items,
            ...
            if (txKeyResult.status === "missing") {
                /** @type {MatchDetails} */
                const details = { txHash: submittedHash };
                const totalFee = normalizeInteger(batch.total_fee);
                if (totalFee !== null) details.totalFee = totalFee;
        ...
        try {
            ...
            await withTransaction(async function finalizeTransaction(connection) {
                ...
                const transactionRecord = {
                    address: batchType === "bulk" ? null : (items[0] ? pickValue(items[0], "paymentAddress", "payment_address") : null),
                    payment_id: null,
                ...
                if (typeof transactionId !== "number") throw new Error(`transactions insert returned no id for batch ${  batch.id}`);
                ...
                batch.transaction_id = transactionId;
            });
    ...
    /** @param {PaymentBatch} batch @param {string} reason @returns {Promise<boolean>} */
    ...
    /** @param {PaymentBatch} batch @param {string} txHash @param {string|null} txKey @param {number} totalFee @param {string|null} lastErrorText @returns {Promise<boolean>} */
    ...
    /** @param {PaymentBatch} batch @param {string} message @param {boolean} incrementHealthyPass @param {Record<string, SqlParam>} [extraFields] @returns {Promise<boolean>} */
    async function recordBatchReconcileState(batch, message, incrementHealthyPass, extraFields) {
        ...
        const cleanPasses = normalizeInteger(batch.reconcile_clean_passes) || 0;
        const healthyPasses = incrementHealthyPass ? cleanPasses + 1 : cleanPasses;
        const fields = {
    ...
    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @param {string} message @returns {Promise<PaymentOutcome>} */
    ...
    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @param {string} reason @param {boolean} [isPostSubmit] @returns {Promise<PaymentOutcome>} */
    async function reconcileSubmittedBatch(batch, items, reason, isPostSubmit) {
        if (!hasPlausibleWalletFee(batch.total_fee)) {
            /** @type {MatchDetails} */
            const details = {};
            if (typeof batch.tx_hash === "string") details.txHash = batch.tx_hash;
            const totalFee = normalizeInteger(batch.total_fee);
            if (totalFee !== null) details.totalFee = totalFee;
            return await markBatchManualReview(batch, items, `submitted batch ${  batch.id  } has an implausible wallet fee`, details);
        }
    ...
    async function reconcileBatch(batch, items, reason, isPostSubmit) {
        ...
        if (matchResult.status !== "matched") {
            return await markBatchManualReview(
                batch,
                items,
                `wallet reconcile returned unexpected match status ${  matchResult.status}`,
                matchResultDetails(matchResult)
            );
        }
    ...
    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @param {string} txHash @param {number} totalFee @param {UnknownRecord} transferReply @returns {Promise<{ok: true, txKey: string|null}|{ok: false, outcome: PaymentOutcome}>} */
    async function resolveSubmittedTxKey(batch, items, txHash, totalFee, transferReply) {
        const txKey = typeof transferReply["tx_key"] === "string" && transferReply["tx_key"] ? transferReply["tx_key"] : null;
    ...
    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @param {string} txHash @returns {Promise<PaymentOutcome|null>} */
    ...
    /** @param {PaymentBatch} batch @param {PaymentItem[]} items @param {RpcResponse} transferReply @returns {Promise<PaymentOutcome|null>} */
    async function handleTransferReplyError(batch, items, transferReply) {
        if (hasRpcError(transferReply)) {
        ...
        if (transferReply instanceof Error || typeof transferReply === "string" || !resultObject(transferReply)) {
    ...
    async function recoverPendingBatches(source) {
        ...
        for (const batch of batches) {
            ...
            if (batch.status === "retrying") {
                ...
                const retryBatch = Object.assign(batch, { items });
                const retryOutcome = await submitReservedBatch(retryBatch);
    ...
    /** @param {ReservableBatch} batch @returns {Promise<PaymentOutcome>} */
    async function submitReservedBatch(batch) {
        ...
        const preflight = await planner.preflightWalletBalance(totalNet === null ? {} : { totalNet });
        if (!preflight.ok) {
        ...
        const transferResult = resultObject(transferReply);
        if (!transferResult) return await reconcileAmbiguousSubmit(batch, items, "ambiguous wallet submit: missing result");
        const txHash = normalizeHash(transferResult["tx_hash"]);
        const totalFee = normalizeInteger(transferResult["fee"]);
        if (!txHash || totalFee === null || !hasPlausibleWalletFee(totalFee)) {
            return await reconcileAmbiguousSubmit(batch, items, "wallet returned implausible transfer success payload");
        }

        const txKeyResult = await resolveSubmittedTxKey(batch, items, txHash, totalFee, transferResult);
