...
module.exports = function createPaymentsProcessor(ctx, state, deps) {
    ...
    async function reconcileRejectedSubmit(batch, items, errorMessage) {
        ...
        const matchResult = await findWalletTransferMatch(batch, items);
    ...
    async function reconcileAmbiguousSubmit(batch, items, message) {
        ...
        return await reconcileBatch(batch, items, "same-process submit") === "cleared" ? "cleared" : "unresolved";
    }

    async function reconcileBatch(batch, items, reason) {
        ...
        if (batch.status === "submitted" && batch.tx_hash) {
            ...
            const submittedVisibility = await findSubmittedTransferVisibility(batch, items);
