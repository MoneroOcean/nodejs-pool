...
module.exports = function createPaymentsCommon(ctx) {
    ...
    function balanceSnapshotText(balance, unlocked, requiredNet) {
        const fields = [];
    ...
    function transferMatchesBatch(batch, items, transfer) {
        ...
        // We only auto-reconcile address-only payouts. If the wallet reports a
        // non-zero payment_id we force operator review instead of guessing that
        // it belongs to this refactored path.
        if (normalizePaymentId(transfer.payment_id) !== null) return false;
    ...
    function logBatchBlock(batch, items, phase, context) {
        ...
        const balanceSnapshot = phase === "submit" && context
            ? balanceSnapshotText(context.walletBalance, context.walletUnlocked, totalNet)
            : "";
        logInfo(
            "batch#" + batch.id,
    ...
    async function callWallet(method, params, suppressErrorLog) {
        return await new Promise(function resolveWalletCall(resolve) {
            support.rpcWallet(method, params, resolve, suppressErrorLog);
        });
    }
