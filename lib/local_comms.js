...
function Database(){
    ...
    this.initEnv = function(){
        ...
        global.database.rewardDB = this.env.openDbi({
            name: 'rewards',
            create: true,
            integerKey: true,
            keyIsUint32: true
...
+    this.setReward = function(cacheKey, cacheData){
        debug("Setting Key: "+cacheKey+ " Data: " + JSON.stringify(cacheData));
        this.refreshEnv();
        let txn = this.env.beginTxn();
        txn.putString(this.rewardDB, cacheKey, JSON.stringify(cacheData));
        txn.commit();
    };

    this.getReward = function(address, limit, offset){
        this.refreshEnv();
        let blockList = [];
        try {
            let txn = this.env.beginTxn({readOnly: true});
            let cursor = new this.lmdb.Cursor(txn, this.rewardDB);
            let count = 0; 
            let foundData = 0;
            for (let found = cursor.goToLast(); found; found = cursor.goToPrev()) {
                    cursor.getCurrentStringUnsafe(function(key, data){
                        data = JSON.parse(data);
                        if (data.miners[address] > 0 ){
                            foundData++
                            if (foundData > offset){
                                if (count <= limit) {
                                    let blockData = {};
                                    blockData.ts = data.ts/1000;
                                    blockData.amount = data.miners[address];
                                    blockData.blockDiff = data.difficulty;
                                    blockData.blockValue = data.value;
                                    blockData.id = key;
                                    blockList.push(blockData);
                                    count++
                                }
    ...
    this.cleanRewardDB = function() {
        if (cleanRewardInProgress) {
            console.error("CleanRewardDB already running");
            ++cleanRewardStuckCount;
            if (cleanRewardStuckCount > 5) global.support.sendEmail(global.config.general.adminEmail,"LongRunner stuck",cleanRewardStuckCount);
        return; // already running
        }
        cleanRewardInProgress = true;
        this.refreshEnv();
        let txn = this.env.beginTxn();
        let cursor = new this.lmdb.Cursor(txn, this.rewardDB);
        let i = 0;
