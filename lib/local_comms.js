...
function Database(){
    ...
    this.storeBulkShares = function(shareObject) {
        ...
        const slotUpdates = recentShareWindow.buildSlotUpdates(shareObject, {
            defaultPort: global.config.daemon.port,
            pplnsPoolType: global.protos.POOLTYPE.PPLNS
        });
        ...
        shareObject.forEach(function(share){
            ...
            if (typeof(share.raw_shares) === "number") {
                ...
                if (port_suffix === "") {
                    ...
                    port_suffix = "_" + defaultStatsPort.toString();
                }
                if (port_suffix !== "") {
            ...
            if(shareObject.length === shareCount){
                ...
                slotUpdates.forEach(function (update, slotId) {
                    const slotKey = recentShareWindow.getSlotCacheKey(slotId);
                    const cachedSlot = txn.getString(global.database.cacheDB, slotKey);
                    let cachedSlotData = null;
                    if (cachedSlot !== null) {
                        try {
                            cachedSlotData = JSON.parse(cachedSlot);
                        } catch (_error) {}
                    }
                    const slotPayload = recentShareWindow.getWritableSlotPayload(
                        cachedSlotData,
                        slotId
                    );
                    recentShareWindow.mergeSlotPayload(slotPayload, update);
                    txn.putString(global.database.cacheDB, slotKey, JSON.stringify(slotPayload));
                });
                let blocksSeen = 0;
    ...
    this.storeAltBlock = function(blockId, blockData, callback){
        try{
            ...
            global.coinFuncs.getPortBlockHeaderByHash(blockDataDecoded.port, blockDataDecoded.hash, function(err, header){
                ...
                if (unlockConfirmationDepth && header.depth < unlockConfirmationDepth) {
    ...
    this.bulkSetCache = function(cacheUpdates){
        let txn = this.env.beginTxn();
        cacheUpdateMarker.acquireCacheUpdateMarker(txn, this.cacheDB, 'cacheUpdate');
        txn.commit();
        //let size = 0;
        ...
        cacheUpdateMarker.releaseCacheUpdateMarker(txn, this.cacheDB, 'cacheUpdate');
        txn.commit();
