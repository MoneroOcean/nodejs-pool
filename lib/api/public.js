...
module.exports = function registerPublicRoutes(ctx) {
    ...
    function getPoolHashHistory(key) {
        ...
        const hashHistory = cachedData && typeof cachedData === "object" ? cachedData.hashHistory : null;
    ...
    function getWorkerStatsEntry(baseKey, identifier) {
        ...
        return {
            ...
            lastShareAlgo: cachedStats ? cachedStats.lastShareAlgo || false : false,
    ...
    async function getRecentPayments(limit, page) {
        ...
        return sortMapped(rows, function build(row) {
            return Object.assign(mapTransactionRow(row), { pool_type: poolTypes[row.id] || "?" });
        });
    }
    ...
    const routes = [
        ...
        cachedRoute("/pool/blocks/:pool_type", 10 * 1000, "pool blocks",
            pagedCacheKey("pool-blocks", 15, undefined, function poolBlockParts(req) { return [req.params.pool_type]; }),
        ...
        cachedRoute("/pool/altblocks/:pool_type", 10 * 1000, "pool altblocks",
            pagedCacheKey("pool-altblocks", 15, undefined, function poolAltBlockParts(req) { return [req.params.pool_type]; }),
        ...
        cachedRoute("/pool/blocks", 10 * 1000, "pool blocks all", pagedCacheKey("pool-blocks-all", 15),
            withPagination(15, undefined, function poolBlocksAllRoute(req, pagination) {
                const bounds = pageBounds(pagination);
        ...
        cachedRoute("/pool/altblocks", 10 * 1000, "pool altblocks all", pagedCacheKey("pool-altblocks-all", 15),
            withPagination(15, undefined, function poolAltBlocksAllRoute(req, pagination) {
                const bounds = pageBounds(pagination);
        ...
        cachedRoute("/pool/payments/:pool_type", 60 * 1000, "pool payments",
            pagedCacheKey("pool-payments", 15, undefined, function poolPaymentParts(req) { return [req.params.pool_type]; }),
        ...
        cachedRoute("/pool/payments", 60 * 1000, "pool payments all", pagedCacheKey("pool-payments-all", 15),
            withPagination(15, undefined, function poolPaymentsAllRoute(req, pagination) {
        ...
        cachedRoute("/miner/:address/payments", 60 * 1000, "miner payments", pagedCacheKey("miner-payments", 15, undefined, function minerPaymentParts(req) { return [req.params.address]; }),
            withPagination(15, undefined, function minerPaymentsRoute(req, pagination) { return getMinerPayments(req.params.address, pagination.limit, pagination.page); }),
            { error: "Issue getting miner payments" }
        ),
        cachedRoute("/miner/:address/block_payments", 60 * 1000, "miner block payments", pagedCacheKey("miner-block-payments", 15, 100, function minerBlockPaymentParts(req) { return [req.params.address]; }),
            withPagination(15, 100, function minerBlockPaymentsRoute(req, pagination) { return getMinerBlockPayments(req.params.address, pagination.limit, pagination.page); }),
