...
function getPagination(query, defaultLimit, maxLimit) {
    return { limit: normalizeInteger(query.limit, defaultLimit, 0, maxLimit), page: normalizeInteger(query.page, 0, 0) };
}
...
function safeDecodeAddress(cnUtil, address) {
    try { return cnUtil.address_decode(Buffer.from(String(address || ""))); } catch (_error) { return null; }
}
...
module.exports = function registerPublicRoutes(ctx) {
    ...
    async function getPoolPayments(poolType, limit, page) {
        const rows = await query(
            "SELECT DISTINCT(transaction_id) as txnID FROM payments WHERE pool_type = ? ORDER BY transaction_id DESC LIMIT ? OFFSET ?",
            [poolType, limit, page * limit]
        );
        if (rows.length === 0) return [];

        const paymentIds = rows.map(function map(row) { return row.txnID; });
        const txnRows = await query(
            "SELECT * FROM transactions WHERE id IN (" + createPlaceholders(paymentIds.length) + ") ORDER BY id DESC",
            paymentIds
        );

        return sortMapped(txnRows, function build(txnRow) {
            return {
                id: txnRow.id,
    ...
    const addressBase58Prefix = safeDecodeAddress(cnUtil, config && config.pool ? config.pool.address : "");
    const routes = [
        ...
        cachedRoute("/pool/address_type/:address", 10 * 1000, "pool address type",
            ...
            function addressTypeRoute(req) {
                const decoded = safeDecodeAddress(cnUtil, req.params.address);
        ...
        cachedRoute("/pool/payments/:pool_type", 60 * 1000, "pool payments",
            ...
            withPagination(10, undefined, function poolPaymentsRoute(req, pagination) {
                return req.params.pool_type === "pplns" ? getPoolPayments(req.params.pool_type, pagination.limit, pagination.page) : INVALID_POOL_TYPE;
            }),
