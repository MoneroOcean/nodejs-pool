...
function sendEmailReal(toAddress, subject, email_body, retry) {
    ...
    makeHttpRequest(mailgunURL, {
        ...
        if (!err && response.statusCode === 200) {
            ...
            if (retry) {
                console.error("Did not send e-mail to '" + recipient + "' successfully!  Response: " + responseBody + " Response: "+JSON.stringify(response));
            } else {
...
function sendEmail(toAddress, subject, body, wallet){
    ...
    if (isAdminEmail) {
        ...
        subject = formatEmailSubject(subject, "miner");
        body = appendUnsubscribeFooter(body, wallet, toAddress);
    }
