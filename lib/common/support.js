...
function sendEmail(toAddress, subject, body, wallet, options){
    ...
    if (isAdminEmail && !subject.includes("FYI")) {
        ...
        if (!(key in emailAcc)) {
            emailAcc[key] = body;
            scheduleBatchedEmail(key, toAddress, subject, batchKey || subject, wallet, isAdminEmail);
        } else {
            emailAcc[key] += "\n\n" + body;
        }
    }
...
function scheduleBatchedEmail(key, toAddress, subject, batchKey, wallet, isAdminEmail) {
    ...
    setTimeout(function(email_address, email_subject, email_batch_key, wallet) {
        ...
        if (!isAdminEmail) email_body = appendUnsubscribeFooter(email_body, wallet, email_address);
        let emailData = { wallet: wallet };
...
module.exports = function () {
    return {
        ...
        sendFyi: sendFyi,
        clearFyi: clearFyi,
        sendAdminFyi: sendAdminFyi,
        sendFyiDaily: sendFyiDaily,
