...
/** @param {number} port @returns {string|undefined} */
function daemonBasicAuth(port) {
    const daemon = global.config && global.config.daemon;
    if (!daemon) return undefined;
    const scoped = daemon[`basicAuth_${port}`];
    if (typeof scoped === "string" && scoped.length > 0) return scoped;
    return typeof daemon.basicAuth === "string" && daemon.basicAuth.length > 0 ? daemon.basicAuth : undefined;
}

/** @param {HttpRequestOptions} options @returns {{method: string|undefined, timeout: number|undefined, headers: Record<string, string|number>}} */
function jsonRequestDebugOptions(options) {
    /** @type {Record<string, string|number>} */
    const safeHeaders = {};
    ["Content-Type", "Accept", "Connection", "Content-Length"].forEach(function (name) {
...
function jsonRequest(host, port, data, callback, path, timeout, optionsOrSuppressErrorLog) {
    ...
    const basicAuth = daemonBasicAuth(port);
    if (basicAuth) options.headers["Authorization"] = basicAuth;
    ...
    debug(`JSON REQUST: ${  JSON.stringify(jsonRequestDebugOptions(options))}`);
