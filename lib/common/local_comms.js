...
function Database(){
	...
	if (cleanShareInProgress) {
	    ...
	    if (cleanShareStuckCount > 5) global.support.sendEmail(global.config.general.adminEmail,"long_runner stuck",cleanShareStuckCount);
        ...
        function loadCleanupWindow(oldestLockedBlockDifficulty) {
            global.coinFuncs.getLastBlockHeader(function(err, body){
                ...
                if (body.height - oldestLockedBlockHeight > global.config.general.blockCleanWarning) {
                    global.support.sendEmail(global.config.general.adminEmail, "long_runner module can not clean DB good enough", "long_runner can not clean " + (body.height - oldestLockedBlockHeight) + " block from DB!");
                }
