...
function Database(){
    ...
    this.initEnv = function(){
        ...
        global.database.shareDB = self.shareDB;
        global.database.blockDB = self.blockDB;
        global.database.altblockDB = self.altblockDB;
        global.database.cacheDB = self.cacheDB;
    };
    ...
    this.cleanShareDB = function(done) {
        ...
        function finalizeCleanup(data) {
            if (global.config.general["blockCleaner"] === true){
                if(data.length > 0){
                    ...
                    try {
                        ...
                        data.forEach(function(block){
                            ...
                            txn.del(global.database.shareDB, parseInt(block, 10));
                ...
                if (typeof global.database.env.sync === "function") global.database.env.sync(function(){});
            } else {
            ...
            global.coinFuncs.getLastBlockHeader(/** @param {unknown} err @param {BlockHeader | undefined} body */ function(err, body){
                ...
                try {
                    ...
                    if (blockDepth > Number(global.config.general["blockCleanWarning"])) {
                        const values = { blocks: blockDepth };
                        global.support.sendEmail(
                            global.config.general.adminEmail,
