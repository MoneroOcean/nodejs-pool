...
function Database(){
    ...
    this.incrementCacheData = function(key, data){
        withWriteTxn(function(txn){
            ...
            data.forEach(function(intDict){
                if (!Object.hasOwn(cached, intDict.location) || intDict.value === false){
    ...
    this.invalidateBlock = function(blockId){
        withWriteTxn(function(txn){
            const blockData = global.protos.Block.decode(txn.getBinary(self.blockDB, blockId));
    ...
    this.invalidateAltBlock = function(blockId){
        withWriteTxn(function(txn){
            const blockData = global.protos.AltBlock.decode(txn.getBinary(self.altblockDB, blockId));
            blockData.valid = false;
            blockData.unlocked = true;
    ...
    this.moveAltBlockReward = function(srcBlockId, dstBlockId, srcAmount){
        withWriteTxn(function(txn){
            const srcBlockData = global.protos.AltBlock.decode(txn.getBinary(self.altblockDB, srcBlockId));
            const dstBlockData = global.protos.AltBlock.decode(txn.getBinary(self.altblockDB, dstBlockId));
    ...
    this.getCache = function(cacheKey){
        ...
        try {
            return withReadTxn(function(txn) {
                const cached = txn.getString(self.cacheDB, cacheKey);
                if (cached !== null){
    ...
    this.bulkSetCache = function(cacheUpdates){
        withWriteTxn(function(txn){
            for (const [key, value] of Object.entries(cacheUpdates)) {
                txn.putString(self.cacheDB, key, JSON.stringify(value));
            }
        });
    };
