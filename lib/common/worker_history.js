...
function normalizeTieredHistoryPayload(payload) {
    ...
    for (let index = 0; index < capacities.length; ++index) {
        const tier = normalizeBinaryTier(payload.tiers[index], capacities[index]);
        if (!tier) return null;
...
function getWorkerHistoryState(payload, layout) {
    ...
    if (normalized) return importHistoryPoints(historyPoints(normalized), layout);
...
module.exports = {
    ...
    importHistoryPayload: importHistoryPayload,
