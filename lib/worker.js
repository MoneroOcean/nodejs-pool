...
function createWorkerRuntime() {
    ...
    function pruneTransientState() {
        ...
        Object.keys(state.workersStoppedHashingTime).forEach(function (miner) {
            if (currentTime - state.workersStoppedHashingTime[miner] <= STOPPED_HASHING_TTL_MS) return;
    ...
    function delayedSendWorkerStoppedHashingEmail(miner, currentTime) {
        if (miner in state.workersStartedHashingTime && Date.now() - state.workersStartedHashingTime[miner] <= STOPPED_HASHING_DELAY_MS) {
            delete state.workersStartedHashingTime[miner];
            delete state.workersStoppedHashingTime[miner];
            return;
        }
    ...
    function queuePoolStats(batcher, baseSnapshot, localStats, localTimes, localMinerCount, currentTime, historyTick, statsBufferLength, historyLayout) {
        ["pplns", "global"].forEach(function (key) {
            ...
            if (cachedData === false || typeof cachedData !== "object") {
                ...
                if (historyTick) {
                    cachedData.hashHistory = appendPoolHashHistorySample(cachedData.hashHistory, historyLayout, { ts: currentTime, hs: cachedData.hash });
