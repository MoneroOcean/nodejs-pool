...
function readCacheEntry(txn, key) {
    const cached = readCacheString(txn, key);
...
function createWorkerRuntime() {
    ...
    function queueCacheWriteIfChanged(readTxn, batcher, key, value, currentRaw) {
        const serializedValue = JSON.stringify(value);
        const previousValue = typeof currentRaw === "string" ? currentRaw : readCacheString(readTxn, key);
        if (previousValue === serializedValue) {
            batcher.skippedBytes += Buffer.byteLength(key) + Buffer.byteLength(serializedValue);
            return false;
        }
    ...
    function updateWorkerTransitions(activeMinerKeys, previousMinerKeys, currentTime, readTxn, batcher) {
        for (const miner of previousMinerKeys) {
            ...
            const minerEntry = readCacheEntry(readTxn, miner);
            const minerStats = minerEntry.value;
            if (!minerStats) continue;

            minerStats.hash = 0;
            queueCacheWriteIfChanged(readTxn, batcher, miner, minerStats, minerEntry.raw);
    ...
    function updateShareStats2(height, callback) {
        ...
        try {
            ...
            const minerSetEntry = readCacheEntry(cacheTxn, "minerSet");
            ...
            ["pplns", "global"].forEach(function (key) {
                ...
                const statsEntry = readCacheEntry(cacheTxn, keyStats);
                ...
                queueCacheWriteIfChanged(cacheTxn, batcher, keyStats, cachedData, statsEntry.raw);
            });
            ...
            queueCacheWriteIfChanged(cacheTxn, batcher, "port_hash", portHashCache);
            ...
            for (const [miner, summary] of minerStats.entries()) {
                ...
                queueCacheWriteIfChanged(cacheTxn, batcher, keyStats, { hash: hash, hash2: hash2, lastHash: summary.lastHash });

                if (!historyTick) continue;

                const historyEntry = readCacheEntry(cacheTxn, keyHistory);
                const updatedHistory = workerHistory.appendHistorySample(
                    historyEntry.value,
                    historyLayout,
                    { ts: currentTime, hs: hash, hs2: hash2 }
                );
                if (queueCacheWriteIfChanged(cacheTxn, batcher, keyHistory, updatedHistory, historyEntry.raw)) {
            ...
            for (const [address, identifierSet] of identifiers.entries()) {
                queueCacheWriteIfChanged(cacheTxn, batcher, "identifiers:" + address, Array.from(identifierSet).sort());
            }
            ...
            queueCacheWriteIfChanged(cacheTxn, batcher, "portMinerCount", portMinerCount);
            queueCacheWriteIfChanged(cacheTxn, batcher, "minerSet", setToCacheObject(activeMinerKeys), minerSetEntry.raw);
        } finally {
            cacheTxn.abort();
        }
        ...
        console.log(
            ...
            formatMegabytes(batcher.skippedBytes) + " MB skipped unchanged cache writes). Pool hashrate is: " + poolHashrate
        );
