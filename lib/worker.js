...
function sendAdminTemplateEmail(subjectItem, bodyItem, values, subjectFallback, bodyFallback) {
    ...
    if (subject.indexOf("FYI:") === 0 && typeof global.support.sendAdminFyi === "function") return global.support.sendAdminFyi("worker:" + subjectItem, subject, body);
...
function createWorkerRuntime() {
    ...
    function updateWorkerTransitions(activeMinerKeys, previousMinerKeys, currentTime, staleMinerEntries, batcher) {
        for (const miner of previousMinerKeys) {
            ...
            const timer = setTimeout(delayedSendWorkerStoppedHashingEmail, 10 * 60 * 1000, miner, currentTime);
    ...
    function scanShareSnapshot(height, windowBounds, identifiers, minerStats, portHashes, localStats, localTimes) {
                ...
                for (let found = shareCursor.goToRange(height) === height; found; ++shareCount, found = shareCursor.goToNextDup()) {
                    shareCursor.getCurrentBinary(function (_key, share) {
                        try {
                            ...
                            logger.logError("Worker share", { height, status: "decode failed", detail: share });
                            return;
                        }
    ...
    function queuePoolStats(batcher, baseSnapshot, localStats, localTimes, localMinerCount, currentTime, historyTick, statsBufferLength) {
        ["pplns", "global"].forEach(function (key) {
            ...
            if (cachedData === false || typeof cachedData !== "object") {
                cachedData = { hash, totalHashes: 0, lastHash: localTimes[key], minerCount: localMinerCount[key], hashHistory: [{ ts: currentTime, hs: hash }], minerHistory: [{ ts: currentTime, cn: localMinerCount[key] }] };
            } else {
    ...
    function queueMinerStats(batcher, minerStats, extraSnapshot, historyLayout, currentTime, historyTick) {
        ...
        for (const [miner, summary] of minerStats.entries()) {
            ...
            queueCacheWriteIfChanged(batcher, keyStats, { hash: hash, hash2: hash2, lastHash: summary.lastHash, lastShareAlgo: lastShareAlgoFromPort(summary.port) }, extraSnapshot[keyStats]);
    ...
    function closeWorkerBatcher(batcher) {
        try {
            batcher.close();
            return true;
        } catch (error) {
            ...
            if (!enterLmdbFailStop("writing worker cache", error)) {
                ...
                sendAdminTemplateEmail("workerDbWriteSubject", "workerDbWriteBody", { error: error }, "Pool DB write failed", "Cannot write to pool DB: %(error)s");
            }
            return false;
        }
    }
    ...
    function alertPoolStateChange(currentTime, poolHashrate, poolWorkers) {
        if (state.prevPoolStateTime && currentTime - state.prevPoolStateTime <= HASHRATE_AVG_MIN * 60 * 1000) return;
    ...
    function updateShareStats2(height, callback) {
        ...
        scanShareSnapshot(height, windowBounds, identifiers, minerStats, portHashes, localStats, localTimes);
        ...
        queuePoolStats(batcher, baseSnapshot, localStats, localTimes, localMinerCount, currentTime, historyTick, statsBufferLength);
        ...
        const historyUpdateCount = queueMinerStats(batcher, minerStats, extraSnapshot, historyLayout, currentTime, historyTick);
