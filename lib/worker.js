...
function readCacheString(txn, key) {
    try {
        return txn.getString(localDatabase().cacheDB, key);
    } catch (_error) {
        return null;
    }
}
...
function writeCacheBatch(entries) {
    ...
    let committed = false;
    try {
        for (const [key, value] of entries) txn.putString(database.cacheDB, key, value);
        txn.commit();
        committed = true;
...
function createWorkerRuntime() {
    ...
    function scanShareSnapshot(height, snapshot) {
        ...
        const database = localDatabase();
        const shareTxn = database.env.beginTxn({ readOnly: true });
        /** @type {LmdbCursor | null} */
        let shareCursor = null;
        try {
            shareCursor = new database.lmdb.Cursor(shareTxn, database.shareDB);
            do {
                let shareCount = 0;
