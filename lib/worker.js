...
function createWorkerRuntime() {
    ...
    function enterLmdbFailStop(scope, error) {
        ...
        global.support.sendEmail(
            ...
            "worker paused after LMDB reported map full while " + scope + ": " + detail + "."
        );
        return true;
    }
    ...
    function updateShareStats2(height, callback) {
        ...
        logger.logInfo("Worker cycle", { height: requestedHeight, status: "start" });
        ...
        try {
            ...
            if (!enterLmdbFailStop("writing worker cache", error)) {
                ...
                global.support.sendEmail(global.config.general.adminEmail, "Pool DB write failed", "Can't write to pool DB: " + error);
            }
            callback();
            return;
        }
