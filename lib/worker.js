...
class CacheUpdateBatcher {
    ...
    ensureMarker() {
        ...
        const startedAt = Date.now();
        const txn = global.database.env.beginTxn();
        txn.putString(global.database.cacheDB, CACHE_MARKER_KEY, CACHE_MARKER_KEY);
        txn.commit();
    ...
    close() {
        ...
        const startedAt = Date.now();
        const txn = global.database.env.beginTxn();
        txn.del(global.database.cacheDB, CACHE_MARKER_KEY);
        txn.commit();
        this.totalWriteMs += Date.now() - startedAt;
        this.markerActive = false;
    }
...
function createWorkerRuntime() {
    ...
    function updateShareStats2(height, callback) {
        ...
        console.log(
            "Starting stats collection for " + height +
            " height (history tiers: " + historyLayout.capacities.join("/") + ")"
        );
