...
function createWorkerRuntime() {
    ...
    function scanShareSnapshot(height, windowBounds, identifiers, minerStats, portHashes, localStats, localTimes) {
        ...
        try {
            do {
                let shareCount = 0;
                for (let found = shareCursor.goToRange(currentHeight) === currentHeight; found; ++shareCount, found = shareCursor.goToNextDup()) {
                    shareCursor.getCurrentBinary(function (_key, share) {
                        ...
                        applyShareToSnapshot(decodedShare, windowBounds, identifiers, minerStats, portHashes, localStats, localTimes);
                    });
                }
