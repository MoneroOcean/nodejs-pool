...
function localDatabase() { return getLocalDatabase(global.database); }
...
function scanDb(db, read, reverse, visit) {
    ...
    while (!done) {
        ...
        let cursor = null;
        try {
            cursor = new database.lmdb.Cursor(txn, db);
            let found;
            if (resumeKey === null) {
                found = cursor[first]();
            } else if (reverse) {
...
function cleanCacheDB() {
    ...
    const database = localDatabase();
    const cacheDb = database.cacheDB;
    const now = Date.now();
    ...
    function liveIdentifiers(baseKey, identifiers) {
        ...
        for (let i = 0; i < identifiers.length; ++i) {
            ...
            const stats = database.getCache(`stats:${  baseKey  }_${  identifier}`);
    ...
    function processHistoryKey(key) {
        ...
        const stats = database.getCache(`stats:${  baseKey}`);
        ...
        if (!database.getCache(baseKey) || !stats || now - lastHashMs > WEEK_MS) queueWorkerFamilyDelete(baseKey);
    }
    ...
    function processStatsKey(key, data) {
        ...
        if (!baseKey || baseKey.length < minKeyLength) return;
        const stats = database.getCache(key);
        const lastHashMs = getLastHashMs(stats);
        if (!database.getCache(baseKey) || !database.getCache(`history:${  baseKey}`) || !stats || now - lastHashMs > WEEK_MS) queueWorkerFamilyDelete(baseKey);
    }

    /** @param {string} key */
    function processWorkerKey(key) {
        const stats = database.getCache(`stats:${  key}`);
        if (!stats || !database.getCache(`history:${  key}`) || now - getLastHashMs(stats) > WEEK_MS) queueWorkerFamilyDelete(key);
    }
...
function cleanAltBlockDB() {
    const startedAt = Date.now();
    const database = localDatabase();
    const altblockDb = database.altblockDB;
    const now = Date.now();
...
function collectLockedBlockHashes() {
    ...
    const database = localDatabase();
    [database.getValidLockedBlocks(), database.getValidLockedAltBlocks()].forEach(function (blocks) {
...
function cleanShareDB() {
    ...
    return new Promise(function (resolve, reject) {
        localDatabase().cleanShareDB(function (error) {
            if (error) {
                reject(error);
                return;
            }
