...
function logTaskProgress(taskName, detail) { logger.logInfo(taskName, { detail: detail }); }

function parseJson(data, key) {
    try {
        return JSON.parse(data);
    } catch (_error) {
        logger.logError("Cache DB cleanup", { status: "bad-cache-data", key: key });
        return null;
    }
}
...
function scanDb(db, reader, reverse, visit) {
    ...
    const chunkSize = global.__longRunnerScanChunkSize || SCAN_CHUNK_SIZE;
    let resumeKey = null;
    let started = false;
    let done = false;
    ...
    while (!done) {
        ...
        try {
            ...
            if (!started) {
                ...
                found = cursor.goToRange(resumeKey) ? cursor.goToPrev() : cursor.goToLast();
            } else {
            ...
            let count = 0;
            while (found && count < chunkSize) {
                cursor[reader](function (key, data) {
                    chunk.push([key, Buffer.isBuffer(data) ? Buffer.from(data) : data]);
                });
                resumeKey = found;
                found = cursor[next]();
                ++count;
        ...
        for (let i = 0; i < chunk.length; ++i) visit(chunk[i][0], chunk[i][1]);
    }
}

function isLmdbNotFound(error) {
    return !!error && (error.code === "MDB_NOTFOUND" || String(error.message || error).indexOf("MDB_NOTFOUND") !== -1);
}
...
function cleanCacheDB() {
    ...
    function processWorkerKey(key) {
        ...
        if (!global.database.getCache("stats:" + key) || !global.database.getCache("history:" + key) || now - lastHashMs > WEEK_MS) queueWorkerFamilyDelete(key);
    }
