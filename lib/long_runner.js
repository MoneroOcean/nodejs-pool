...
function cleanCacheDB() {
    ...
    function hasLiveIdentifier(baseKey, identifiers) {
        for (let i = 0; i < identifiers.length; ++i) {
            const stats = global.database.getCache("stats:" + baseKey + "_" + identifiers[i]);
            if (stats && now - stats.lastHash <= DAY_MS) return true;
        }
        return false;
    }
    ...
    function processIdentifierKey(key, data) {
        ...
        if (identifiers.length === 0 || !hasLiveIdentifier(baseKey, identifiers)) {
