    ...
    while (!done) {
        ...
        try {
            ...
            if (!started) {
                ...
                found = atOrAbove !== null && atOrAbove !== false ? cursor.goToPrev() : cursor.goToLast();
            } else {
            ...
            let count = 0;
            while (found !== null && found !== false && count < chunkSize) {
...
function isLmdbNotFound(error) {
    return Boolean(error) && (error.code === "MDB_NOTFOUND" || String(error.message || error).indexOf("MDB_NOTFOUND") !== -1);
}
