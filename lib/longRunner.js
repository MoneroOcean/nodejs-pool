...
function logTaskProgress(taskName, message) {
    console.log("LongRunner " + taskName + ": " + message);
}
...
function cleanCacheDB() {
    ...
    logTaskProgress(
        ...
        ", identifierClears=" + identifierClearCount +
...
function cleanShareDB() {
    logTaskProgress("share DB cleanup", "delegating to database cleaner");
    global.database.cleanShareDB();
}

async function runTask(name, task) {
    const startedAt = Date.now();
    console.log("LongRunner " + name + ": start");
    try {
        await task();
        console.log("LongRunner " + name + ": done in " + formatDurationMs(Date.now() - startedAt));
    } catch (error) {
        console.error("LongRunner " + name + " failed after " + formatDurationMs(Date.now() - startedAt) + ": " + (error && error.stack ? error.stack : error));
    }
}
...
async function startLongRunner() {
    ...
    console.log("LongRunner scheduler: installed " + TASKS.length + " recurring tasks");
