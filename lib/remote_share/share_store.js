...
function releaseCacheMarker(database) {
    const txn = database.env.beginTxn();
    try {
        cacheUpdateMarker.releaseCacheUpdateMarker(txn, database.cacheDB, CACHE_MARKER_KEY);
        txn.commit();
    } catch (error) {
        txn.abort();
        throw error;
    ...
    return {
        storeShares(shares) {
            ...
            for (const share of shares) {
                ...
                const minerEntry = ensureCacheEntry(cacheUpdates, minerID, function createMinerEntry() {
                    return { totalHashes: 0, goodShares: 0 };
                });
                const workerEntry = ensureCacheEntry(cacheUpdates, minerWorkerID, function createWorkerEntry() {
                    return { totalHashes: 0, goodShares: 0 };
                });
                const statsTypeEntry = ensureCacheEntry(cacheUpdates, statsTypeKey, function createStatsEntry() {
                    return { totalHashes: 0, roundHashes: 0 };
                });

                let portSuffix = typeof share.port !== "undefined" && share.port !== global.config.daemon.port
                ...
                if (portSuffix !== "") {
                    const globalPortEntry = ensureCacheEntry(cacheUpdates, globalStatsKey + portSuffix, function createPortGlobalEntry() {
                        return { totalHashes: 0, roundHashes: 0 };
                    });
                    const statsPortEntry = ensureCacheEntry(cacheUpdates, statsTypeKey + portSuffix, function createPortStatsEntry() {
                        return { totalHashes: 0, roundHashes: 0 };
                    });
            ...
            let markerTxn = database.env.beginTxn();
            try {
                cacheUpdateMarker.acquireCacheUpdateMarker(markerTxn, database.cacheDB, CACHE_MARKER_KEY);
                markerTxn.commit();
            } catch (error) {
                markerTxn.abort();
                throw error;
