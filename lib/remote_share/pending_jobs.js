...
const LEGACY_PENDING_JOB_DB_NAME = "remoteSharePendingBlocks";
...
function createLmdbStorage(database, options) {
    const opts = options || {};
    const logger = opts.logger || console;
    const migrateLegacyPendingJobs = opts.migrateLegacyPendingJobs !== false;
    let pendingJobDb = null;
    let legacyPendingJobDb = undefined;
    ...
    function getLegacyPendingJobDb() {
        if (typeof legacyPendingJobDb !== "undefined") return legacyPendingJobDb;
        try {
            legacyPendingJobDb = database.env.openDbi({
                name: LEGACY_PENDING_JOB_DB_NAME,
                create: false
            });
        } catch (_error) {
            legacyPendingJobDb = null;
        }
        return legacyPendingJobDb;
    }
    ...
    function migrateLegacyDbIfNeeded() {
        if (!migrateLegacyPendingJobs) return;
        const legacyDb = getLegacyPendingJobDb();
        if (!legacyDb) return;

        const legacyEntries = [];
        const readTxn = database.env.beginTxn({ readOnly: true });
        try {
            withCursorEntries(readTxn, legacyDb, function collectLegacyEntry(key, value) {
        ...
        if (legacyEntries.length > 0) {
            const writeTxn = database.env.beginTxn();
            try {
                const pendingDb = getPendingJobDb();
                for (const entry of legacyEntries) {
                    writeTxn.putString(pendingDb, entry[0], entry[1]);
                }
                writeTxn.commit();
            } catch (error) {
                writeTxn.abort();
        ...
        const dropTxn = database.env.beginTxn();
        try {
            if (typeof legacyDb.drop === "function") {
                legacyDb.drop({ txn: dropTxn, justFreePages: false });
            } else if (typeof dropTxn.drop === "function") {
                dropTxn.drop(legacyDb, true);
            }
            dropTxn.commit();
            legacyPendingJobDb = null;
            if (legacyEntries.length > 0) {
                logger.log("Migrated " + legacyEntries.length + " legacy pending block jobs from " + LEGACY_PENDING_JOB_DB_NAME + " to " + PENDING_JOB_DB_NAME);
            }
        } catch (error) {
            dropTxn.abort();
            throw error;
...
module.exports = function createPendingJobs(options) {
    ...
    const storage = opts.storage || createLmdbStorage(database, {
        logger,
        migrateLegacyPendingJobs: opts.migrateLegacyPendingJobs
    });
    ...
    function processBlockJob(job, callback) {
        ...
        const coinPort = formatCoinPort(global.config.daemon.port);
        try {
        ...
        global.coinFuncs.getBlockHeaderByHash(blockDataDecoded.hash, function onHeader(err, header) {
            ...
            if (err && isOrphanHeader(header)) {
                ...
                if (timeNow - firstSeen >= orphanGraceMs) {
                    ...
                    saveDrop(job, result.status, "Finalized orphan block " + blockDataDecoded.hash + " on " + coinPort);
                    return callback();
                }
                saveRetry(job, "waiting_orphan", "Waiting for orphan confirmation for " + coinPort + " block hash " + blockDataDecoded.hash);
                return callback();
            }

            if (err || !header || !header.reward) {
                saveRetry(job, "waiting_block_header", "Waiting for block header/reward for " + coinPort + " block hash " + blockDataDecoded.hash);
                return callback();
            }

            blockDataDecoded.value = header.reward;
            const result = saveResolvedBlock(job.blockId, blockDataDecoded);
            saveDrop(job, result.status, (result.status === "stored" ? "Stored block " : "Skipped duplicate block ") + blockDataDecoded.hash + " on " + coinPort);
            return callback();
        });
    }
    ...
    function processAltBlockJob(job, callback) {
        ...
        global.coinFuncs.getPortBlockHeaderByHash(blockDataDecoded.port, blockDataDecoded.hash, function onHeader(err, header) {
            const timeNow = nowMs();
            const coinPort = formatCoinPort(blockDataDecoded.port);
            ...
            if (err && isOrphanHeader(header)) {
                ...
                if (timeNow - firstSeen >= orphanGraceMs) {
                    ...
                    saveDrop(job, result.status, "Finalized orphan altblock " + blockDataDecoded.hash + " on " + coinPort);
                    return callback();
                }
                saveRetry(job, "waiting_alt_orphan", "Waiting for orphan confirmation for altblock " + blockDataDecoded.hash + " on " + coinPort);
                return callback();
            }
            ...
            if (unlockConfirmationDepth && header && header.depth < unlockConfirmationDepth) {
                saveRetry(job, "waiting_for_depth", "Waiting for depth on " + coinPort + " block hash " + blockDataDecoded.hash);
                return callback();
            }

            if (err || !header || !header.reward) {
                if (state.badAltBlocks.has(blockDataDecoded.hash)) {
                    saveDrop(job, "invalid_altblock", "Dropping invalid altblock " + blockDataDecoded.hash + " on " + coinPort);
                    return callback();
                }
                ...
                saveRetry(job, "waiting_altblock_header", "Waiting for altblock header/reward for " + blockDataDecoded.hash + " on " + coinPort);
                return callback();
            }
            ...
            blockDataDecoded.value = header.reward;
            const result = saveResolvedAltBlock(job.blockId, blockDataDecoded);
            saveDrop(job, result.status, (result.status === "stored" ? "Stored altblock " : "Skipped duplicate altblock ") + blockDataDecoded.hash + " on " + coinPort);
            return callback();
        });
    }
