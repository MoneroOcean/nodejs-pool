...
const PENDING_JOB_DB_NAME = "remoteSharePendingBlocks";
const LEGACY_PENDING_JOB_DB_NAME = "remoteSharePending";
...
function createLmdbStorage(database, options) {
    ...
    function migrateLegacyDbIfNeeded() {
        ...
        try {
            legacyDb.drop({ txn: dropTxn });
            dropTxn.commit();
            if (legacyEntries.length > 0) {
                logger.log("Migrated " + legacyEntries.length + " legacy remoteShare pending jobs to " + PENDING_JOB_DB_NAME);
            }
        } catch (error) {
            dropTxn.abort();
            throw error;
        } finally {
            if (legacyPendingJobDb && typeof legacyPendingJobDb.close === "function") {
                legacyPendingJobDb.close();
            }
            legacyPendingJobDb = null;
        }
    }
    ...
    return {
        ...
        loadDueJobs(timeNow, limit) {
            ...
            try {
                ...
                if (jobsByKey.size < limit) {
                    withCursorEntries(txn, getLegacyPendingJobDb(), collectJobs);
                }
            } finally {
                txn.abort();
            }
...
module.exports = function createPendingJobs(options) {
    ...
    const storage = options && options.storage ? options.storage : createLmdbStorage(database, {
        logger: logger,
        migrateLegacyPendingJobs: !options || options.migrateLegacyPendingJobs !== false
    });
