...
function formatBlockFields(hash, extraFields) {
...
function formatAltBlockFields(blockDataDecoded, extraFields) {
    return Object.assign({
...
module.exports = function createPendingJobs(options) {
    ...
    function saveRetry(job, stateName, message, options) {
        ...
        job.nextAttemptAt = timeNow + nextRetryDelayMs;
    ...
    function processBlockJob(job, callback) {
        ...
        const blockFields = formatBlockFields(blockDataDecoded.hash);
        ...
        global.coinFuncs.getBlockHeaderByHash(blockDataDecoded.hash, function onHeader(err, header) {
            ...
            if (err && isOrphanHeader(header)) {
                ...
                if (timeNow - firstSeen >= orphanGraceMs) {
                    ...
                    saveDrop(job, result.status, formatLogEvent("Pending block", Object.assign({}, blockFields, { status: "orphan-finalized" })));
                    return callback();
                }
                saveRetry(job, "waiting_orphan", formatLogEvent("Pending block", withDetail(Object.assign({}, blockFields, {
    ...
    function processAltBlockJob(job, callback) {
        ...
        const altBlockFields = formatAltBlockFields(blockDataDecoded);
        ...
        global.coinFuncs.getPortBlockHeaderByHash(blockDataDecoded.port, blockDataDecoded.hash, function onHeader(err, header) {
            ...
            if (err && isOrphanHeader(header)) {
                ...
                if (timeNow - firstSeen >= orphanGraceMs) {
                    ...
                    saveDrop(job, result.status, formatLogEvent("Pending altblock", Object.assign({}, altBlockFields, { status: "orphan-finalized" })));
                    return callback();
                }
                saveRetry(job, "waiting_alt_orphan", formatLogEvent("Pending altblock", withDetail(Object.assign({}, altBlockFields, {
                    status: "waiting-orphan-confirmation"
                }), err, header)));
                return callback();
            }
            ...
            if (unlockConfirmationDepth && header && header.depth < unlockConfirmationDepth) {
                saveRetry(job, "waiting_for_depth", formatLogEvent("Pending altblock", Object.assign({}, altBlockFields, { status: "waiting-maturity" })));
                return callback();
            }
