        ...
        const flushNextChunk = () => {
            ...
            try {
                ...
                if (enterLmdbRejectMode("flushing queued shares", error, true)) {
                    ...
                    // Reject mode means this queue will not drain; release waiters
                    // (e.g. stop()) instead of leaving them hung on a non-empty queue.
                    state.shareDrainActive = false;
                    resolveShareDrainWaiters();
                    return;
                }
                // Non-map-full write error (e.g. transient I/O): keep the batch for a later
                // retry instead of throwing out of this timer/setImmediate callback, which would
                // be uncaught and crash the process. storeShares uses a single atomic txn so
                // nothing in the batch was committed. Rate-limit the log; the next queued share
                // reschedules the drain. Waiters are resolved (not rejected) so stop() proceeds.
                state.shareQueue = batch.concat(state.shareQueue);
                state.shareDrainActive = false;
                const timeNow = Date.now();
                if (timeNow - state.shareDrainLastErrorAt >= 30 * 1000) {
                    state.shareDrainLastErrorAt = timeNow;
                    logger.logError("Share flush failed", { detail: formatError(error) });
                }
                resolveShareDrainWaiters();
                return;
            }
