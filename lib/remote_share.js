"use strict";
// Node exposes the cluster singleton through CommonJS; its declarations model an ESM default.
...
// Match nginx's loopback upstream without depending on localhost DNS ordering.
...
/** @typedef {(worker: ClusterWorker, code?: number, signal?: string) => void} ClusterListener */
/** @typedef {{cluster?: import("node:cluster").Cluster, os?: typeof os, database?: import("../types/runtime").LocalDatabaseRuntime, shareStore?: import("./remote_share/share_store").ShareStore, pendingJobs?: import("./remote_share/pending_jobs").PendingJobs, shareFlushIntervalMs?: number, shareBatchSize?: number, pendingJobPollMs?: number, shareSummaryIntervalMs?: number, requestSummaryIntervalMs?: number, clusterEnabled?: boolean, port?: number, host?: string, numWorkers?: number}} RuntimeOptions */
/** @param {unknown} error */
...
function createRemoteShareRuntime(options) {
    ...
    const requestedBatchSize = opts.shareBatchSize;
    const shareBatchSize = typeof requestedBatchSize === "number" && Number.isSafeInteger(requestedBatchSize) && requestedBatchSize > 0
        ? requestedBatchSize : DEFAULT_SHARE_BATCH_SIZE;
    ...
    // Both stores are local LMDB consumers. Resolve the role once at the boundary so a
    // remote uplink cannot accidentally be passed into a local persistence helper.
    const database = opts.database || (opts.shareStore && opts.pendingJobs ? undefined : getLocalDatabase(global.database));
    const storeOptions = database ? { database } : {};
    const shareStore = opts.shareStore || createShareStore(storeOptions);
    const pendingJobs = opts.pendingJobs || createPendingJobs(storeOptions);

    const app = express();
    const state = {
        attachedWorkers: new Set(/** @type {ClusterWorker[]} */ ([])),
        clusterListeners: /** @type {{eventName: string, listener: ClusterListener}[]} */ ([]),
        pendingJobInterval: /** @type {NodeJS.Timeout | null} */ (null),
        pendingJobLastErrorAt: 0,
        parentMessageListener: /** @type {((message: unknown) => void) | null} */ (null),
        lmdbRejecting: false,
        lmdbRejectDetail: "",
        lmdbRejectScope: "",
        requestSummaryInterval: /** @type {NodeJS.Timeout | null} */ (null),
        requestStats: {
            altBlockRequests: 0,
            authRejected: 0,
        ...
        server: /** @type {import("node:http").Server | null} */ (null),
        shareDrainActive: false,
        shareDrainWaiters: /** @type {{resolve: () => void, reject: (reason: unknown) => void}[]} */ ([]),
        shareQueue: /** @type {unknown[]} */ ([]),
        shareSummaryInterval: /** @type {NodeJS.Timeout | null} */ (null),
        shareStats: {
            accepted: 0,
            flushBatches: 0,
            flushed: 0,
        ...
        shareTimer: /** @type {NodeJS.Timeout | null} */ (null),
        started: false
    };
    ...
    function notifyWorkersRejectWrites(scope, detail) {
        ...
        for (const worker of state.attachedWorkers) {
            ...
            } catch (_error) { /* worker may have exited; ignore send failure */ }
        }
    }

    // Pause the runtime: reject new share/block frames, notify cluster workers, and email the
    // admin once. Used for any unrecoverable LMDB write failure; the map-full case keeps its
    // existing wording, other write errors get a generic message.
    /** @param {string} scope @param {unknown} error @param {boolean} [suppressLog] */
    function enterRejectMode(scope, error, suppressLog) {
        const detail = formatLmdbError(error);
        const mapFull = isLmdbMapFull(error);
        ...
        if (firstEnter || !suppressLog) {
            logger.logError("LMDB", { status: mapFull ? "map full" : "write failed", scope, detail });
        }
        if (firstEnter) {
            ...
            if (global.support && typeof global.support.sendEmail === "function" && global.config?.general?.adminEmail) {
                ...
                const subject = mapFull
                    ? renderEmailTemplate("remoteShareLmdbSubject", values, "remote_share rejecting new work due to LMDB full")
                    : renderEmailTemplate("remoteShareWriteSubject", values, "remote_share rejecting new work due to LMDB write failure");
                const body = mapFull
                    ...
                    : renderEmailTemplate("remoteShareWriteBody", values, "remote_share is rejecting new share and block frames after an LMDB write failure while %(scope)s: %(detail)s.");
                global.support.sendEmail(global.config.general.adminEmail, subject, body);
            }
        }
    }

    // Pause only when LMDB reports the map is full. Used by pending-job processing, where other
    // errors may be transient or non-LMDB (e.g. daemon RPC) and should not stop ingress.
    /** @param {string} scope @param {unknown} error @param {boolean} [suppressLog] */
    function enterLmdbRejectMode(scope, error, suppressLog) {
        if (!isLmdbMapFull(error)) return false;
        enterRejectMode(scope, error, suppressLog);
        return true;
    }
    ...
    function drainShareQueue() {
        ...
        const flushNextChunk = () => {
            ...
            try {
                ...
                // Any LMDB write failure pauses the runtime (reject mode) instead of retrying a
                // broken DB or throwing out of this timer/setImmediate callback (which would be
                // uncaught and crash the process). Keep the un-committed batch (storeShares is a
                // single atomic txn), stop accepting new work, notify the admin, and release
                // waiters so stop() proceeds. The operator restarts once the DB is healthy.
                enterRejectMode("flushing queued shares", error, true);
            ...
            if (state.shareQueue.length > 0) setImmediate(flushNextChunk);
            else finishShareDrain();
        };
    ...
    function handleParentMessage(message) {
        ...
        state.lmdbRejecting = true;
        state.lmdbRejectScope = "scope" in message && typeof message.scope === "string" ? message.scope : "worker notification";
        state.lmdbRejectDetail = "detail" in message && typeof message.detail === "string" ? message.detail : "";
    ...
    function handleWorkerMessage(message) {
        ...
        case "remoteShareShare":
            if ("share" in message && message.share && !state.lmdbRejecting) queueShare(message.share);
            break;
    ...
    function processPendingJobs(reason) {
        ...
        try {
            ...
            if (timeNow - state.pendingJobLastErrorAt >= 30 * 1000) {
                ...
                logger.logError("Pending job processing failed", { reason, detail: formatError(error) });
            }
        }
    }

    // Tri-state result the callers branch on: false = decode failed (-> 400),
    // null = enqueue failed (-> 500), true = queued.
    /**
     * @template T
     * @param {import("../types/runtime").WSDataMessage} msgData
     * @param {(data: Buffer) => T} decoder
     * @param {(id: number, data: Buffer, value: T) => void} enqueue
    ...
    /** @param {import("../types/runtime").ExpressResponse} res @param {200 | 400 | 403 | 500 | 503} statusCode */
    function respond(res, statusCode) {
        /** @type {`status${typeof statusCode}`} */
    ...
    function logRequestSummary() {
        if (!Object.values(state.requestStats).some(value => value !== 0)) return;

        /** @type {import("./common/logging").LogFields} */
        const fields = {
        ...
        for (const key of Object.keys(state.requestStats)) {
            Object.assign(state.requestStats, {[key]: 0});
        }
    }

    /** @param {[string, keyof typeof state.requestStats][]} entries */
    function collectPositiveStats(entries) {
        const parts = [];
        for (const [label, key] of entries) {
            const value = state.requestStats[key];
    ...
    /** @param {import("../types/runtime").ExpressRequest} req @param {import("../types/runtime").ExpressResponse} res */
    function handleLeafFrame(req, res) {
        state.requestStats.requests += 1;
        let msgData;
        try {
            if (!Buffer.isBuffer(req.body)) throw new TypeError("Expected a binary leaf frame");
    ...
    function listenServer() {
        ...
        const server = app.listen(opts.port ?? DEFAULT_PORT, opts.host ?? DEFAULT_HOST, function onListen() {
            const address = server.address();
            logger.logInfo("Listen", { service: "remote-share", host: opts.host ?? DEFAULT_HOST, port: address && typeof address === "object" ? address.port : opts.port ?? DEFAULT_PORT });
        });
        state.server = server;
    }
    ...
    /** @param {string} eventName @param {ClusterListener} listener */
    ...
    /** @param {"shareTimer" | "pendingJobInterval" | "requestSummaryInterval" | "shareSummaryInterval"} name @param {(timer: NodeJS.Timeout) => void} clearFn */
...
module.exports = Object.assign(runtime || {}, { createRemoteShareRuntime });
