...
function postWithCallback(url, body, callback) {
    request.post(url, {
        body: body,
        timeout: 30000,
    }, function (error, response, responseBody) {
        if (!error && response) {
            return callback(null, { statusCode: response.statusCode }, responseBody);
        }
        callback(error);
    });
}
    ...
    this.sendQueue = async.queue(function (task, callback) {
        async.doUntil(
            function (intCallback) {
                postWithCallback(global.config.general.shareHost, task.body, function (error, response, body) {
