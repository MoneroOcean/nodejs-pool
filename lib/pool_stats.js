...
function setCaches(entries) {
    ...
    if (typeof database.bulkSetCache === "function") {
        database.bulkSetCache(entries);
        return;
    }

    Object.keys(entries).forEach(function (key) {
        database.setCache(key, entries[key]);
    });
}
...
function scanDb(db, visit, reverse) {
    ...
    const cursor = new database.lmdb.Cursor(txn, db);
...
function buildBlockState() {
    ...
    scanDb(localDatabase().blockDB, function (key, data) {
...
function buildAltBlockState() {
    ...
    scanDb(localDatabase().altblockDB, function (key, data) {
...
function refreshBlockState() {
    ...
    const entryCount = getDbEntryCount(localDatabase().blockDB);
...
function refreshAltBlockState() {
    ...
    const entryCount = getDbEntryCount(localDatabase().altblockDB);
    if (entryCount !== null && entryCount < state.totals.global) {
        historyState.altblocks = buildAltBlockState();
        return historyState.altblocks;
    }
...
function collectRecentAltBlockSummaries(altBlockState, minBlockRewards) {
    ...
    scanDb(localDatabase().altblockDB, function (_key, data) {
        const block = global.protos.AltBlock.decode(data);
...
async function refreshPoolPorts(poolServers) {
    try {
        ...
        localDatabase().setCache("poolPorts", cache);
        return cache;
    } catch (error) {
        logger.logError("SQL", { status: "query-failed", detail: formatError(error) });
        return false;
...
async function refreshPoolInformation() {
    try {
        ...
        localDatabase().setCache("poolServers", poolServers);
...
async function refreshBlockHeader() {
    ...
    localDatabase().setCache("networkBlockInfo", networkInfoCache);
