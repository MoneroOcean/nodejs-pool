...
async function refreshBlockHeader() {
    ...
    for (const port of ports) {
        ...
        nextInfo.difficulty = networkDifficulty(port, hasRpcResult ? rpcResult.result.difficulty : result.body.difficulty);
...
function badHeaderStart(port, error) {
    const values = { port: port, error: formatError(error) };
    logger.logError("Node monitor", { port: port, status: "header-unavailable", error: values.error, detail: "skipping node monitor" });
...
function nodeLagStatus(options) {
    const { chain, port, height, topHeight, xmrHeight, xtmHeight } = options;
...
async function monitorNodes() {
    try {
        ...
        if (matchingRows > 1) {
            const xmr = nodeLagStatus({ chain: "xmr", port: mainPort, height: xmrHeight, topHeight: topXmrHeight, xmrHeight, xtmHeight });
            const xtm = nodeLagStatus({ chain: "xtm", port: mainPort, height: xtmHeight, topHeight: topXtmHeight, xmrHeight, xtmHeight });
