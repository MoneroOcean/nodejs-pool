...
function loadSharedStats(minBlockRewards) {
    const activePort = global.config.daemon.port;
...
async function refreshPoolStats() {
    ...
    const pricePromise = priceCache.btc === 0 && priceCache.usd === 0 && priceCache.eur === 0 ? refreshPrices() : Promise.resolve(priceCache);
...
function rpcPortDaemon(port, method, params) {
    return new Promise(function (resolve) {
        global.support.rpcPortDaemon(port, method, params, resolve);
    });
}
...
async function refreshBlockHeader() {
    ...
    for (const port of ports) {
        ...
        if (port !== global.config.daemon.port) continue;

        const rpcResult = await rpcPortDaemon(port, "get_info", []);
        nextInfo.difficulty = rpcResult.result ? rpcResult.result.difficulty : result.body.difficulty;
...
async function monitorNodes() {
    try {
        ...
        const header = await getLastBlockHeader(global.config.daemon.port);

        if (header.error !== null && typeof header.error !== "undefined") {
            badHeaderStart(global.config.daemon.port);
            return false;
        }

        badHeaderStop(global.config.daemon.port);
        ...
        if (isMasterDaemonIssue) {
            global.coinFuncs.fixDaemonIssue(header.block.height, topHeight, global.config.daemon.port);
        }
...
async function startPoolStats() {
    await runTask("price refresh", refreshPrices);
    await Promise.all([
