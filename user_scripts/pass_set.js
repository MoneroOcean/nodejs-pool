...
cli.init(function() {
	userDb.requireMissingUser(user, "Your password is already set, so can not set it again", { logRowsOnFailure: true }).then(function () {
		return userDb.runLoggedQuery("INSERT INTO users (username, email, enable_email) VALUES (?, ?, 0)", [user, pass], "INSERT INTO users (username, email, enable_email) VALUES (" + user + ", " + pass + ", 0)");
	}).then(function () {
		userDb.finish("Done.");
	});
});
